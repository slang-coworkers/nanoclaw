...
describe('applyProviderSkill on an already-installed provider', () => {
  it('reports no change, so the caller skips the image rebuild', async () => {
    ...
    // `applied` is EMPTY here, and that is correct: install.ts now passes
    // `skipEffects: ['build','test','external']`, so the flow-owned runs are
    // skipped rather than executed as no-ops and journalled. (They used to be
    // counted, which is what the old `applied.length > 0` assertion pinned.)
    // The caller rebuilds the container image when `changed`, so a false positive
    // costs a no-op rebuild and fails hard wherever the build can't run — that is
    // the guarantee this case exists for. "The skill really applies" is pinned by
    // the missing-barrel case below, which asserts a real journal entry.
