 ...
 */
function buildStaleOrigin(root: string, opts: { hostLifecycleTest?: boolean } = {}): string {
  const hostLifecycleTest = opts.hostLifecycleTest ?? true;
  const seed = path.join(root, 'seed');
  fs.mkdirSync(seed);
  git(seed, 'init', '-q', '-b', 'base', '.');
  ...
  if (hostLifecycleTest) w('src/host-lifecycle.test.ts', 'test dead\n');
...
describe('merge-train.sh --reconcile-stale (prod update)', () => {
  ...
  it('ABORTS rather than discard an uncommitted local edit to an owned path (prod hotfix safety)', () => {
    const { root, repo } = reconcile();
    try {
      // A local hotfix to an owned file none of the branches touch: it survives
      // the merges and the blanket checkout would silently wipe it.
      fs.writeFileSync(path.join(repo, 'src', 'keep.ts'), 'LOCAL HOTFIX\n');
      const res = runMergeTrainArgs(repo, ['--reconcile-stale', 'nv-main', 'nv-overlay']);
      expect(res.status).toBe(1);
      expect(res.stderr).toMatch(/UNCOMMITTED local changes|stash/);
      // The hotfix is intact — nothing was overwritten.
      expect(fs.readFileSync(path.join(repo, 'src', 'keep.ts'), 'utf-8')).toBe('LOCAL HOTFIX\n');
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
  });

  it('removes the dead module even when the overlay carries no matching test (per-path rm)', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'mt-reconcile-hl-'));
    const origin = buildStaleOrigin(root, { hostLifecycleTest: false });
    const repo = cloneOn(root, origin, 'c', 'base');
    try {
      const res = runMergeTrainArgs(repo, ['--reconcile-stale', 'nv-main', 'nv-overlay']);
      expect(res.status, res.stderr).toBe(0);
      // The module is gone even though its test never existed — a bundled
      // `git rm` of both paths would have failed on the absent test and left the
      // module behind while still reporting success.
      expect(fs.existsSync(path.join(repo, 'src', 'host-lifecycle.ts'))).toBe(false);
      expect(fs.existsSync(path.join(repo, 'src', 'host-lifecycle.test.ts'))).toBe(false);
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
  ...
  it('reconciles even when nothing newly merges — a box composed by a prior UNFLAGGED merge', () => {
    const { root, repo } = reconcile();
    try {
      // Compose WITHOUT the flag first: the stale drift is left in place.
      expect(runMergeTrainArgs(repo, ['nv-main', 'nv-overlay']).status).toBe(0);
      expect(fs.existsSync(path.join(repo, 'src', 'legacy.test.ts'))).toBe(true);

      // Re-run WITH the flag: the branches are already ancestors (nothing merges
      // this run), but reconcile must still run and clean the drift — otherwise
      // an already-stale box could never be reconciled.
      const res = runMergeTrainArgs(repo, ['--reconcile-stale', 'nv-main', 'nv-overlay']);
      expect(res.status, res.stderr).toBe(0);
      expect(fs.existsSync(path.join(repo, 'src', 'legacy.test.ts'))).toBe(false);
      expect(fs.existsSync(path.join(repo, 'src', 'host-lifecycle.ts'))).toBe(false);
