...
document.getElementById('timeline-list')?.addEventListener('click', async (e) => {
  ...
  try {
    ...
    updateTimeline();
    const newEntries = container.querySelectorAll('.tl-entry');
    const scrollTarget = newEntries[newEntries.length - prevCount];
...
function renderCwSidebar() {
  ...
  list.innerHTML = coworkers.map((cw) => {
    ...
    return `<div class="cw-item${selected}" data-folder="${esc(cw.folder)}">
      ...
      ${unread ? '<div class="cw-unread-badge"></div>' : ''}
    </div>`;
  }).join('');
  // Click handlers
...
async function renderCwWork(subpath, isDir) {
  ...
  if (wsInput) {
    ...
    wsInput.placeholder = cwd ? `runs in /workspace/group/${cwd}` : 'runs in /workspace/group/';
  }
...
cwState.shellCwd = '/workspace/group';
function execCwShellCommand(cmd) {
  ...
  if (cdMatch) {
    ...
    // Also sync work view path if it's a /workspace/group subpath
    if (cwState.shellCwd.startsWith('/workspace/group')) {
      const rel = cwState.shellCwd.slice('/workspace/group'.length).replace(/^\//, '');
...
function execWorkShellCommand(cmd) {
  ...
  if (cdMatch) {
    let target = cdMatch[1].trim().replace(/^['"]|['"]$/g, '');
    let newPath;
    if (target === '/' || target === '/workspace/group') {
      newPath = '';
    } else if (target === '..') {
      newPath = cwState.workPath ? cwState.workPath.replace(/\/?[^/]+$/, '') : '';
    } else if (target.startsWith('/workspace/group/')) {
      newPath = target.slice('/workspace/group/'.length);
    } else if (target.startsWith('/')) {
  ...
  const dir = '/workspace/group' + (cwState.workPath ? '/' + cwState.workPath : '');
...
// Export coworker as JSON bundle
document.getElementById('cw-export-btn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    a.download = `coworker-${cwState.selected}-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    URL.revokeObjectURL(a.href);
  } catch (e) { alert('Export error: ' + e.message); }
});

// Import coworker from JSON bundle
document.getElementById('cw-import-btn')?.addEventListener('click', () => {
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      if (!data.coworker?.name) { alert('Invalid export file'); return; }
      const fileCount = Object.keys(data.files || {}).length;
      const taskCount = (data.tasks || []).length;
      if (!confirm(`Import "${data.coworker.name}"?\n\n${fileCount} files, ${taskCount} scheduled tasks\nFolder: ${data.coworker.folder}`)) return;
