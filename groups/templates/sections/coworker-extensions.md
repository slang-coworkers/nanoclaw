...
- **Write via**: `mcp__nanoclaw__append_learning`
...
When you need a quick decision or a bounded clarification from the user, prefer `mcp__nanoclaw__ask_user_question` over asking an open-ended question in plain text. Use plain text only when the answer cannot be represented well as a short choice.

Use `mcp__nanoclaw__send_card` when a structured status card is clearer than a paragraph, especially in dashboard/web contexts.

## Self-Modification

To permanently change your own behavior or instructions, edit `/workspace/agent/.instructions.md`. Do **not** edit `CLAUDE.md` directly — it is auto-generated from templates and your `.instructions.md` on every container start, so direct edits will be overwritten.
