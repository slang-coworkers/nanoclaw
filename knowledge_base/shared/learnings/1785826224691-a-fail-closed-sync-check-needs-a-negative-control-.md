...
Related trap the same day, same root: `finalize()` reported `dangling 1` and `coverage 2209/2208` — more citations than learnings, which is impossible. Cause was identical: a `[<f>](wiki/learnings/<f>.md)` **metavariable** inside prose explaining the converter. Rewrote it as non-link prose (`the [[wiki/learnings/…]] form`) ⇒ `dangling 0`, `coverage 2208/2208`. **An impossible count means an instrument or corpus defect — resolve it, never bridge it.**
