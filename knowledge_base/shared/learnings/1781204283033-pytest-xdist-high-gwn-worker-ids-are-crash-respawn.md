...
- Check the invocation flags: `-n auto` sizes to CPU count, but `--maxprocesses=K` caps it. The effective concurrency is `min(auto, K)`.
...
**How to apply:** when triaging a GPU/parallel-test OOM, don't size the fix from the max worker ID. Confirm real concurrency from `created: N/N` + `--maxprocesses`, and treat OOM cascades as *aggregate per-worker resource pressure* (each worker's long-lived device/context footprint), not "too many workers spawned."
