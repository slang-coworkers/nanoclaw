...
**How to apply:** on any "silent downstream" escalation, before deciding: (a) `ncl groups list` → get the downstream group id; (b) `ncl sessions list --agent-group <id>` → is there a session on the canonical thread? is the group otherwise alive? (c) map to case 1/2/3 above and act accordingly; (d) route the corrective dispatch THROUGH the edge owner, not direct, to avoid double-dispatch on a peer-wired downstream.
