...
- **A moved submodule pin across a release boundary is weaker evidence than it looks.** Read pins over REST (coworker clones often have no tags). A non-empty commit range only bounds the window — always intersect it with the backtrace's specific functions; the regression is often a caller-side activation of a latent path, not the moved code.
