---
title: "CI Build: Git Checkout Depth & Submodule Pins"
type: concept
group: ci-tooling
...
Four git/checkout hazards that produce confidently wrong answers in CI, review, and provenance work:

- **Untracking a checked-in build binary** (`git rm --cached` + `.gitignore`) is safe **only if no deploy script, CI workflow, or Makefile CONSUMES the tracked copy** — grep for consumers before concluding. Untracking stops *future* bloat only; it does not shrink existing history/packs, and a full history purge (git-filter-repo/BFG) rewrites shared history and is disproportionate for one blob.
- **Shallow checkouts give confidently wrong answers, per clone, silently.** Audit per clone, not per workspace. Three failure modes: (1) `git log -S` / `blame` / `--follow` name the graft root as the introducer of a line they can't see behind, with full confidence; (2) `git show --stat` on the graft root reports a whole-tree diff (a 191k-line reading for an 8-line change) — and `--depth 1` makes YOUR OWN HEAD the graft root; (3) object-not-found for a real-but-unfetched SHA is byte-identical to a fabricated one, manufacturing a false "ref absent" — the dangerous mode. Rules: check depth first; route diff / provenance / existence facts to the REST API, not local git; verify provenance by the PATCH, not by proximity; always name the ref. The correct silent-regime discriminator is empty `%P` on HEAD guarded by `--is-shallow-repository` — NOT `head -1 .git/shallow`, which has a false negative on multi-branch clones.
- **Never `--depth 1` a submodule update.** A `--depth N` clone of the *superproject* is fine (submodule SHAs come from the gitlink), but a shallow *submodule fetch* gets only branch tips, and pinned SHAs aren't tips — the fetch fails, then keeps going with exit 0, leaving empty worktrees that look populated. Verify with `^+` (wrong commit) AND `^-` (uninitialized) AND a file count, all `--recursive`; CI uses no depth flag for exactly this reason.
- **A moved submodule pin across a release boundary is weaker evidence than it looks.** Read pins over REST (coworker clones often have no tags). A non-empty commit range only bounds the window — always intersect it with the backtrace's specific functions; the regression is often a caller-side activation of a latent path, not the moved code. And to resolve a **keep-ours** pin merge conflict safely, first confirm master's pin is a strict ancestor of the PR's via `compare/<master-pin>...<pr-pin>` (`status==ahead` AND `behind_by==0`) — otherwise keep-ours silently reverts master's own pin move.
...
**Keep-ours on a submodule-pin merge conflict needs an ancestor check.** When a PR bumps a submodule pin (e.g. `external/slang-rhi`) and later hits a merge conflict because master ALSO moved that pin, resolving by "keep ours" (the PR's forward pin) is safe ONLY if master's pin is a **strict ancestor** of the PR's pin — otherwise keep-ours silently REVERTS whatever master intentionally picked up in its own pin move, a subtle regression no test may catch. Verify read-only (no clone) with the compare API on the submodule repo: `gh api repos/<owner>/<subrepo>/compare/<MASTER_PIN>...<PR_PIN> --jq '{status,ahead_by,behind_by,merge_base:.merge_base_commit.sha}'` — safe to keep-ours iff `status=="ahead"` AND `behind_by==0` (equivalently `merge_base == MASTER_PIN`), which proves MASTER_PIN is an ancestor so the PR's pin is a superset and drops nothing; a `behind_by>0` means keep-ours would revert master's newer work and you must instead re-merge/rebase onto a descendant of master's pin. On shader-slang/slang#9030 / PR #13105 the PR bumped slang-rhi `d6d31411`→`079b75b54c` (for PR #852's coop-matrix-2 feature names) while master had moved its pin to `29dc332e55` (via a different PR #12522, which PREDATED #852); the compare returned `status=ahead`, `behind_by=0`, `merge_base=29dc332e55` ⇒ strict ancestor ⇒ keep-ours confirmed correct. This applies to any pinned dependency (submodule gitlink, CMake FetchContent `GIT_TAG`, a vendored SHA) — and don't take a coworker's "it's a strict superset" ancestor claim on faith on a maintainer-watched PR; one compare call confirms it ([verify ancestor before trusting a keep-ours submodule-pin merge conflict resolution](../learnings/1789529223205-verify-ancestor-before-trusting-a-keep-ours-submod.md)).

## Updating a PR Branch's History Safely (2026-09-18 fold)

**Rebase, don't merge, to update a bot PR branch past master `.github/workflows` changes.** The `nv-slang-bot` App lacks the `workflows` permission, so a `git merge origin/master` that pulls in any `.github/workflows/**` change since the merge-base produces a merge commit whose push is **rejected** — the App is treated as creating/updating a workflow. `git rebase origin/master` instead replays only the bot's own (non-workflow) commits onto master HEAD; master's workflow changes arrive via the new base (already in the repo, authored by others), so the push introduces no workflow edits and passes. Push with `git push --force-with-lease=<branch>:<remote-sha>` pinned to the known remote sha. Caveat: this rewrites history and replaces any human merge commit on the branch — note the swap when reporting (the PR comment is operator-gated). If the bot's *own* commit must add/edit a workflow file there is no workaround; embed the proposed YAML in the PR body for a human with `workflows` permission ([workaround: rebase (not merge) to update a bot PR branch past master workflow changes](../learnings/1789475499114-workaround-rebase-not-merge-to-update-a-bot-pr-bra.md)).
...
- [verify ancestor before trusting a keep-ours submodule-pin merge conflict resolution](../learnings/1789529223205-verify-ancestor-before-trusting-a-keep-ours-submod.md) — keep-ours is safe only if master's pin is a strict ancestor of the PR's (`compare/<master>...<pr>` → status=ahead, behind_by=0, merge_base==master); else it silently reverts master's pin move
- [workaround: rebase (not merge) to update a bot PR branch past master workflow changes](../learnings/1789475499114-workaround-rebase-not-merge-to-update-a-bot-pr-bra.md) — bot App lacks `workflows` permission; rebase replays only non-workflow commits so the push passes; `--force-with-lease` pinned to remote sha
- [reconciling a diverged PR branch without force-push](../learnings/1789505131994-reconciling-a-diverged-pr-branch-without-force-pus.md) — remote had a maintainer Merge-master commit; prove no remote content lost, then `git rebase --onto FETCH_HEAD` and fast-forward push
