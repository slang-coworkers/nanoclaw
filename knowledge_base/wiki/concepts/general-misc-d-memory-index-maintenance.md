...
- **okf-synthesis: run `finalize` exactly once per run and fold the current top offender first.** Every call appends a history row, and ESCALATE fires on three non-shrinking rows *or* an unchanged top offender, so a double run or an unfolded #1 escalates falsely.
- **A path-form `[[dir/name]]` resolves in okf_synth only with `.md`**; and with `autoMemoryEnabled: false` nothing loads `memory/MEMORY.md`, so check the setting before assuming a second loader.
...
One DANGLING-LINK shape is real rather than an artifact. The scanner resolves a path-form `[[dir/name]]` (one containing a slash) only if it ends in `.md`, so `[[imported/hold-12192]]` is flagged even though `imported/hold-12192.md` exists, and a bare `[[name]]` without a slash is ignored; subagents writing memory links should use `[[dir/name.md]]`. Two neighbouring facts come from the same slang-fixer sweep (2026-09-26). With `"autoMemoryEnabled": false` in `/home/node/.claude/settings.json` and no MEMORY.md in the CLI store, `/workspace/agent/memory/MEMORY.md` is loaded by nothing; it was a 36 KB inert dossier, retired to a stub after verifying that all 74 of its targets were index-reachable, so check the setting before assuming a second loader. And parallel fold subagents on live ledgers (active-fixlog/active-holds) worked, shrinking 78 KB to 22 KB with no fact loss, when each carried unique facts into the leaf under `## Carried from <src> (date)` before cutting the row and re-read before every Edit, because they share leaf files ([OKF synth: path-form wikilinks need .md; MEMORY.md inert when autoMemoryEnabled=false](../learnings/1790397155008-okf-synth-path-form-wikilinks-need-md-memory-md-in.md)).
...
The rule for the running side: `okf_synth.py finalize` appends a history row on every call, and ESCALATE fires when the last three rows show a non-shrinking backlog **or** the same top offender (path and size). Running finalize twice in one run (seen 2026-09-22/23/25 in slangpy-triager) burns two of the three stall slots on one run, so a single skipped fold the next day escalates falsely. Run it once per run, and fold the current #1 offender first; otherwise the top-unchanged trigger fires on day 3 even while the backlog shrinks. Before distilling a loose `triage-N.md`, grep `imported/` for a same-issue concept first ([okf-synthesis: run finalize exactly once, and always fold the top offender](../learnings/1790396413821-okf-synthesis-run-finalize-exactly-once-and-always.md)).
