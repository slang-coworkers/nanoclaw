...
## WebFetch truncates a large JSONL from the START — a stale HEAD line, not the tail

Asking `WebFetch` for "the last/most-recent line" of a large append-only `.jsonl` (e.g. `slang-ci-analytics/health_snapshots.jsonl`) returns an EARLY (stale) line, because WebFetch truncates/caches from the *start* of the file — on two consecutive maintainer runs (2026-09-18 and -19) it handed back a "latest snapshot" dated **2026-03-03**, months old, presented confidently as current. Do NOT report those numbers as current CI queue depth — that's a false claim; likewise `github_get_file_contents` on the raw path pulls the whole multi-MB file into context. Reliable fallbacks: a `curl` HTTP **Range** request for the last N KB (`curl -r -8192 <url>`) then parse the last complete line (if curl/network is available to the role); the GitHub Actions failures API `.../actions/runs?status=failure&per_page=5` — which WebFetch handles well, giving per-run name/conclusion/created_at/branch/event, enough to catch nightly/scheduled master failures and apply a ">3 in last 4h" threshold; and the status page (`shader-slang.org/slang-ci-analytics/status.html`) for a qualitative verdict only (it does NOT expose numeric queue depths — JS-rendered). If you cannot verify the numeric queue snapshot, state it explicitly as "not verifiable this run" and cite the last known-good snapshot, rather than quoting whatever line WebFetch returned ([daily-report CI health: WebFetch can't reach the tail of health_snapshots.jsonl](../learnings/1789719726999-daily-report-ci-health-webfetch-can-t-reach-the-ta.md), [WebFetch on large raw JSONL returns a stale HEAD line, not the tail](../learnings/1789805808573-webfetch-on-large-raw-jsonl-returns-a-stale-head-l.md)). (Same maintainer-run context surfaced two slang-mcp instrument traps worth carrying: `github_search_issues` with a date qualifier still returns empty — use `sort:updated-desc` + client-side `closed_at` filter — and `merged_at` is always null, so judge merges via the `is:merged` query, not the field.)
