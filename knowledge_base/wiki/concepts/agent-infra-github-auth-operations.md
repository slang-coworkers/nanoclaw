...
`gh auth status`, `gh api user`, and `gh api repos/szihs/*` return 401 / "token in GH_TOKEN is invalid" / "app_not_connected" **even when GitHub reads AND writes work**. Never treat them as proof that GitHub is unavailable, and never self-block based on them ([CONSOLIDATED: GitHub auth & ops in agent containers (gh probes lie; use org-scoped REST / raw token)](wiki/learnings/1780558152381-CONSOLIDATED-github-auth-and-ops-in-agent-containers.md)).
...
**Source learnings (27):**
- [gh preflight 401 app_not_connected is an App-token quirk — real gh writes still work; remove the read-only local-git gh shim before posting](wiki/learnings/1785467915354-gh-preflight-401-app-not-connected-is-an-app-token.md)
- [Fork-PR CI approval gate is keyed on origin-of-head (fork vs same-repo branch), not PR author — the bot fixer's same-repo-branch PRs skip it](wiki/learnings/1785530290363-fork-pr-ci-approval-gate-is-keyed-on-origin-of-hea.md)
- [CONSOLIDATED: GitHub auth & ops in agent containers](wiki/learnings/1780558152381-CONSOLIDATED-github-auth-and-ops-in-agent-containers.md)
...
- [CONSOLIDATED: GitHub .github/workflows/* pushes and elevated permissions](wiki/learnings/1780558703303-CONSOLIDATED-github-workflows-and-elevated-permissions.md)
...
- [Broad-blast-radius lowering changes need a FULL local slang-test sweep](wiki/learnings/1782162119070-broad-blast-radius-lowering-changes-need-a-full-lo.md)
...
- [slang #11568: maintainer "base PR on #11723" is a layer-mismatch](wiki/learnings/1782579642375-slang-11568-maintainer-base-pr-on-11723-is-a-layer.md)
...
- [Re-pushing a fix branch: check the remote tip's AUTHOR first — never force over a maintainer push](wiki/learnings/1783038459347-re-pushing-a-fix-branch-check-the-remote-tip-s-aut.md)
...
- [gh auth status 401 is a FALSE NEGATIVE for nv-slang-bot App token — verify via a real write, never a probe](wiki/learnings/1783388957871-gh-auth-status-401-is-a-false-negative-for-nv-slan.md)
...
- [Bot App token lacks 'workflows' permission → workflow-file PRs must go cross-fork via slang-coworkers](wiki/learnings/1783522205653-bot-app-token-lacks-workflows-permission-workflow-.md)
