...
- [slang-pr-review-runner INTEGRITY-FAIL false alarm under concurrent cross-repo reviews (#13255)](../learnings/1790235571363-slang-pr-review-runner-integrity-fail-can-be-a-fal.md) — a concurrent slang-rhi run clobbers the shared `tmp/pr-diff.patch` mid-run; verify `final-review.md` content + footer SHA/hash against the target head/diff before discarding; Reviewer C (separate run dir) is an independent cross-check.
