...
- Debian Bookworm standard repos ship no `clang-format`; installing versions 17/18 requires the LLVM apt repo (`apt.llvm.org`) or a Dockerfile-level change — `ncl groups config add-package` alone is insufficient ([ncl group container fixes — Bookworm package gaps + approval sequencing](../learnings/1780060974231-ncl-group-container-fixes-bookworm-package-gaps-ap.md)).
...
- [worktree GC: a `wt-<issue>` may belong to `slangpy-samples`; check `.git` gitdir + scan for unsaved work before `rm -rf`](../learnings/1784507002898-worktree-gc-wt-lt-issue-gt-may-belong-to-slangpy-s.md)
...
- [codex-critique artifacts must live under /workspace, not /tmp](../learnings/1782156860693-codex-critique-artifacts-must-live-under-workspace.md)
...
- [How to render agent group session transcripts as browsable HTML](../learnings/legoop-reference_show_transcript_skill.md)
...
- [Slices/shared worktree can be live: verify branch + open slice PRs before adopting](../learnings/1781318983764-slices-shared-worktree-can-be-live-verify-branch-o.md)
...
- [a vanished worktree DIR doesn't delete its branch; verify via git branch --list + git ls-remote before claiming code is gone](../learnings/1784283551975-a-vanished-git-worktree-dir-does-not-delete-its-br.md)
- [container reset corrupts submodule working trees → configure fails at add_subdirectory; fix with submodule update --force](../learnings/1784324456149-container-reset-can-corrupt-submodule-working-tree.md)
