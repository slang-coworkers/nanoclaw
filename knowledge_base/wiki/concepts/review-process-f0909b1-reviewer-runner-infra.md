...
## Reviewer A's 600s background-wait ceiling → a false 0/0/0 that looks like APPROVE

A distinct Reviewer-A failure that reads as a clean approve: `compose-and-run.sh` drives an outer claude CLI that dispatches the 6 `.claude/agents/*` review subagents in the background then waits for them, and the inner CLI has a **600s background-task wait ceiling** (`CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS`, default 600000). If subagents are still running at 600s, the CLI logs `Background tasks still running after 600s; terminating`, **kills the unfinished subagents** (status `killed`, 0 tokens), and ends the main turn WITHOUT the editorial synthesis — so `final-review.md` is a tiny fragment (e.g. 61 bytes) and the summarizer reports **0 bugs / 0 gaps / 0 questions**, which looks exactly like a clean APPROVE but is *no review produced*. `compose-and-run.sh`'s REVIEW-GUARD catches it (`!!! REVIEW-GUARD FAIL: final review is N bytes (<500)`) and the run exits non-zero, but a `nohup … ; echo exited $?` wrapper can mask the real exit code as 0 — so always check `final-review.md` size and grep the log for `REVIEW-GUARD FAIL` / `Background tasks still running after` before trusting the counts. Fix: re-run with the ceiling lifted — `export CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0` before calling `compose-and-run.sh` (the scripts don't set it, so it's inherited from the shell); cost is unchanged (~$20-27/run on Opus). It's intermittent — it only bites when subagents run long (large diffs / deep verification), so a first run can succeed and a later one on the same PR fail ([Reviewer A can hit the claude CLI 600s background-wait ceiling → kills subagents → false 0/0/0 review](../learnings/1789695101887-reviewer-a-slang-pr-review-runner-can-hit-the-clau.md)).

Reviewer C (`run-clarity.sh`) has a mundane cousin: the script ships as `-rw-rw-r--` (**no execute bit**, unlike its `slang-pr-review-runner` siblings), so executing it directly fails instantly with exit 126 `Permission denied`. Dispatch it via `bash /home/node/.claude/skills/slang-clarity-review-runner/scripts/run-clarity.sh --mode pr --pr <N> --repo <owner/repo> …` — `bash <script>` ignores the missing exec bit (prefer this over `chmod +x` on a skill file you didn't author). The tell after an exit-126 dispatch: the "newest transcripts dir" you find will be a STALE prior run (wrong PR number in the dir name), so validate the run_dir name contains the current PR/head before trusting its `clarity-review.md` ([slang-clarity-review-runner run-clarity.sh lacks the execute bit — invoke via `bash`](../learnings/1789719730698-slang-clarity-review-runner-run-clarity-sh-lacks-e.md)).
...
- [Reviewer A can hit the claude CLI 600s background-wait ceiling → kills subagents → false 0/0/0 review](../learnings/1789695101887-reviewer-a-slang-pr-review-runner-can-hit-the-clau.md) — a tiny `final-review.md` (61B) reads as a clean APPROVE; check its size + grep the log for `REVIEW-GUARD FAIL`/`Background tasks still running`; re-run with `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0`; intermittent.
- [slang-clarity-review-runner run-clarity.sh lacks the execute bit — invoke via `bash`](../learnings/1789719730698-slang-clarity-review-runner-run-clarity-sh-lacks-e.md) — exit 126 Permission denied; `bash <script>` ignores the missing bit; after exit-126 the newest transcripts dir is a stale prior run — validate the PR/head in the dir name.
