...
The same persistence gap bites the **fixer-owned, no-PR** carve-out specifically. `scan.py::we_owe_next_step` classifies any fixer-owned chain that is bot-last, has no resolvable PR, carries no recognized disposition, and has been silent ≥ 60 min as `awaiting_us → needs_nudge` — correct for genuinely-dropped work, but a false positive on chains the orchestrator has ruled NO-GO / defer-to-self-assigned-maintainer / hold / resolved-by-author. Because the disposition is the R3 resumable artifact, it must be **written to the orchestrator's `supervisor-state.json` the instant the NO-GO/defer/hold call is made**, with a token `scan.py` recognizes — `stood-down`, `advisory`, `maintainer-driving`, `external-pr`, `awaiting-pickup`, `closed-by-us`, `human-debate`, `assignee-owned`, `needs-info-external` — or the carve-out re-flags the chain every 12h and re-pages the fixer as false-positive-silent. Measured Tick 227 (2026-09-16): 10 of 14 nudges were false positives, driving a ~14-message clarification flood ("parked warm by design", "resolved-by-author", "held pending operator ruling"); Tick 229 (2026-09-17): five no-PR chains (#13140/#13142/#13143/#13145/#13148) had `disposition:''` despite prior NO-GO handoffs, so all five were nudged and every woken session replied within ~1 min "held by design, not stalled." The human-last→`awaiting_us` heuristic additionally false-positives on three shapes that are NOT open asks: an authorization comment already acted-on via a direct orchestrator→fixer dispatch (the reply channel is that edge and the footprint is the fixer's `Fixes #N` PR — the #13072/#13088 pattern, no issue-side bot reply by design); a resolved-by-author FYI (a self-assigned MEMBER implemented it and dropped a draft-PR-FYI — a bot post would be noise); and bot `pr-board-sync` / coderabbit "review skipped — bot user" notices stamped "do not reply." The durable rule: a fixer-owned no-PR chain is a "promise we owe" ONLY while it has no recorded disposition — blank disposition = "unaccounted," not "parked" — so the owning tier records the parked/NO-GO verdict to state at decision time rather than letting the next tick rediscover it ([supervise-issues: persist NO-GO/deferred/held dispositions immediately or the carve-out re-nudges every tick](../learnings/1789565098418-supervise-issues-persist-no-go-deferred-held-dispo.md), [Supervisor: record NO-GO/parked disposition or it re-pages the fixer as false-positive-silent](../learnings/1789651416238-supervisor-record-no-go-parked-disposition-or-it-r.md)).
...
- [supervise-issues: persist NO-GO/deferred/held dispositions immediately or the fixer-carve-out re-nudges every tick](../learnings/1789565098418-supervise-issues-persist-no-go-deferred-held-dispo.md) — `we_owe_next_step` re-flags a fixer-owned no-PR chain lacking a recognized disposition every tick (Tick 227: 10/14 false); also false-positives on acted-on authorizations, resolved-by-author FYIs, and do-not-reply bot notices.
- [Supervisor: record NO-GO/parked disposition or it re-pages the fixer as false-positive-silent](../learnings/1789651416238-supervisor-record-no-go-parked-disposition-or-it-r.md) — Tick 229: 5 chains with `disposition:''` re-nudged; the disposition IS the R3 artifact — blank on a fixer-owned no-PR chain = "unaccounted," not "parked"; write the token at decision time.
