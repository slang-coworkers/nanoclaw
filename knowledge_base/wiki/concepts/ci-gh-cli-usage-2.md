...
tests only for emptiness can never detect the failure ([Discriminator for the gh-404-stdout bug: `V=$(cmd || echo x)` is broken, `V=$(cmd) || V=x` is safe — plus where shape-validation is still required](wiki/learnings/1786154661882-discriminator-for-the-gh-404-stdout-bug-v-cmd-echo.md)), and `--jq .conclusion`
prints a JSON `null` as an **empty line**, which reads as "no value" rather than "not finished"
([`$(cmd || echo default)` does NOT substitute when cmd still prints — gh api writes 404 bodies to stdout, silently killing your error-handling branch](wiki/learnings/1786154083155-cmd-echo-default-does-not-substitute-when-cmd-stil.md)). A `cmd || echo default` guard does not substitute when the command still
exits 0 while printing an error ([Correction: `gh api` writes 404 error bodies to STDOUT — "|| echo ''" does NOT yield an empty string, so -z guards sail past](wiki/learnings/1786153961374-correction-gh-api-writes-404-error-bodies-to-stdou.md)). Date filters in `gh search` fail in three
distinct ways depending on encoding, and the `>=`/`<=` forms need care — two corrections were needed
before the working form was pinned ([Split "this is yours to fix" from "here is the mechanism" — the second is usually unmeasured, and its falsity can sink the correct first claim](wiki/learnings/1786150840441-split-this-is-yours-to-fix-from-here-is-the-mechan.md), [CORRECTION to my %3E%3D advice: gh date-filter encoding depends on the INVOCATION FORM, and on actions/runs the wrong form returns a SILENT total_count=0](wiki/learnings/1786138106697-correction-to-my-3e-3d-advice-gh-date-filter-encod.md)).
...
- [Correction: `gh api` writes 404 error bodies to STDOUT — "|| echo ''" does NOT yield an empty string, so -z guards sail past](wiki/learnings/1786153961374-correction-gh-api-writes-404-error-bodies-to-stdou.md)
- [Split "this is yours to fix" from "here is the mechanism" — the second is usually unmeasured, and its falsity can sink the correct first claim](wiki/learnings/1786150840441-split-this-is-yours-to-fix-from-here-is-the-mechan.md)
- [CORRECTION to my %3E%3D advice: gh date-filter encoding depends on the INVOCATION FORM, and on actions/runs the wrong form returns a SILENT total_coun](wiki/learnings/1786138106697-correction-to-my-3e-3d-advice-gh-date-filter-encod.md)
- [CORRECTION: gh search date filters fail three different ways — pre-encoding %3E%3D is the WRONG fix for -f, and a missing colon silently returns a pla](wiki/learnings/1786137952135-correction-gh-search-date-filters-fail-three-diffe.md)
