...
## Confirm which PR triggered a cross-repo SlangPy Tests run before attributing its failure

A `classify-only` subagent checking PR #13242 reported a "legitimate" SlangPy Tests (Vulkan gradient/tensor) failure on run `35934606946` — but that run's `displayTitle` was `"Slang PR #13219: Add SPIR-V lexical debug scopes (…)"`, i.e. it belonged to #13219, and #13242's own SlangPy Tests check (`35914564014`) was clean. The trap: cross-repo `repository_dispatch` runs on `shader-slang/slangpy` carry no obvious slang-PR-number field, so a subagent grabbing "the failing SlangPy run" from a loosely-filtered `gh run list` (by recency/workflow name) on the slangpy repo can pick up a *sibling* slang PR's run. Attribution rule: **verify the run's `displayTitle` contains `"Slang PR #<N>"`, or cross-check that `gh pr checks <N>` on the slang PR returns that exact run URL, before attributing the failure to PR N** — never trust a `gh run list`/`gh api` lookup on the slangpy repo in isolation. Cheap insurance that caught this: read the existing `rerun-tracker.json` entry before accepting a fresh subagent classification (the tracker already held the correct #13219 attribution from an earlier same-day sweep). [classify-only subagent mis-attributed a cross-repo SlangPy Tests failure to the wrong Slang PR](../learnings/1790266311097-classify-only-subagent-misattributed-a-cross-repo-.md)

**Source learnings (9):**
- [CI failure triage is runner-blind if you filter only on event+branch](../learnings/1785880778531-ci-failure-triage-is-runner-blind-if-you-filter-on.md) — a broken self-hosted node produces `schedule`-on-`master` failures that pass every noise filter; fetch `runner_name`, treat a surviving failure as unclassified.
- [Attribute a runner-scoped CI defect with a host×job cross-tab, not a list of reds](../learnings/1785853592478-attribute-a-runner-scoped-ci-defect-with-a-host-jo.md) — the defect is the single all-fail cell; one table rules out both code-regression and dead-host.
- [A figure correction is not a mechanism check (CI streak attribution)](../learnings/1785882161001-a-figure-correction-is-not-a-mechanism-check-ci-st.md) — correcting a carried number does not re-check the carried explanation; run the control especially when it's about to agree with you.
- [A CI job failing 0/N while compilation passes N/N is a broken runner — find a tree-identical control pair](../learnings/1785876635240-a-ci-job-failing-0-n-while-compilation-passes-n-n-.md) — merge queues re-run identical trees under different hosts; opposite results splitting only on host ⇒ infra.
- [Counting merge_group CI health: tally the gating workflow by name, not event=merge_group](../learnings/1785899774910-counting-merge-group-ci-health-tally-the-gating-wo.md) — one queue entry fans out to many runs; the non-gating ones are always green and swamp a raw tally.
- [Keying a CI flake cost tally on the RUNNER HOST over-counts when two defects share the box](../learnings/1785917535341-keying-a-ci-flake-cost-tally-on-the-runner-host-ov.md) — re-open each run's job list; a grep over free-text `reason` conflates "ran on X" with "X broke it".
- [A pass on a pooled resource is evidence about the draw, not about the fix; a 0-success box is negative capacity](../learnings/1785941832757-a-pass-on-a-pooled-resource-is-evidence-about-the-.md) — record `runner_name` on the FAILING attempt too, at run time.
- [Falcor #12145 crash code is emitted in DECIMAL only — a hex grep is a false-negative trap](../learnings/1785910460636-falcor-12145-crash-code-is-emitted-in-decimal-only.md) — key on the pair (exact test name + `return code 3221225477`); test a load-bearing empty probe against a known-positive control.
- [classify-only subagent mis-attributed a cross-repo SlangPy Tests failure to the wrong Slang PR](../learnings/1790266311097-classify-only-subagent-misattributed-a-cross-repo-.md) — verify a cross-repo run's `displayTitle` (`"Slang PR #<N>"`) or `gh pr checks <N>` before attributing; check the tracker before accepting a fresh subagent classification.
