...
**`mcp__slang-mcp__discord_read_messages` can start returning `Unsupported channel type` on Discord thread/forum channels** (URL shape `discord.com/channels/<guild>/<thread_id>/<msg_id>`). Observed 2026-09-16: it worked earlier in the same session on the same channel_id, then began failing (and once a socket-closed error) and stayed broken across turns — a Discord-side/library rejection (the call reaches Discord), not an auth failure. Root cause is NOT yet confirmed: the "worked-then-failed on the same id" detail argues against a pure thread/forum channel-type limitation and toward a mid-session connection degradation — don't relay either as fact. Workaround (currently non-blocking): read the message text from the dashboard webhook notification content, which carries the full body; the tool only truly matters when actively pulling history not already delivered as a notification. If it recurs, prefer the notification fallback and surface it to the operator to look at the slang-mcp Discord adapter. ([slang-mcp discord_read_messages fails 'Unsupported channel type' on thread channels](../learnings/1789537044392-slang-mcp-discord-read-messages-fails-unsupported-.md))

**DeepWiki MCP renamed its Q&A tool `ask_question` → `ask_wiki_question` (2026-09-22), and the stale allowlists deny the correct current name fleet-wide.** The server (`https://mcp.deepwiki.com/mcp`) now exposes `read_wiki_structure`/`read_wiki_contents`/`ask_wiki_question`, so a call to `mcp__deepwiki__ask_question` returns `No such tool available` (the rename landed mid-session — it worked earlier the same conversation, then began erroring). But every coworker skill's `allowed-tools:` frontmatter and shader-slang/slang `CLAUDE.md:12` still list the OLD name, so the `PreToolUse` allowlist gate DENIES the current `ask_wiki_question` even though it is correct — silently degrading PR-review, clarity-review, code-reader/explore, and deep-research lookups across deep-research, slang-code-reader, slang-explore, slang-pr-review-runner, slang-clarity-review-runner, slangpy-code-reader, and nanoclaw-code-reader. If a DeepWiki call errors "No such tool available," don't assume it is transient: check your own injected MCP tool schema (the "MCP Server Instructions" system-reminder block) for the current name before retrying — and if the correctly-named tool is then hook-denied, that is the real bug (allowlist not yet updated), not something to fix by guessing names. Trust the live injected schema over static docs when they disagree. Interim workaround: fall back to GitHub source/issue research (`github_get_file_contents`, `github_search_issues`) and disclose the fallback in the answer. Durable fix is operator-side: update the lego-registry SKILL.md `allowed-tools` (source-of-truth in `container/skills/*`, prefer a wildcard `mcp__deepwiki__*` for resilience to future renames) and PR the stale name in shader-slang/slang CLAUDE.md ([DeepWiki MCP tool renamed ask_question → ask_wiki_question; fleet-wide allowlist gap](../learnings/1790105381468-deepwiki-mcp-tool-renamed-ask-question-ask-wiki-qu.md)).

**Source learnings (27):**
- [DeepWiki MCP renamed ask_question → ask_wiki_question; stale allowlists deny the current tool fleet-wide](../learnings/1790105381468-deepwiki-mcp-tool-renamed-ask-question-ask-wiki-qu.md) — check the live injected MCP schema for the current name; the hook-deny of the correct name is the real bug; fall back to GitHub-source research; operator fixes SKILL.md allowed-tools (wildcard `mcp__deepwiki__*`) + slang CLAUDE.md.
- [slang-mcp discord_read_messages can fail 'Unsupported channel type' on thread channels mid-session — fall back to the webhook notification content](../learnings/1789537044392-slang-mcp-discord-read-messages-fails-unsupported-.md)
