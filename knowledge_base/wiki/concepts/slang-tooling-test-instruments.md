...
## Proving stripping / obfuscation on serialized modules — moved

The per-chunk parse method, the finding that `-obfuscate` leaves the `ast ` chunk intact, the `.slang-module` RIFF chunk layout, and the serialization-flag facts now live on their own page: [Serialized-Module & Obfuscation Verification](../concepts/slang-tooling-serialized-module-obfuscation.md). The one-line principle stays in the summary table below: a whole-file byte count cannot attribute bytes to a chunk, so parse per chunk with the header layout read from the source.

## The container HAS a GPU — "no GPU, can't reproduce" is a false capability-negative

The same false-negative shape applies to the environment's own capabilities. The Slang coworker container is NOT CPU/interpreter-only: `nvidia-smi` reports a real **NVIDIA L40S**, driver **595.91.07** (CUDA 13.2), with the NVIDIA Vulkan stack present (`libGLX_nvidia.so.595.91.07`, `libnvidia-glvkspirv.so.595.91.07` — the SPIR-V→NVVM compiler). So driver-dependent Vulkan repros (e.g. `VK_ERROR_UNKNOWN` at `vkCreateGraphicsPipelines`, SPIR-V→NVVM crashes) ARE testable here — do not tell a GitHub commenter, or route a task, on the premise "no GPU, can't reproduce" (surfaced routing #7370; an incorrect CPU-only claim was corrected by the triager and refuted by a direct `nvidia-smi`). The "no GPU" absence claim fell to exactly one positive control (`nvidia-smi`), the discipline this whole page argues for. Caveat, stated honestly: Vulkan-runtime tests still need the right SDK/loader wiring and `slang-test` GPU test categories may still be gated — but a hand-driven `vkCreateGraphicsPipelines` repro on the L40S is a valid check ([the Slang agent environment HAS a GPU — NVIDIA L40S, driver 595.91.07](../learnings/1790098423189-slang-agent-environment-has-a-gpu-nvidia-l40s-driv.md)).
...
- [the Slang agent environment HAS a GPU (NVIDIA L40S, driver 595.91.07) — driver-dependent Vulkan repros are testable; "no GPU, can't reproduce" is a false capability-negative refuted by one nvidia-smi](../learnings/1790098423189-slang-agent-environment-has-a-gpu-nvidia-l40s-driv.md)
