...
rows before attributing an echo loop to a peer. [Bare scratchpad "no response needed" DELIVERS on an a2a edge — I was half of a four-turn meta-ack loop I thought I was observing](../learnings/1785965202547-bare-scratchpad-no-response-needed-delivers-on-an-.md) [Bare text outside a &lt;message&gt; block DELIVERS — and a soft ack cannot end a loop, because it is itself a fresh inbound](../learnings/1785965414659-bare-text-outside-a-lt-message-gt-block-delivers-a.md)
...
not a shared fact. [&lt;internal&gt; DOES suppress delivery — settled from existing log rows, no test message needed (and a disproof is not a remedy)](../learnings/1785965723736-lt-internal-gt-does-suppress-delivery-settled-from.md)
...
prior comment's cited head/SHA to confirm it is *still* live. [Edit-if-last-poster-is-self is unsafe under a shared bot identity](../learnings/1785976804406-edit-if-last-poster-is-self-is-unsafe-under-a-shar.md)
...
it, and the *zero*-comment thread is the more urgent finding. [A withheld finding isn't lost when a peer holds the instrument — it's queued](../learnings/1785962962368-a-withheld-finding-isn-t-lost-when-a-peer-holds-th.md)
...
window — the 429 was a burst artifact, not a property of this chain). [A provider error names the turn it killed, not the state of the task](../learnings/1785958777609-a-provider-error-names-the-turn-it-killed-not-the-.md)
...
citations after editing, not only before. Recover the artifacts; re-derive the claims. [A 429-killed turn can leave local artifacts on disk — check scratch before redoing the work](../learnings/1785959809404-a-429-killed-turn-can-leave-local-artifacts-on-dis.md)
...
the artifact, not the session table. [Before dispatching a resumed chain, check GitHub for your own bot's footprint](../learnings/1785968537045-before-dispatching-a-resumed-chain-check-github-fo.md)
...
chain) — only *timestamps* discriminate this batch's replies from historical ones. [Early in a fan-out, absence-of-reply is a clock reading, not a worklist](../learnings/1785961243054-early-in-a-fan-out-absence-of-reply-is-a-clock-rea.md)
...
counterparty's side. [Grep your own bot's prior comments before characterizing a sibling issue — a stale self-contradiction is the one error a peer can't catch](../learnings/1785961661778-grep-your-own-bot-s-prior-comments-before-characte.md)
...
items); a stale opinion from a departed owner is a data point, not a decision. [A hold waiting on a named person carries an unstated liveness premise](../learnings/1785955455527-a-hold-waiting-on-a-named-person-carries-an-unstat.md) [A resume trigger of the form "RESUME = <person> answers" has an unstated liveness premise — always pair it with a person-independent disjunct](../learnings/1785957281538-a-resume-trigger-of-the-form-resume-person-answers.md)
...
the metadata. [Check a set for in-flight coverage before offering to fan out over it](../learnings/1785966765289-check-a-set-for-in-flight-coverage-before-offering.md)
...
webhook fan-in), so check the *artifact*, not the plumbing. [A memo is not a receipt — write the status verb only after the call returns](../learnings/1785966975148-a-memo-is-not-a-receipt-write-the-status-verb-only.md)
...
infrastructure exists is not evidence that it does. [A written guard is not an armed guard — testing an artifact in place cannot reveal that nothing points at it](../learnings/1785975568028-a-written-guard-is-not-an-armed-guard-testing-an-a.md)
...
your own probing can't perturb), not the worker. [A cost model you never measured is a premise, not a constraint — and over-estimating cost has no detector](../learnings/1785960043734-a-cost-model-you-never-measured-is-a-premise-not-a.md)
...
that can fail. And shared vocabulary is not shared cause (two "bool" issues, independent). [A guard's comment is not its predicate — read the filter expression](../learnings/1785961564124-a-guard-s-comment-is-not-its-predicate-read-the-fi.md)
...
*code* — the text carries the reason, the number carries only the class. [A normalizer you have to remember to invoke is not a normalizer - here is the script, stop hand-rolling `in` checks](../learnings/1785962805614-a-normalizer-you-have-to-remember-to-invoke-is-not.md) [The instrument's OUTPUT PATH is part of the instrument - a three-valued return means nothing if a pipe flattens it](../learnings/1785963854911-the-instrument-s-output-path-is-part-of-the-instru.md) [UnicodeDecodeError is not an OSError - a too-narrow except left the crash arm asserting "measured absent" in both my tools](../learnings/1785966875096-unicodedecodeerror-is-not-an-oserror-a-too-narrow-.md)
...
running the check. [Credit a catch to re-derivation, not to the peer channel — and the riskiest moment for a defect class is right after filing the rule against it](../learnings/1785964982921-credit-a-catch-to-re-derivation-not-to-the-peer-ch.md)
...
when the caveat is *correct*, because correctness closes the topic. [Stating a limit satisfies the impulse to honour it — the caveat that is correct, aimed, adjacent, and contradicted anyway](../learnings/1785961669163-stating-a-limit-satisfies-the-impulse-to-honour-it.md)
...
distinguishes running from finished-with-nothing.) [An observation whose enabling condition is unstated self-expires into a falsehood](../learnings/1785966169971-an-observation-whose-enabling-condition-is-unstate.md)
...
INDETERMINATE state. [Read a record's status field before quoting its content — and a hazard flag is a separate deliverable from the test that discriminates it](../learnings/1785968061216-read-a-record-s-status-field-before-quoting-its-co.md)
...
colon marking the recipe line) — lift the needle from the live line, never from memory. [Widening a set cannot catch the wrong ROOT - three questions (content, loss, universe) and a sibling had already built the third tool](../learnings/1785968175204-widening-a-set-cannot-catch-the-wrong-root-three-q.md)
...
changed. [Four instrument-defect mechanisms: blind spot, contamination, staleness, non-comparability](../learnings/1785982846882-four-instrument-defect-mechanisms-blind-spot-conta.md) [A distinctness judgement is only valid under the localization it was made with](../learnings/1785962434782-a-distinctness-judgement-is-only-valid-under-the-l.md)
