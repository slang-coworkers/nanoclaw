...
**Premature scrape — the most common failure mode.** `devin-fetch.sh` can exit `rc=0` while Devin's AI Analysis panel is still rendering (`Generating...`). When this happens, `devin-flags.md` echoes the PR description back verbatim under `## AI Analysis`, and `## Bugs` / `## Flags` show `(none reported)` — which looks like a clean all-clear but means nothing. Observed repeatedly: [[wiki/learnings/1780870637455-slang-pr-review-reviewer-b-devin-fetch-exit-0-devi.md]], [[wiki/learnings/1781179189519-devin-reviewer-unknown-commit-status-generating-we.md]], [[wiki/learnings/1781192458084-devin-reviewer-devin-fetch-can-exit-0-while-analys.md]], [[wiki/learnings/1781731413287-devin-reviewer-b-can-exit-rc-0-while-analysis-is-s.md]], [[wiki/learnings/1782761345395-slang-pr-review-devin-can-report-0-bugs-while-anal.md]], [[wiki/learnings/devin-reviewer-scrape-reliability.md]].
...
**Cross-PR contamination.** When devin-fetch runs concurrently for multiple PRs, all `devin-flags.md` files can contain the same AI Analysis blob (from whichever tab rendered first). Serialize Devin fetches, or grep each file for the expected PR number to confirm the page content matches the URL [[wiki/learnings/devin-reviewer-scrape-reliability.md]].

**Draft PRs.** Devin analysis is typically inconclusive on draft PRs — the page shows "Generating...", commit-status "unknown." Treat B as best-effort-skipped on any draft [[wiki/learnings/1781134206455-slang-pr-review-reviewer-a-budget-cap-mid-analysis.md]], [[wiki/learnings/1781324278003-stacked-pr-review-reviewer-a-checks-out-master-pro.md]].

**Healthy Devin scrape signals:** `## AI Analysis` has synthesized prose (not diff fragments); `## Flags` explicitly says "no flags" (not blank); the PR number in the analysis text matches the URL; `devin-commit-status.txt` says "Analysis is up to date" [[wiki/learnings/devin-reviewer-scrape-reliability.md]].

**"30m timeout" in devin-error.txt** is a templated message meaning "no Devin analysis available for this PR" — not a literal 30-minute wall-clock measurement. Treat B as best-effort-skipped and proceed on A+C [[wiki/learnings/1782735484974-slang-pr-11815-review-a-c-converge-on-dead-flavor-.md]].

## Reviewer C (Clarity) Failure and Recovery

Reviewer C can die mid-run from a transient `API Error: The socket connection was closed unexpectedly`. The failure leaves `clarity-review.md` as a tiny stub (~80–135 bytes) containing only the error string — even though the wrapper exits and the file exists. A naive "artifact present?" check would silently lose a whole reviewer [[wiki/learnings/1780603736166-slang-pr-review-reviewer-c-can-die-mid-run-on-a-tr.md]], [[wiki/learnings/1781213312260-slang-pr-review-reviewer-c-clarity-can-drop-with-t.md]].
...
A maintainer's literal code suggestion is a request to honor, not a spec to paste. On PR #11628 (WGSL emitter), applying a maintainer's predicate-simplification literally would have introduced a regression: the predicate was read in TWO places, and one of those sites ran for ALL ops (including `GlobalParam`) where the simpler form fired incorrectly [[wiki/learnings/1781640634164-a-maintainer-s-literal-review-suggestion-can-be-lo.md]].
...
When a PR establishes a new null-possible invariant for a member (e.g. "`m_param` can be null during result legalization") and adds a safe-access helper, grep the whole file for `<member>->` and confirm each remaining raw dereference is either unreachable under the new condition or asserts/uses the helper. Missing a sibling site reads as either an oversight or a latent crash of the same class as the bug just fixed. Both Reviewer A and C flagging the same untouched sibling site is a strong convergence signal [[wiki/learnings/1781792411472-pr-review-heuristic-when-a-fix-adds-a-null-possibl.md]].

## Reviewer-Traced Unreachable Paths

A code-reading reviewer can trace a plausible bug path that is foreclosed upstream by the front-end. Before adding a regression test for a reviewer-traced "gap," try to construct the triggering input and compile it. If it's rejected upstream (e.g. E30702 for `SV_DepthGreaterEqual` as input), the gap is unreachable — say so with the diagnostic code as evidence, and don't ship a test that can't compile [[wiki/learnings/1782175276058-depth-sv-semantics-are-output-only-e30702-a-review.md]].
...
- [[wiki/learnings/1780418605612-don-t-instruct-coworkers-to-mark-prs-ready-for-rev.md]] — Don't instruct coworkers to mark PRs ready-for-review
...
- [[wiki/learnings/1780509591502-don-t-conflate-internal-a2a-review-with-github-rev.md]] — Don't conflate internal a2a review with GitHub reviewDecision
- [[wiki/learnings/1780603736166-slang-pr-review-reviewer-c-can-die-mid-run-on-a-tr.md]] — Reviewer C can die mid-run on a transient API socket error
- [[wiki/learnings/1780690000002-never-add-a-reviewer-to-a-draft-pr-it-spams-the-human.md]] — Never add a reviewer to a draft PR
- [[wiki/learnings/1780769188437-slang-review-verify-inaccurate-comment-flags-again.md]] — Verify "inaccurate comment" flags against code text, not PR-body citations
- [[wiki/learnings/1780769199724-multi-round-pr-review-converges-scope-down-to-targ.md]] — Multi-round PR review converges; scope down when delta is comment-only
- [[wiki/learnings/1780769238745-slang-pr-review-reviewer-a-and-c-share-one-checkou.md]] — Reviewer A and C share one checkout — parallel runs collide on git index.lock
- [[wiki/learnings/1780870637455-slang-pr-review-reviewer-b-devin-fetch-exit-0-devi.md]] — Reviewer B (devin-fetch) exit 0 ≠ Devin analysis complete
- [[wiki/learnings/1781125005627-triaging-an-issue-that-already-has-a-community-fix.md]] — Triaging an issue that already has a community fix PR
- [[wiki/learnings/1781134206455-slang-pr-review-reviewer-a-budget-cap-mid-analysis.md]] — Reviewer A budget-cap mid-analysis hypotheses are NOT findings
- [[wiki/learnings/1781177378439-slang-reviewer-a-false-positives-on-own-line-check.md]] — Reviewer A false positives on own-line //CHECK directives from prompt condensation
- [[wiki/learnings/1781179189519-devin-reviewer-unknown-commit-status-generating-we.md]] — Devin reviewer: 'unknown' commit-status + 'Generating...' = weak signal
- [[wiki/learnings/1781192458084-devin-reviewer-devin-fetch-can-exit-0-while-analys.md]] — devin-fetch can exit 0 while analysis still "Generating" — low-confidence
- [[wiki/learnings/1781213312260-slang-pr-review-reviewer-c-clarity-can-drop-with-t.md]] — Reviewer C (clarity) can drop with transient socket error — detect tiny output, just re-run
- [[wiki/learnings/1781324278003-stacked-pr-review-reviewer-a-checks-out-master-pro.md]] — Stacked-PR review: Reviewer A checks out master, producing false positives + missed-drop
- [[wiki/learnings/1781640634164-a-maintainer-s-literal-review-suggestion-can-be-lo.md]] — A maintainer's literal review suggestion can be locally unsafe
- [[wiki/learnings/1781731413287-devin-reviewer-b-can-exit-rc-0-while-analysis-is-s.md]] — Devin reviewer (B) can exit rc=0 while analysis is still "Generating" — false all-clear
- [[wiki/learnings/1781731735312-recover-slang-clarity-review-candidates-after-a-tr.md]] — Recover slang clarity-review candidates after a transient API error
- [[wiki/learnings/1781792411472-pr-review-heuristic-when-a-fix-adds-a-null-possibl.md]] — PR-review heuristic: when a fix adds a null-possible invariant, audit ALL structurally-identical sites
...
- [[wiki/learnings/1782465056185-verify-n-reviewers-approve-against-github-reviewde.md]] — Verify "N reviewers APPROVE" against GitHub reviewDecision before posting
...
- [[wiki/learnings/1782594329649-slang-pr-review-scope-a-re-run-to-focused-verifica.md]] — slang-pr-review: scope a re-run to focused verification when re-push is test-only
