...
The same "verify the claim about compiler behavior, don't read the prose" discipline governs **language-reference conversion/diagnostic-doc PRs**: a documented "this conversion is allowed (no warning)" vs "this conversion warns" cell is a CLAIM about the compiler, so spot-check each warn/allow classification against the per-conversion `kConversionCost_*` value vs the E30081 threshold (`kConversionCost_Default = 500`, logic in `slang-check-conversion.cpp` `_coerce`) — or compile a minimal repro per target type — never against deepwiki's summarized numbers [[conversion-doc PRs: verify each warn/allow cell against `kConversionCost_*`/E30081, and separate current-vs-planned cells]](../learnings/1788960523826-approver-challenger-probe-language-reference-conve.md) (slang#12039). Concrete trap: the doc listed `bool → integer type` as allowed, but the nonconstant `bool→intptr_t` path falls through to a cost ≥ 500 and warns — so the blanket cell is inaccurate. Two rules: separate "documents current behavior" cells from "documents planned/tracked-future" cells (a planned cell that already matches current behavior is a real accuracy gap, not a nit); and if you cannot conclusively confirm a cell at the pinned head, that is OPEN_GAP → ABSTAIN — uncertainty never rounds up to WOULD_APPROVE, and it is BLOCK only if you verify a cell is actually wrong.
...
- [Conversion-doc PRs: verify each warn/allow cell against `kConversionCost_*`/E30081, separate current-vs-planned cells (slang#12039)](../learnings/1788960523826-approver-challenger-probe-language-reference-conve.md) — a documented allowed/warns classification is a claim about the compiler; check the cost constant vs `kConversionCost_Default = 500` or compile per target type, never deepwiki numbers; unconfirmable cell → OPEN_GAP/ABSTAIN.
