...
Note the author test must be written suffix-tolerantly, not as `login == "nv-slang-bot[bot]"` — this REST path returns the bare `nv-slang-bot` (see the [companion state-verification page](general-misc-github-state-verification-and-bot-hygiene.md) for the login-form detail), so a strict `[bot]`-suffix equality check *fails closed* on our own PRs while a branch-name check *fails open* on other people's. Live 2026-08-04 incident: a supervisor tick recovered a `❌ stale` CI cell for `fix/issue-11004` and nudged the fixer to rebase PR **#11234** — which is `author: szihs`, `head_repo: szihs/slang`, an issue filed by `undefdev` and assigned to `saipraveenb25`. The branch-name collision with our own convention was pure coincidence. Four rules fall out ([a `fix/issue-N` branch on a FORK is not ours — resolve by head_repo + author](wiki/learnings/1785803367777-a-fix-issue-n-branch-on-a-fork-is-not-ours-resolve.md)):
...
- [Verify a fix PR's closing reference matches the tracked issue (title Fix #N doesn't auto-close)](wiki/learnings/1782936185036-verify-a-fix-pr-s-closing-reference-matches-the-tr.md)
- [PR-description prose fixes-N hijacks auto-close for the wrong issue; commit subject doesn't auto-close](wiki/learnings/1782951523566-pr-description-prose-fixes-n-closes-the-wrong-issu.md)
- [the control is the counterfactual question, not a blocklist — a null result is evidence only if the instrument demonstrably works](wiki/learnings/1785770932836-the-control-is-the-counterfactual-question-not-a-b.md)
