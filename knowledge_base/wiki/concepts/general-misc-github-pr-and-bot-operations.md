...
- **A *bare* `<login>@users.noreply.github.com` (no numeric id) does NOT map to the bot account** — CLA-assistant then flags the commit unsigned and sets `mergeStateStatus=BLOCKED`, even with the CLA signed. A container restart can silently reset `git config user.email` to the bare value, so verify it at the start of any commit-producing session; re-author (never revert — CLA scans *all* PR commits) the offending commits and force-push (a gated destructive op — get session authorization first).
...
The email must be the account's real address or its **account-scoped** noreply form
`<numericId>+<login>@users.noreply.github.com` — for the bot, `274397474+nv-slang-bot[bot]@users.noreply.github.com`. A *bare* `nv-slang-bot@users.noreply.github.com` (no numeric id) does not associate the commit with the account, so CLA-assistant (`license/cla`) reports the bot as an unsigned committer ("N of M committers signed", bot ❌) and flips `mergeStateStatus=BLOCKED` even when the CLA is signed and earlier commits passed. A container restart can reset the worktree's `git config user.email` to the bare/wrong value, so commits *after* the restart regress while pre-restart ones are fine — diagnose with `git log --format='%h %ae' <branch> --not origin/master`. Fix: `git config user.email "274397474+nv-slang-bot[bot]@users.noreply.github.com"` (+ `user.name "nv-slang-bot[bot]"`), then **re-author** the offending commits — `git commit --amend --reset-author --no-edit` for the tip, or a `--exec` rebase for several — and `git push --force-with-lease` (a gated destructive op: get explicit session authorization first). A reverted or superseding new commit does NOT help — CLA scans every PR commit, so history must be rewritten. Durable fix: pin the container's default git identity to the account-scoped email so future sessions don't regress ([bot commits must use the account-associated noreply email or CLA-assistant blocks merge](../learnings/1789476879264-bot-commits-must-use-the-account-associated-norepl.md)).
...
- [Bot commits must use the account-scoped noreply email (`274397474+nv-slang-bot[bot]@…`) or CLA-assistant flags them unsigned and BLOCKs merge; re-author, don't revert](../learnings/1789476879264-bot-commits-must-use-the-account-associated-norepl.md)
