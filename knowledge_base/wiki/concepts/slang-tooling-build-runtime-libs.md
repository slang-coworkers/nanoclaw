...
A distinct macOS-DXC-from-source failure surfaced on **macOS 27 / Apple Silicon** (#13077): a universal build of the vendored DXC died with `clang-tblgen: Bad CPU type in executable`. The root cause was Slang's own argument plumbing, not DXC. `cmake/FetchDXC.cmake` escapes the list-valued `CMAKE_OSX_ARCHITECTURES=x86_64;arm64` with `string(REPLACE ";" "\\;" ...)` (line 495) and then expands it **unquoted** inside `set(_dxc_configure_args ... ${fwd} ...)` (line 625), before the final unquoted `execute_process(COMMAND cmake ${args})`. A `\;` escape survives exactly one unquoted expansion, so the `set()` hop consumed it and the child received `-DCMAKE_OSX_ARCHITECTURES=x86_64` plus a stray positional `arm64`, reported only as a "CMake Warning: Ignoring extra path" that is lost when stderr prints only on failure. DXC, both its tblgen and the shipped `libdxcompiler.dylib`, was configured x86_64-only; on machines with Rosetta the build succeeds silently with no arm64 slice, so the bug can hide in shipped artifacts. The fix (PR #13079) appends with one quoted, guarded `list(APPEND args "${fwd}")` (`if(NOT fwd STREQUAL "")`, because an empty quoted append adds an element and changes the stamp hash), which leaves `execute_process` as the only unquoted expansion. Discriminating check: after a universal build `lipo -archs` on `libdxcompiler.dylib` shows `x86_64 arm64`, or DXC's own `CMakeCache.txt` has `CMAKE_OSX_ARCHITECTURES:STRING=x86_64;arm64`; both fail on master even on runners with Rosetta. The reporter confirmed the fix on macOS 27 / Xcode 27 on 2026-09-25 ([CMake: unquoted list expansion consumes \; escapes — verify your own arg plumbing before blaming a vendored dep (slang#13077)](../learnings/1790377335548-cmake-unquoted-list-expansion-consumes-escapes-ver.md)).

Three earlier rounds went after the wrong layer: defaulting an unset arch to the host, then `LLVM_USE_HOST_TOOLS`, then forcing NATIVE host-tool flags through `CROSS_TOOLCHAIN_FLAGS_NATIVE`. All of them rested on the theory that the vendored LLVM's arch detection was at fault, and each was "proven" by a harness that replayed only the edited block. The real bug surfaced only when a harness replayed the consuming code end to end (FetchDXC.cmake:485-503 and :606-681, verbatim, into a real child cmake, comparing the argv it received on master and head). The fast recipe needs no Mac: a `cmake -P` script that builds the list exactly as master and head do and runs `execute_process(COMMAND python3 -c "import sys,json;print(json.dumps(sys.argv[1:]))" ${args})` prints the exact argv, `string(SHA256 …)` compares stamp hashes, and an end-to-end child `cmake -S tiny -B …` prints `list(LENGTH CMAKE_OSX_ARCHITECTURES)` ([CMake: escaped list forwarded to a sub-configure is lost by a second unquoted expansion — verify with a cmake -P argv harness](../learnings/1790373222851-cmake-escaped-list-forwarded-to-a-sub-configure-is.md)). Two process rules follow. Before blaming a third-party build, dump what the child process actually received. And treat a workaround that works as a clue: the explicit `-DCMAKE_OSX_ARCHITECTURES=arm64` worked because a single arch has no `;` to lose, just as "Bad CPU type" on a requested-universal binary means the host slice never arrived. The issue had also been auto-closed by a CI-only PR (#13103, touching only `.github/workflows/*`) whose body said "Fixes #13077" while the bug was live, so when a reporter says "still failing", check the issue state and what they actually built.
