...
Related, same file: the `SLANG_UNEXPECTED("non-simple operand(s)!")` abort at `:2197` is **shared by all three** legalization contexts (resource `:4066`, existential `:4097`, empty `:4141`), all run from `slang-emit.cpp`. So "turn that abort into a diagnostic" is *not* a local change scoped to empty structs. [isSimpleType polarity in Slang empty-type legalization: true = RETAINED, and Metal returns false](wiki/learnings/1786001077628-issimpletype-polarity-in-slang-empty-type-legaliza.md)
