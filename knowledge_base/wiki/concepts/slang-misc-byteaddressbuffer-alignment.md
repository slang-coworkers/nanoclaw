...
- **Slice 2**: Add pow2-alignment validation error; diagnostic 41303 cannot live here because the single-arg forwarder still passes natural STRIDE, causing false-rejects of valid 3-component loads ([[wiki/learnings/1781776078394-slang-11591-11590-prior-41303-to-slice-3-ruling-17.md]])  
...
Adding pow2-alignment validation must be coupled in the same PR with changing the implicit single-arg LoadAligned forms to pass natural alignment, because float3's stride (12) is non-power-of-two and would trigger the new error on every 3-component load ([[wiki/learnings/1781133163629-slang-11545-pow2-alignment-validation-must-fix-imp.md]]).

Details on the four-slice decomposition and specific file locations for natural-alignment infrastructure ([[wiki/learnings/1781315409904-slang-11545-alignment-work-split-into-4-slices-sli.md]]).

## Chunked Byte-Address Codegen (Slice 4 / #11593)

The chunked tier adds widest-power-of-two sub-vector load/store for values larger than the maximum atomic width. The correct insertion point is `slang-ir-byte-address-legalize.cpp`; chunking must be vector-specific; alignment must be recomputed per-chunk; dependencies on prior slices must be satisfied first ([[wiki/learnings/1781315228593-slang-11593-chunked-byte-address-codegen-insertion.md]]).

## Scalar Block Layout and ArrayStride

`VK_EXT_scalar_block_layout` is required by the declared SPIR-V `ArrayStride` decoration (validated at pipeline creation), **not** by how loads are emitted. Scalarizing loads while keeping a `float3[]@12` declaration still requires the device feature. For portability, declare a scalar `float[]` array instead ([[wiki/learnings/1782722840826-slang-scalar-block-layout-is-forced-by-the-arrayst.md]]).

---
**Source learnings (9):**
- [[wiki/learnings/1780768566167-slang-byteaddressbuffer-single-arg-aligned-forms-u.md]] — single-arg aligned forms use natural stride
- [[wiki/learnings/1781030980541-slang-byteaddressbuffer-implicit-loadaligned-t-off.md]] — implicit LoadAligned passes STRIDE as alignment
- [[wiki/learnings/1781133163629-slang-11545-pow2-alignment-validation-must-fix-imp.md]] — pow2 validation must fix implicit forms in same PR
- [[wiki/learnings/1781158312869-slang-byte-address-a-naive-pow2-alignment-check-br.md]] — naive pow2 check breaks float3
- [[wiki/learnings/1781315228593-slang-11593-chunked-byte-address-codegen-insertion.md]] — chunked byte-address codegen insertion point
- [[wiki/learnings/1781315409904-slang-11545-alignment-work-split-into-4-slices-sli.md]] — alignment work split into 4 slices
- [[wiki/learnings/1781776078394-slang-11591-11590-prior-41303-to-slice-3-ruling-17.md]] — prior 41303-to-Slice-3 ruling re-verified
- [[wiki/learnings/1782722840826-slang-scalar-block-layout-is-forced-by-the-arrayst.md]] — scalar block layout forced by ArrayStride
