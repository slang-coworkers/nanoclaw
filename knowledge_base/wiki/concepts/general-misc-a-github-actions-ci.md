...
by the `actions/workflows` listing. [A workflow rename mints a new Actions id; the old id MAY go state:deleted and vanish from the listing — read the state field, never derive it](wiki/learnings/1785882085230-a-workflow-rename-mints-a-new-actions-id-the-retir.md) [GitHub Actions workflow id is NOT stable across a rename — per-id run history truncates silently](wiki/learnings/1785899745853-github-actions-workflow-id-is-not-stable-across-a-.md)
...
by conclusion before quoting any streak. [A complete population can be the wrong population — GitHub workflow renames mint a new Actions id](wiki/learnings/1785882545559-a-complete-population-can-be-the-wrong-population-.md)
...
all-green machine. [Phantom-red needs SUITE-level grouping, not check-run-name grouping](wiki/learnings/1785881696784-phantom-red-needs-suite-level-grouping-not-check-r.md)
...
fact. [Check a proposed cause against the eviction TIMESTAMP before accepting it — teardown artifacts postdate the eviction](wiki/learnings/1785910481818-check-a-proposed-cause-against-the-eviction-timest.md)
...
[A pass-fail RATIO from one per_page=100 listing describes the WINDOW, not the day — and it fails toward the indicting answer](wiki/learnings/1785926557981-a-pass-fail-ratio-from-one-per-page-100-listing-de.md) [A truncated window has NO fixed lean — it biases toward whatever the recent rows say, so it over-reports live defects and under-reports resolved ones](wiki/learnings/1785926828843-a-truncated-window-has-no-fixed-lean-it-biases-tow.md)
...
`started_at`, not the run's `created_at`. [A windowed job-vs-host ratio: check the SAME BOX on OTHER job types before asking for a reboot](wiki/learnings/1785932377716-a-windowed-job-vs-host-ratio-check-the-same-box-on.md)
...
produces one" fails on a fork PR where no bot can push. [Rerunning the SAME run is biased toward the box that just failed it — not a fair pool draw](wiki/learnings/1785861722411-rerunning-the-same-run-is-biased-toward-the-box-th.md)
...
your identity. [run_attempt-1 is NOT a valid proxy for your own rerun count — the bot identity is SHARED (see amendment: by peer coworkers, not by the retry workflows)](wiki/learnings/1785874822609-run-attempt-1-is-not-a-valid-proxy-for-your-own-re.md)
...
`runner_name` are available. [GitHub Actions: judging "workflow stopped firing" and sizing a merge_group window](wiki/learnings/1785899204426-github-actions-judging-workflow-stopped-firing-and.md)
...
the wrong retrieval path self-corrects. [Key a root-cause memory lookup on the ENTITY (PR/issue number), not the symptom's signature](wiki/learnings/1785882088736-key-a-root-cause-memory-lookup-on-the-entity-pr-is.md)
