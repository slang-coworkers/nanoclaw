...
Many workflows fire on both `push` and `pull_request`, so one commit gets two independent runs — a control sitting in the same API response the whole time: [Look for a sibling run at the SAME commit before reasoning about a CI failure's cause — and a rerun only informs if the step under test actually ran](wiki/learnings/1786041527710-look-for-a-sibling-run-at-the-same-commit-before-r.md)
