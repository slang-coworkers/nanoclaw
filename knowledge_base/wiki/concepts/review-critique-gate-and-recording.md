...
- **The DECISION/OUTPUT_REVIEW gate can must-fix an INPUT the approver cannot edit.** A generic comment-hygiene rule ("narrates change-history ⇒ must-fix") is scoped to *your* work product; in the approver the diff is untrusted INPUT and read-only, so the offered remedy (revise the comments) is impossible — and it can conflict with the project's own comment conventions. Accept the gate's FACTUAL catches, hold the scope line, and label an unresolvable soft-cap **ESCALATED** (not CHALLENGER_CONCERN, not a silent WOULD_APPROVE).
...
Framing them as one story matters because the natural reaction to a noisy gate is to trust its *positive* signal more: after five bogus denials, an `OUTPUT_REVIEW=approve` reads as the instrument finally agreeing with you. It is the same instrument, with a known parse defect on exactly that field. **Every other critique-gate defect fails closed — it over-blocks read-only `gh api .../pulls` GETs, its escalation path can be unreachable, and its workspace-scoped edit counter can birth a fresh session already-denied. Those cost friction. The verdict recorder fails open** ([the critique-gate verdict recorder fails OPEN](../learnings/1785989503064-the-critique-gate-verdict-recorder-fails-open-a-mu.md)).

## Input side: three false-positive modes, all fails-closed

Measured on slang PR #12378; all three cost real rounds and none indicated a genuine problem ([critique-gate false positives — GET vs POST, container-wide counter, command text](../learnings/1785992478312-critique-gate-false-positives-it-cannot-tell-a-get.md)).
...
## Gate-scope error: a must-fix on an input the approver can't edit

The gate can misclassify not just a *call* (verb-blind) or a *verdict* (fails-open) but the *scope of its own must-fix*. On shader-slang/slang#12453 @425c2a95 the DECISION_REVIEW gate (codex) held must-fix across all three rounds (soft-cap) and demanded WOULD_APPROVE → ABSTAIN; after every factual catch was fixed, its sole surviving objection was that two **PR-author** comments narrate change-history / a rejected alternative (`m_suppressConsoleOutput` "fd-redirection was tried, not portable on Windows"; a unit-test comment "This is not hypothetical: … failed"). That objection is a **gate-scope error**: the codex developer-instructions carry a generic cross-project comment-hygiene rule scoped "when a code diff is under review," but in the approver the diff is *untrusted INPUT to a decision*, not the agent's own work product, and the approver is **read-only and cannot edit the PR** — so the only remedy the gate offers (revise the comments) is impossible, forcing a downgrade. Worse, the rule conflicts with shader-slang/slang's OWN documented conventions (its CLAUDE.md prescribes conversational, concrete-incident comments — literally "This is not hypothetical:"), which the production reviewer (github-actions[bot], "no bugs/UB/coverage gaps") runs and did NOT flag; a human (jkiviluoto-nv) had already APPROVED the exact head.

Three disciplines follow. **WOULD_APPROVE coexists with acknowledged 🟡 clarity nits** — the approver's enum has no "approve-with-nits" state, and a comment-style nit clears under the conservative-lean rubric (zero trigger reachability, zero coverage impact, zero blast radius — comments don't execute); it is not grounds to ABSTAIN. **Still accept every FACTUAL catch the gate makes** — on #12453 codex correctly caught a "only one untested change" miscount (a second untested seam existed), wrong audit-trail line numbers, a CI-green-as-coverage overstatement, and a false "already counted" claim; fix the facts, then hold only the scope line. **The mechanical trap:** the host gate blocks recording WOULD_APPROVE/BLOCK without a critique `approve` but is *relaxed for ABSTAIN_\**, so a stubborn must-fix on an unfixable input makes WOULD_APPROVE mechanically unreachable — the only recordable outcomes become abstains. Per the skill an unresolved soft-cap **escalates to a human and never silently passes**, so the honest terminal label is **ESCALATED** (a gate/approver deadlock), NOT CHALLENGER_CONCERN (no substantive concern existed) and NOT a silent WOULD_APPROVE; the human APPROVE at head is the falsifiable proof the abstain was gate-induced, not merited. The durable fix is an approver-context carve-out — do not treat PR-author comment style in the input diff as a DECISION/OUTPUT_REVIEW must-fix — and until it lands this shape recurs: escalate as ESCALATED and flag the gate over-application [[approver/critique-mustfix] Critique gate over-applies generic comment-hygiene to un-editable PR-author comments → false ESCALATE](../learnings/1786490730776-approver-critique-mustfix-critique-gate-over-appli.md).
