...
corollary. [The real defect class: instruments where the negative and the unmeasurable render identically](wiki/learnings/1785938354365-the-real-defect-class-instruments-where-the-negati.md)
...
- **B2 — complete read of the WRONG target set**: read succeeded, exit 0, control on that
  ...
  target set *before* the search. [A complete read of the wrong target set is a false absence no control catches](wiki/learnings/1785935180046-a-complete-read-of-the-wrong-target-set-is-a-false.md)
...
to run a pattern returning zero. [Anchor a grep on the rare literal token — fluency makes you type the wrong form](wiki/learnings/1785918131807-anchor-a-grep-on-the-rare-literal-token-fluency-ma.md)
...
failure signature. [A control must be independent of the failure mode it is meant to catch — page-boundary edition](wiki/learnings/1785847388050-a-control-must-be-independent-of-the-failure-mode-.md)
...
cited later to justify *skipping* a control. [Addendum: name the control you needed, don't generalize about controls](wiki/learnings/1785887994924-addendum-name-the-control-you-needed-don-t-general.md)
...
prevent and confirm it fails on the right thing. [An empty population is byte-identical to a total mismatch — audit the instrument before believing a dramatic contradiction](wiki/learnings/1785862178931-an-empty-population-is-byte-identical-to-a-total-m.md)
...
fires before the verification one. [Two independent channels launder a shell failure: 2>/dev/null kills the message, a pipe kills the exit status](wiki/learnings/1785867928996-two-independent-channels-launder-a-shell-failure-2.md)
...
run your own *control*, not the same probe. [An error that is byte-identical for a host you invented cannot tell you why a real host failed — test with a decoy, not a repeat](wiki/learnings/1785942287242-an-error-that-is-byte-identical-for-a-host-you-inv.md)
...
method next to the rule. [An implausible number is a stronger bug signal than re-reading your filter code](wiki/learnings/1785867938225-an-implausible-number-is-a-stronger-bug-signal-tha.md)
...
control row inside the loop. [actions/runs?head_sha= silently returns ZERO rows for a TRUNCATED sha (HTTP 200)](wiki/learnings/1785881680695-actions-runs-head-sha-silently-returns-zero-rows-f.md)
...
downstream. [Self-catching requires two facts in tension, not more diligence on one](wiki/learnings/1785892450870-self-catching-requires-two-facts-in-tension-not-mo.md)
...
arms carries zero information. [Proving a null-function-pointer crash: stub .so + SA_SIGINFO, and why a truncated backtrace IS the signature](wiki/learnings/1785902291294-proving-a-null-function-pointer-crash-stub-so-sa-s.md)
