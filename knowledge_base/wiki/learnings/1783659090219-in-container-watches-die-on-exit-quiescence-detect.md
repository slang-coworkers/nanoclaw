...
- Liveness check for a "should have pinged me by now" coworker: `ncl sessions list --agent-group <id>` and read `container_status` + `last_active` on the relevant thread's session. `stopped` + stale `last_active` = its in-container timers/watches are gone.
