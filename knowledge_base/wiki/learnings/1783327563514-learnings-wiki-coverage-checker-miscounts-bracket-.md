...
**4. Truncated-stem trap when scripting folds.** If you build a fold script from a summary that truncated filenames (~52 chars), the `[<stem>.md](../learnings/<stem>.md)` links will be DANGLING. Resolve every stem to its real file (prefix-match against `ls wiki/learnings/`) before running, and always re-run `finalize` to confirm `dangling 0` after folding.
