...
- [[approver/clause-gap] Bot-authored upstream-sync: size cap is the only human-routing gate under v0-shadow-wide](../learnings/1787808971576-approver-clause-gap-bot-authored-upstream-sync-siz.md)
- [[approver/confirmed-safe] CI env-var-gap-fill PRs (set an existing gated var in one more workflow) merge clean — positive-control + sibling-precedent is sufficient](../learnings/1788170617675-approver-confirmed-safe-ci-env-var-gap-fill-prs-se.md)
- [[approver/confirmed] Main-merge synchronize: re-validate against the new merge base, but don't re-litigate byte-identical PR content — and never approve on pending CI](../learnings/1787586621701-approver-confirmed-main-merge-synchronize-re-valid.md)
- [[approver/confirmed] R1 OPEN_GAP (untested reachable branch) closed by a falsifiable test on re-gate](../learnings/1787596830824-approver-confirmed-r1-open-gap-untested-reachable-.md)
...
- [[approver/false-safe] A supervisor "a non-bot spoke last" nudge can be false: the last event was a BOT review, and re-posting would be the bot talking to itself](../learnings/1786452477308-approver-false-safe-a-supervisor-a-non-bot-spoke-l.md)
...
- [[approver/infra] The OneCLI GitHub proxy is an ALLOW-LIST of path prefixes — `repositories/`, `orgs/`, `user/`, `rate_limit` are all uncredentialed, not just paginated routes](../learnings/1786398787755-approver-infra-the-onecli-github-proxy-is-an-allow.md)
...
- [`ncl` does not support `--flag=value` syntax anywhere — use space-separated values; plus the final inert-flag spec](../learnings/1786243340982-ncl-does-not-support-flag-value-syntax-anywhere-us.md)
...
- [A base64-decode failure makes every grep count 0, including the must-hit control](../learnings/1786363611912-a-base64-decode-failure-makes-every-grep-count-0-i.md)
...
- [A bot-PR "nudge the maintainer to re-approve" unblock routes to the OPERATOR, not the fixer](../learnings/1789181517895-a-bot-pr-nudge-the-maintainer-to-re-approve-unbloc.md)
- [A build subagent that returns a narrated plan is not a completed build — gate on the binary](../learnings/1787331423750-a-build-subagent-that-returns-a-narrated-plan-is-n.md)
...
- [A conclusion surviving every revision is not evidence its current explanation is sound — and a rule that fits the symptom is not a diagnosis](../learnings/1786083921780-a-conclusion-surviving-every-revision-is-not-evide.md)
- [A constant conclusion field can hide a changed mechanism: 277 "declines" were two different diseases](../learnings/1786296545684-a-constant-conclusion-field-can-hide-a-changed-mec.md)
...
- [A delegate's finding about a PR must be checked against the DIFF, not the base — it will read HEAD by default](../learnings/1786085513564-a-delegate-s-finding-about-a-pr-must-be-checked-ag.md)
...
- [A delegated VERIFIED can be circular — check what source it landed on](../learnings/1786131129263-a-delegated-verified-can-be-circular-check-what-so.md)
...
- [A destructive dispatch cannot be recalled — verify before sending, because the executor's refusal is the only gate](../learnings/1786196858803-a-destructive-dispatch-cannot-be-recalled-verify-b.md)
...
- [A diagnostic can be asserted by CODE or by MESSAGE TEXT — census the assertion forms before concluding it is untested](../learnings/1786201352335-a-diagnostic-can-be-asserted-by-code-or-by-message.md)
...
- [A false counter-example holds up the wrong conclusion — and the worst instruments answer confidently about a scope you never checked](../learnings/1786194845056-a-false-counter-example-holds-up-the-wrong-conclus.md)
...
- [A figure you re-type each session is not a measurement — compute elapsed time from an anchor](../learnings/1786284213122-a-figure-you-re-type-each-session-is-not-a-measure.md)
...
- [A fixer "budget-stop" is often a per-session cap, not a funding gap — re-dispatch fresh on the canonical thread](../learnings/1789146576052-a-fixer-budget-stop-is-often-a-per-session-cap-not.md)
...
- [A gate arm written as a negation reads a failed probe as the event](../learnings/1786363841744-a-gate-arm-written-as-a-negation-reads-a-failed-pr.md)
- [A gate armed for one failure mode is not armed for the others — my orphan check false-passed minutes after I proved it worked](../learnings/1786193374139-a-gate-armed-for-one-failure-mode-is-not-armed-for.md)
...
- [A green-lit fix to a scheduled task can be inert — verify it's DEPLOYED (ncl tasks get), not just coded in the dev file](../learnings/1789533346751-a-green-lit-fix-to-a-scheduled-task-can-be-inert-v.md)
- [A grep zero from the wrong pattern will publish the inverse conclusion — trace, don't match](../learnings/1786195883017-a-grep-zero-from-the-wrong-pattern-will-publish-th.md)
...
- [A guard on the ledger write path leaves the report surface ungated — the advice reaches the human either way](../learnings/1786328904632-a-guard-on-the-ledger-write-path-leaves-the-report.md)
...
- [A job conclusion is not a cause: 'skipped' has 4+ distinct causes, and reading them all as lost coverage overstated a gap 28x](../learnings/1786222091299-a-job-conclusion-is-not-a-cause-skipped-has-4-dist.md)
...
- [A local coworker-skill fix is clobbered by the skill's own Step-0 rewrite and never propagates](../learnings/1787117960695-a-local-coworker-skill-fix-is-clobbered-by-the-ski.md)
...
- [A lone pr_closed webhook can be a transient mis-click — re-verify live PR state before acting](../learnings/1789421368163-a-lone-pr-closed-webhook-can-be-a-transient-mis-cl.md)
...
- [A one-shot time-gated nudge needs a durable 'already-fired' latch, not just data conditions + live re-verification](../learnings/1788102208361-a-one-shot-time-gated-nudge-needs-a-durable-alread.md)
- [A partially-adopted review candidate recorded as one verdict loses the declined half — and that half is what gets re-litigated](../learnings/1786219752735-a-partially-adopted-review-candidate-recorded-as-o.md)
- [A passive rebase-hold cannot gate a maintainer with merge rights — escalate, don't rely on the hold](../learnings/1787888940678-a-passive-rebase-hold-cannot-gate-a-maintainer-wit.md)
...
- [A removed POSIX include can compile clean on Linux yet break macOS/libc++](../learnings/1786516325450-a-removed-posix-include-can-compile-clean-on-linux.md)
- [A rerun that surfaces MORE failures did not reproduce them — compare per-JOB, never per-run conclusion](../learnings/1786083740698-a-rerun-that-surfaces-more-failures-did-not-reprod.md)
...
- [A run conclusion does not retract a job that already failed — same field name, different unit](../learnings/1786368150585-a-run-conclusion-does-not-retract-a-job-that-alrea.md)
...
- [A scheduled task's liveness is its own last_run, not the pinned session's last_active](../learnings/1789251566343-a-scheduled-task-s-liveness-is-its-own-last-run-no.md)
...
- [A shared environment gate is not a queue — and two runs with identical job tallies are the easiest pair to merge into one](../learnings/1786307264759-a-shared-environment-gate-is-not-a-queue-and-two-r.md)
- [A shared loader routine is reached by callers with different (path, blob) provenance](../learnings/1787260711213-a-shared-loader-routine-is-reached-by-callers-with.md)
...
- [A single-user-producer census does not clear a per-origin gate](../learnings/1787149294286-a-single-user-producer-census-does-not-clear-a-per.md)
- [A sound measurement is not a sound conclusion — and when a fact is an absence, establish the mechanism that PRODUCES absences first](../learnings/1786306423492-a-sound-measurement-is-not-a-sound-conclusion-and-.md)
- [A successful container restart is not a delivered wake message — verify the inbound row in the target session](../learnings/1786082935150-a-successful-container-restart-is-not-a-delivered-.md)
- [A supervisor artifact check with an unpopulated input reports "no artifact" for 100% of chains](../learnings/1786409083102-a-supervisor-artifact-check-with-an-unpopulated-in.md)
...
- [A test passing for the WRONG REASON is worse than a vacuous one — every routine check comes back green](../learnings/1786217652424-a-test-passing-for-the-wrong-reason-is-worse-than-.md)
...
- [An acknowledged-exceptions list must be honoured by the gate that BLOCKS, not just the one that REPORTS](../learnings/1786343179420-an-acknowledged-exceptions-list-must-be-honoured-b.md)
- [An all-green CI dashboard can mean a stopped pipeline — cross-check throughput, not just queue depth](../learnings/1786264150678-an-all-green-ci-dashboard-can-mean-a-stopped-pipel.md)
...
- [ask_user_question from an a2a-born session is echo-dropped as a noop — the card is emitted, counted, and arrives payload-less](../learnings/1786129496986-ask-user-question-from-an-a2a-born-session-is-echo.md)
...
- [Attribution and routing behind one coworker name is a missing-key problem, not a care problem](../learnings/1786109852075-attribution-and-routing-behind-one-coworker-name-i.md)
...
- [Background builds die on container teardown — foreground chunking is the workaround (looks like a 16h hang)](../learnings/1787337179999-background-builds-die-on-container-teardown-foregr.md)
- [Bare send_message is refused on a long-lived cron session (677 unresponded inbounds)](../learnings/1786296125151-bare-send-message-is-refused-on-a-long-lived-cron-.md)
...
- [Before reporting an action blocked-on-approval, GREP THE POLICY STORE for that action's authority — I invented an operator gate that my own notes retired, and stalled a finished PR for 2 days](../learnings/1786451909191-before-reporting-an-action-blocked-on-approval-gre.md)
...
- [Bodyless-kernel SIGSEGV fix must gate before ALL pytorch/cuda passes, not per-site — sink has no dedup](../learnings/1786714887238-bodyless-kernel-sigsegv-fix-must-gate-before-all-p.md)
...
- [Chain-routing gate needs in_reply_to when message text carries a chain marker](../learnings/1789374291867-chain-routing-gate-needs-in-reply-to-when-message-.md)
...
- [clang-format not on PATH in slang-fixer container; pip-install 17.x per-session](../learnings/1786489601678-clang-format-not-on-path-in-slang-fixer-container-.md)
- [Classifier .mjs edits silently drift from the live deployed scheduled task — check ncl tasks get after every fix](../learnings/1789777475607-classifier-mjs-edits-silently-drift-from-the-live-.md)
- [claude-vs-codex-provider-parity](../learnings/1778085879531-claude-vs-codex-provider-parity.md)
- [Clone the actual repro repo before modeling it from a triage paraphrase](../learnings/1787799707169-clone-the-actual-repro-repo-before-modeling-it-fro.md)
- [CMake: per-target compile flags don't propagate to linked OBJECT libraries](../learnings/1789384635713-cmake-per-target-compile-flags-don-t-propagate-to-.md)
- [Codex critique attestation cascade — batch source edits before the final round](../learnings/1789563826008-codex-critique-attestation-cascade-batch-source-ed.md)
- [Codex critique gate is human-reserved — agents (even Main) cannot self-waive it](../learnings/1789598780832-codex-critique-gate-is-human-reserved-agents-even-.md)
...
- [codex-reply does not count toward the critique delivery gate](../learnings/1787672164164-codex-reply-does-not-count-toward-the-critique-del.md)
- [codex-reply does NOT count toward the critique-gate; OUTPUT_REVIEW must be a fresh mcp__codex__codex call](../learnings/1789452785929-codex-reply-does-not-count-toward-the-critique-gat.md)
- [codex-reply does not record a critique round for the delivery gate](../learnings/1787339458278-codex-reply-does-not-record-a-critique-round-for-t.md)
...
- [CORRECTION to "ci-babysitter-use-canonical-memory-rerun-tracker": phantom-instance inference was wrong — check task session_id pin + transcript first](../learnings/1789749222980-correction-to-ci-babysitter-use-canonical-memory-r.md)
...
- [CORRECTION to the yielded-bot lookback finding: the lookback is UNREACHABLE behind an earlier gate — fix ACTIVE_STATUSES first](../learnings/1786300033438-correction-to-the-yielded-bot-lookback-finding-the.md)
...
- [CORRECTION: `ncl sessions messages --full` is the fix for truncation — my earlier "read the truncated flag" advice taught the workaround, not the remedy](../learnings/1786240671164-correction-ncl-sessions-messages-full-is-the-fix-f.md)
- [CORRECTION: bot CAN ship .github/workflows PRs via the coworkers fork (not patch-only)](../learnings/1787748419987-correction-bot-can-ship-github-workflows-prs-via-t.md)
- [CORRECTION: disableIRValidationScope does NOT gate the post-pass validateIRModule (E40007) — only at-insert asserts](../learnings/1788583742043-correction-disableirvalidationscope-does-not-gate-.md)
...
- [CORRECTION: my "assert returned id == passed id" guard is unsound under auto-fill — the remedy for ncl's equals-form defect is the syntax rule, not any detector](../learnings/1786244050796-correction-my-assert-returned-id-passed-id-guard-i.md)
...
- [CORRECTION: nv-slang-bot GH_TOKEN is VALID — `gh auth status` is cosmetically wrong; use `gh api` (not `gh issue create`) or the onecli-gateway](../learnings/1789716480655-correction-nv-slang-bot-gh-token-is-valid-gh-auth-.md)
- [CORRECTION: NVRTC -pch helps #include-form prelude only, heap is 256MB default (reactive mgmt), needs amortization guard](../learnings/1787307716454-correction-nvrtc-pch-helps-include-form-prelude-on.md)
- [CORRECTION: PR comments DO post via onecli-gateway (curl+HTTPS_PROXY) when gh/GH_TOKEN is broken](../learnings/1789225153684-correction-pr-comments-do-post-via-onecli-gateway-.md)
...
- [Critique delivery gate re-hashes attested files — edit-after-approve blocks delivery](../learnings/1789505147702-critique-delivery-gate-re-hashes-attested-files-ed.md)
...
- [Critique-gate audit false-positives on the literal string "[Fix Report]" in a triager's upstream report](../learnings/1787274342384-critique-gate-audit-false-positives-on-the-literal.md)
...
- [Critique-gate bypass is a human dashboard approval, not an orchestrator action; draft-PR is the release valve for a gate-blocked fix](../learnings/1789272925575-critique-gate-bypass-is-a-human-dashboard-approval.md)
- [critique-gate counts edit EVENTS since last critique — even a no-op revert re-blocks GitHub writes](../learnings/1789652658992-critique-gate-counts-edit-events-since-last-critiq.md)
...
- [CUDA prelude: templated vector operators must be SFINAE-gated + coexist with packed __half2 overloads](../learnings/1787075202710-cuda-prelude-templated-vector-operators-must-be-sf.md)
...
- [Delivery-gate OUTPUT_REVIEW compares the POSTED PR vs your local tree — commit+push+update-body BEFORE running it](../learnings/1789402846129-delivery-gate-output-review-compares-the-posted-pr.md)
...
- [Deployment drift is now self-detecting for the CI babysitter sweep — driftCheck + gate 0e](../learnings/1789778044849-deployment-drift-is-now-self-detecting-for-the-ci-.md)
- [DescriptorKind is a coarse proxy for NV-bindless uint64 representation — gate on the IR type](../learnings/1789559398644-descriptorkind-is-a-coarse-proxy-for-nv-bindless-u.md)
- [Design-RFC issues authored by a language lead: hold the fixer, don't auto-forward](../learnings/1788909843006-design-rfc-issues-authored-by-a-language-lead-hold.md)
- [Detached nohup PR-review runs are killed by container restart with no artifacts](../learnings/1788159236458-detached-nohup-pr-review-runs-are-killed-by-contai.md)
...
- [devin-fetch: three line-count figures name three DIFFERENT artifacts — and the fix landed at the poll predicate, not the exit gate](../learnings/1786124227464-devin-fetch-three-line-count-figures-name-three-di.md)
- [Diagnosing a gh 403/invalid-token in the coworker container (OneCLI app_not_connected)](../learnings/1787673998635-diagnosing-a-gh-403-invalid-token-in-the-coworker-.md)
- [Diff for substance before re-routing the Nth synchronize of a stuck PR-approver loop](../learnings/1789554413155-diff-for-substance-before-re-routing-the-nth-synch.md)
...
- [Empty runner_name does not distinguish "starved by a dead pool" from "skipped by a gate"](../learnings/1786438262793-empty-runner-name-does-not-distinguish-starved-by-.md)
- [Enforce scheduled-sweep safety carve-outs in the deterministic script gate, not in memory/prompt](../learnings/1789248262100-enforce-scheduled-sweep-safety-carve-outs-in-the-d.md)
- [Enum __EnumType conformance failing in multi-file __include modules = stale negative inheritance-info cache (hypothesis, #12926)](../learnings/1788776018731-enum-enumtype-conformance-failing-in-multi-file-in.md)
...
- [Extending TryJoinTypes with a new join arm: gate it OFF for the constraint solver](../learnings/1789678492053-extending-tryjointypes-with-a-new-join-arm-gate-it.md)
...
- [formatting.sh --check-only SKIPS markdown (run_markdown not gated by run_all)](../learnings/1786987129465-formatting-sh-check-only-skips-markdown-run-markdo.md)
...
- [FOURTH presentation of the ncl equals-form defect: swallowed `--id=` + auto-fill returns a valid record for the WRONG identity, exit 0](../learnings/1786243869582-fourth-presentation-of-the-ncl-equals-form-defect-.md)
- [Fresh git worktree lacks submodules; critique gate re-hashes the PR body at send](../learnings/1787818646140-fresh-git-worktree-lacks-submodules-critique-gate-.md)
- [Fresh-worktree Slang builds can hit the per-session cost cap and look like container churn](../learnings/1787277018261-fresh-worktree-slang-builds-can-hit-the-per-sessio.md)
- [Gate a post that exceeds a tightly-bounded pre-clearance; honest after-the-fact flag is the right recovery](../learnings/1789765970729-gate-a-post-that-exceeds-a-tightly-bounded-pre-cle.md)
...
- [gh --paginate silently truncates at page 1 under the OneCLI credential proxy](../learnings/1786098119124-gh-paginate-silently-truncates-at-page-1-under-the.md)
...
- [gh CLI 'invalid GH_TOKEN' — post GitHub comments via the onecli-gateway (curl api.github.com) with the app token](../learnings/1789225036565-gh-cli-invalid-gh-token-post-github-comments-via-t.md)
...
- [gh search/code is DEAD on a fork — every query returns 0, and the two known blind spots do not explain it](../learnings/1786370596653-gh-search-code-is-dead-on-a-fork-every-query-retur.md)
...
- [git add -A in an amend silently ships build logs into a PR — codex OUTPUT_REVIEW caught it](../learnings/1789167532159-git-add-a-in-an-amend-silently-ships-build-logs-in.md)
...
- [GitHub writes: gh GH_TOKEN is invalid in-container; use OneCLI gateway via curl](../learnings/1789373604516-github-writes-gh-gh-token-is-invalid-in-container-.md)
- [github.pr_merged webhook head_sha is the PR BRANCH TIP, not the squash-merge commit — resolve the real master SHA before citing it](../learnings/1787138985818-github-pr-merged-webhook-head-sha-is-the-pr-branch.md)
- [globallycoherent on resource params: two front-end gates + an emit gap](../learnings/1787706145739-globallycoherent-on-resource-params-two-front-end-.md)
...
- [Invalid GH_TOKEN blocks all PR/issue-comment posting — delegate or refresh, don't spin](../learnings/1789224850935-invalid-gh-token-blocks-all-pr-issue-comment-posti.md)
- [Invalid per-agent GH_TOKEN silently degrades a fixer to read-only — triage via MCP, report as blocker (don't retry pushes)](../learnings/1788265619832-invalid-per-agent-gh-token-silently-degrades-a-fix.md)
- [INVERTED HEADLINE: `ncl` silently accepts arbitrary flag names on nearly every verb and returns FULL data — `tasks list` is the lone validating exception](../learnings/1786242295161-inverted-headline-ncl-silently-accepts-arbitrary-f.md)
...
- [Maintainer-driving disposition must be persisted in the Orchestrator's supervisor-state, not a coworker's workspace](../learnings/1789416210923-maintainer-driving-disposition-must-be-persisted-i.md)
- [Mark retention-expired CI reds terminal-unclassifiable once — don't re-derive the same verdict every sweep](../learnings/1786213458375-mark-retention-expired-ci-reds-terminal-unclassifi.md)
...
- [Never bundle a git push with a critique-gated command in one bash block](../learnings/1787846612049-never-bundle-a-git-push-with-a-critique-gated-comm.md)
- [Never forward webhook payload labels/assignees/milestone — delete the slot, don't stamp it](../learnings/1786442851025-never-forward-webhook-payload-labels-assignees-mil.md)
...
- [Peephole test-baseline churn from a gate fix is often finding-1 (missing gate), not finding-2 (state leak) — attribute by reverting on master](../learnings/1787586359149-peephole-test-baseline-churn-from-a-gate-fix-is-of.md)
...
- [Post GitHub comments via OneCLI gateway when gh token is invalid](../learnings/1789496103678-post-github-comments-via-onecli-gateway-when-gh-to.md)
- [Post-restart survivability: only the durable store + GitHub persist — verify memory writes land on disk, ground timestamps with `date -u`, don't cite a prior session's message ids](../learnings/1789590881410-post-restart-survivability-only-the-durable-store-.md)
- [PR age is not gate age — a draft period makes "neglected for months" out of a 2-hour wait](../learnings/1786292353856-pr-age-is-not-gate-age-a-draft-period-makes-neglec.md)
- [PR BLOCKED with all checks green + APPROVED = maintainer gate, not a CI problem](../learnings/1789218490824-pr-blocked-with-all-checks-green-approved-maintain.md)
...
- [PR-body claims must be grounded in the actual disasm, not the triage handoff's framing](../learnings/1786556362704-pr-body-claims-must-be-grounded-in-the-actual-disa.md)
- [PR-review OUTPUT_REVIEW gate rejects "relied on the fixer's test run" — build+run it yourself in a detached worktree](../learnings/1787104438946-pr-review-output-review-gate-rejects-relied-on-the.md)
...
- [Re-verify a triaged draft-PR issue at PR HEAD before investing in a fix](../learnings/1787669928995-re-verify-a-triaged-draft-pr-issue-at-pr-head-befo.md)
...
- [Read the lines around a file:line you cite — and a rule filed only in per-container memory is invisible to every peer](../learnings/1786128826682-read-the-lines-around-a-file-line-you-cite-and-a-r.md)
...
- [Reconcile when your fix PR is closed in favour of a fork](../learnings/1787558759338-reconcile-when-your-fix-pr-is-closed-in-favour-of-.md)
- [Record-replay createSession leaked a ref on registered-FS reuse; live-count stub catches it](../learnings/1788252612832-record-replay-createsession-leaked-a-ref-on-regist.md)
...
- [Refined: ncl's --id / --agent-group-id are globally recognized flags honored only where a verb declares them — `tasks get --id` works, `tasks list --id` is silently inert](../learnings/1786242851046-refined-ncl-s-id-agent-group-id-are-globally-recog.md)
...
- [RESOLVED: CSE-ing read-only resource reads needs TWO coherent/volatile checks (location field-key + by-value aggregate member type)](../learnings/1789453151457-resolved-cse-ing-read-only-resource-reads-needs-tw.md)
...
- [Right evidence, conclusion one axis wider — name what the measurement rules out before publishing](../learnings/1786086589671-right-evidence-conclusion-one-axis-wider-name-what.md)
...
- [Slang PR review: gh/api blocked by OneCLI but git-fetch + slang-mcp work — use --mode patch](../learnings/1788913121098-slang-pr-review-gh-api-blocked-by-onecli-but-git-f.md)
...
- [slangi VM operand .size is an aggregate/storage size, not the scalar width — extracted sub-objects keep their container's size](../learnings/1788900537665-slangi-vm-operand-size-is-an-aggregate-storage-siz.md)
...
- [Split the untested CI bucket by conclusion: skipped is by-design, cancelled is lost coverage](../learnings/1786170572229-split-the-untested-ci-bucket-by-conclusion-skipped.md)
...
- [Staleness is a property of how an artifact is READ; and a correction's radius includes the reasoning that depended on it](../learnings/1786193488406-staleness-is-a-property-of-how-an-artifact-is-read.md)
...
- [Static REF+patch diff can settle a vcpkg overlay-removability question without a build](../learnings/1787228997352-static-ref-patch-diff-can-settle-a-vcpkg-overlay-r.md)
...
- [Step-9 triage comment vs go/no-go: the decision gates the fixer/PR, not a deferring triage note](../learnings/1789599918617-step-9-triage-comment-vs-go-no-go-the-decision-gat.md)
...
- [supervise-issues: persist NO-GO/deferred/held dispositions immediately or the fixer-carve-out re-nudges every tick](../learnings/1789565098418-supervise-issues-persist-no-go-deferred-held-dispo.md)
...
- [supervise-issues: suppress nudges on maintainer-owned / explicitly-deferred chains](../learnings/1789252641916-supervise-issues-suppress-nudges-on-maintainer-own.md)
...
- [Supervisor awaiting_us over-fires: uncredited bot comments + board-sync notices as human](../learnings/1787331439589-supervisor-awaiting-us-over-fires-uncredited-bot-c.md)
...
- [Supervisor nudge false-positives on author-managed PRs](../learnings/1787331527549-supervisor-nudge-false-positives-on-author-managed.md)
- [Supervisor nudge is never a go — and can fire on an explicitly HELD chain](../learnings/1789440749196-supervisor-nudge-is-never-a-go-and-can-fire-on-an-.md)
...
- [Supervisor: record NO-GO/parked disposition or it re-pages the fixer as false-positive-silent](../learnings/1789651416238-supervisor-record-no-go-parked-disposition-or-it-r.md)
...
- [Switching a gate macro #ifdef→#if leaves stale prose; sweep comments+docs+PR-body](../learnings/1788296573909-switching-a-gate-macro-ifdef-if-leaves-stale-prose.md)
...
- [test-falcor waiting on environment gate blocks gh run rerun for sibling jobs in the SAME PR run (not cross-PR)](../learnings/1786986200860-test-falcor-waiting-on-environment-gate-blocks-gh-.md)
- [Testing a threaded/propagated preservation flag: nest the trigger inside an UNDECORATED sub-struct](../learnings/1788428197044-testing-a-threaded-propagated-preservation-flag-ne.md)
- [The "[Resolution]" tag trips the CRITIQUE OVERLAY GATE — use plain phrasing for routine chain-close acks](../learnings/1789633290729-the-resolution-tag-trips-the-critique-overlay-gate.md)
...
- [The receipt for "I routed X" is a session row on X's edge, not a memory note](../learnings/1787068230691-the-receipt-for-i-routed-x-is-a-session-row-on-x-s.md)
...
- [Triage #12562: a follow-up issue split from a chain can describe a diagnostic that does not exist yet](../learnings/1786806563010-triage-12562-a-follow-up-issue-split-from-a-chain-.md)
...
- [Triage a test-coverage follow-up against the PR that introduces the code — verify the symbols exist on master first](../learnings/1788159619973-triage-a-test-coverage-follow-up-against-the-pr-th.md)
- [Triage against stale checkout produced a phantom producer + false maintainer escalation (#12667/#12304)](../learnings/1787620127054-triage-against-stale-checkout-produced-a-phantom-p.md)
...
- [Triage memo file:line findings go stale across HEADs — re-verify before instructing a fixer to add code](../learnings/1787353491544-triage-memo-file-line-findings-go-stale-across-hea.md)
...
- [Triage: check HEAD before treating a perf/feature ask as open — it may already be fixed by a different PR than the issue cites](../learnings/1787226156638-triage-check-head-before-treating-a-perf-feature-a.md)
...
- [Triage: verify a feature exists on master before dispatching a fixer — issues can describe unmerged-PR work](../learnings/1788212132503-triage-verify-a-feature-exists-on-master-before-di.md)
...
- [Triager memo sent UP to the orchestrator is FYI, not a relay request — run the two-session detector before dispatching](../learnings/1788910357132-triager-memo-sent-up-to-the-orchestrator-is-fyi-no.md)
...
- [Verify fixer-claimed PR against live GitHub before rolling up a triage resolution](../learnings/1787067953134-verify-fixer-claimed-pr-against-live-github-before.md)
- [Verify GitHub state before posting a triage resolution — a maintainer may have closed the issue while your chain waited](../learnings/1788159643469-verify-github-state-before-posting-a-triage-resolu.md)
- [Verify live GitHub PR state before acting on a lone pr_closed webhook — state events arrive out-of-order or drop](../learnings/1789421394369-verify-live-github-pr-state-before-acting-on-a-lon.md)
...
- [Which memory store is auto-injected is PER-EDGE — measure it from your own SessionStart output, never inherit a peer's topology](../learnings/1786084759196-which-memory-store-is-auto-injected-is-per-edge-me.md)
- [Whole-object bitcast fast path: four independent correctness gates, each caught only by a different check](../learnings/1787213800352-whole-object-bitcast-fast-path-four-independent-co.md)
- [Wired triager→fixer: don't double-dispatch; claim the edge on the canonical thread](../learnings/1788903865197-wired-triager-fixer-don-t-double-dispatch-claim-th.md)
