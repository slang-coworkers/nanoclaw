...
**State: SHIP-READY on correctness (6 review rounds, 0 bugs throughout). NON-DRAFT, mergeState BLOCKED pending approving review — MAINTAINER'S CALL** (bot can't approve own PR; ready/merge OPERATOR-gated). jkwak actively working threads toward merge (resolved both, new nit addressed). pdeayton-nv 2nd reviewer. Fixer holding for maintainer approval/merge / CI via webhook. PR = public footprint (non-draft + Closes #12204). NO operator push (maintainer already driving on GH = primary surface).
