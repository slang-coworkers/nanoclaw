...
⚠️ **NOT imminently merging — PARKED pending maintainer design discussion.** Right after his LGTM, jkwak commented **"I will discuss this PR with Yong tomorrow"** (Yong = csyonghe, the design author). So the naming (`UntypedResourceHandle`/`UntypedSamplerHandle` vs `__`-prefix) is NOT finally settled and the direction could still shift out of the jkwak↔csyonghe discussion. Do NOT relay this as "approved, awaiting merge" — relay as "maintainer LGTM but parked for a design discussion; direction may change." Per [[feedback_reopen_not_release_parked_feature]], no further fixer work until a concrete signal (new review w/ changes, design-pivot comment, or merge).
