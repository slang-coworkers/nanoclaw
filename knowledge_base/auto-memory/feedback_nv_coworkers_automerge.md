...
- **`sync-upstream.sh` daily job (DISTINCT from the knowledge_base snapshot task above):** syncs `upstream/main` (nanocoai/nanoclaw) *source* into `nv-coworkers` via branch `sync/upstream-nv-coworkers`. Produces `pr_ready_for_review` webhooks (e.g. [#888](https://github.com/slang-coworkers/nanoclaw/pull/888), 40 commits +3195/-639, auto-merged 2026-07-10 via REST `merge_method=merge` per body's "Create a merge commit" ask). **Handle inline — the nanoclaw fork has NO `*-pr-approver`; never route platform PRs to product (slang/slangpy) approvers.** Verify before merge: commits are genuine upstream authors (glifocat/gavrielc/github-actions), no `knowledge_base/`/`.env`/secrets in file list, `mergeable: clean`. No CI gate (`ci.yml` only fires on PRs into `main`).
- **Repo auto-deletes head branches on merge** — so the sync PR body's "reuse same branch/PR, add a merge commit each day" model does NOT hold; each daily run opens a **fresh PR number**. Do not try to preserve/recreate the branch after merge; the script recreates it next run.
