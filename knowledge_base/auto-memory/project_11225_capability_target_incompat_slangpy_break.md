...
## 08-05 18:00Z — human attention arrived; PR is approved+green and MERGE-GATED (Main-verified via REST)

**PR #1088** `dev/slangpy-fixer/1087` head `1dc014be3029`, *"Only request hlsl_nvapi capability where the NVAPI Slang module is linked"*, one hunk in `src/sgl/device/shader.cpp` `+8/−5`, `Fixes #1087`, assignee `jkwak-work`, reviewers requested `skallweitNV`+`jkwak-work`.

- **`skallweitNV` APPROVED @ 08-03T15:14:36Z** — the ONLY review, zero open threads.
- **CI green on `1dc014b`: 13 success / 1 skipped (`Claude Code Assistant`) / 0 failures** — 12 build legs (linux×{x86_64,aarch64}×{gcc,clang}×{Debug,Release}, macos-aarch64-clang×2, windows-msvc×2) + `pre-commit`; combined status `success` (`license/cla`).
- ⚠️**`draft=true` still** — promotion + merge are maintainer-only. Approval ≠ authorization to promote; the fixer deliberately left it draft (correct, per drafts-only guardrail).
- ⚠️**NEW, not in the bot's tracking comment: `mergeable_state=behind`, `diverged`, behind_by=1/ahead_by=2** vs main (`08ae47a4e` #1084 board-sync, `507b4cf16` #1078). If branch protection wants up-to-date, needs an update-to-main first — and a force-push may dismiss skallweitNV's approval, which sits on `1dc014b`. Routed to the triager (who owns the fixer edge); Main did NOT dispatch the fixer directly.
- **Fix verification is the fixer's LOCAL A/B against a source build of `pull/11225/head` (`v2026.14.1-24-gdb61cec`) via `SGL_LOCAL_SLANG=ON`: 28× `E36121` without the guard → 0 with it, delta exactly 28.** Plus 3 `slangc` arms isolating causation (`spirv+hlsl_nvapi`→error, `spirv`→ok, `dxil+hlsl_nvapi`→ok).
  ⛔**The GREEN CI CANNOT OBSERVE THE FIX** — `SGL_SLANG_VERSION` is pinned `2026.12`, which predates #11225, so CI only shows *no regression*. Do not let "approved + green" be read as "fix CI-verified". The fixer itself caught the sharper version of this trap: a positive control (`strings | grep -c 'is incompatible with compilation target'`) gave **2** for its source build vs **0** for the `slang-2026.14.1` release artifact the build downloads — so testing against the *newer release tag* would have returned clean on BOTH arms. ⭐⭐**VERSION-NEWER IS NOT CHANGE-INCLUSIVE WHEN THE CHANGE IS UNMERGED.**
- **Fixer's own `true`-branch closure:** the windows-msvc job runs `SGL_HAS_NVAPI: ON` + `SGL_HAS_D3D12: ON` (the config where the guard KEEPS the request) at 200/200 cases / 20238 assertions / zero `E36121`, and `tests/sgl/testing.cpp:72-83` iterates `{d3d12, vulkan}` per-SUBCASE ⇒ no over-suppression on d3d12. Residual caveat kept honest by the fixer: on Linux `SGL_HAS_NVAPI`=0 so the predicate **short-circuits before the device comparison** — the A/B validates the MECHANISM, not the choice of two-clause predicate (that rests on consistency with the 4 runtime sites `:250`/`:406`/`:508`/`:659` + Windows CI).
- **Non-blocking follow-ups (do NOT expand #1088):** (1) D3D12 CI for the guard's true branch; (2) on metal/cuda/wgpu/cpu slangpy requests NO profile, so dropping the capability entry re-enables `descriptor_handle` auto-promotion (`slang-type-layout.cpp:3549`/`:3559-3563`) — **a behaviour change under the PINNED 2026.12, so it does NOT wait on #11225**; one shader shape showed rc=0 on all of ptx/metal/cpp/wgsl both ways (only diff: a *helpful* E41012 warning removed on ptx), but layout/offsets were never compared.

**Human inbound 08-05T18:00:05Z — `jhelferty-nv` comment `5195409568` on #1087**, addressed to `@zangold-nv` + `@ccummingsNV` (not to the bot ⇒ NOT an `@nv-slang-bot` mention ⇒ no fresh post authorization granted): *"It looks like the bot is giving us a heads-up that slang#11225 we're working on might cause some downstream issues with slangpy?"*
⇒ ⭐⭐**Their premise is UNDERSTATED and the understatement is what keeps #11225 blocked** — this is not a speculative heads-up, it's a confirmed defect with an approved+green fix idle in draft for 2 days. The bot's tracking comment (`5164070567`, last edited 08-03T15:25Z) already said all of it and they commented *after* it and still framed it as a "might" ⇒ **assume an in-place edit does not notify and was missed.** Dispatched `slangpy-triager` (msg 21, pinned `target_session_id=sess-1785745076437-eti3f3`) to reply, lead with #1088, and state the merge gate to zangold-nv. Left the new-comment-vs-edit authorization question for the triager to raise back.

- **RESUME (superseded 08-05 — see the 08-05 18:00Z section above; the draft is OPEN, APPROVED and GREEN as #1088, so the fixer leg is DONE. Remaining gate = a human promotes out of draft + merges, then a human re-dispatches `SlangPy Tests`. The `bmillsNV` requirement below is a SIBLING SESSION'S claim, not Main-verified — re-check the #11225 reviewer list before relaying it.)** = ~~`slangpy-fixer` opens the Approach-A draft on `dev/slangpy-fixer/1087`~~ → **merge to slangpy `main` is required** (an open/draft PR does NOT change `SlangPy Tests`, because `ci-latest-slang.yml` checks slangpy out at its *default branch* for `repository_dispatch`) → human re-dispatch (**not bot-rerunnable**) → `SlangPy Tests` green on #11225 → #11225 unblocked (still ALSO needs `bmillsNV` review). So this chain has a **hard operator merge gate**, not just a draft gate. Or a fresh human comment on either side.
