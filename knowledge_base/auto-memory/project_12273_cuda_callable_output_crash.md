...
## ✅ SHIPPED & CLOSED (2026-07-31 04:54Z) — TERMINAL

**FIXED & MERGED.** PR #12280 MERGED by maintainer jkwak-work — merge commit `c3791ed4ee`, merged from approved head `8694fcb278` (no extra commits). Issue #12273 CLOSED/COMPLETED, `closedBy=[12280]`. Triager re-read the merged 4-file diff: new diag `39032` in slang-diagnostics.lua; 3 RT-param branches in slang-parameter-binding.cpp guard `if (auto rules = ...getRay/Callable/HitAttributesParameterRules())` → E39032 + fall-through on null (mirrors E39017/18); `SLANG_RELEASE_ASSERT(rules)` backstop in slang-type-layout.cpp; regression test `callable-output-unsupported-target.slang` (`//CHECK: E39032` + `//CHECK-METAL: E39032`). Crash→diagnostic on CUDA callable-output + Metal closesthit/anyhit (all-null RT rules); real-rules paths still compile. Issue verdict refreshed in place (comment 5124599262 → "Fixed, merged in PR #12280"). Actual CUDA/OptiX callable-output codegen remains separate feature [[project_12182_cuda_optix_callable_rdc_linkage]] #12182. **RE-OPEN only on fresh substantive human comment.**
