...
- **UPDATE (upstream MERGED, cmt 4996028191): jkwak reports SPIRV-Tools fix merged — issue #6794 + PR KhronosGroup/SPIRV-Tools#6798. "The next SPIRV module update will resolve/close this issue." → Approach A landed upstream; B-interim is now MOOT (no in-tree fold needed). Chain = AWAITING VENDORED-SUBMODULE BUMP (routine SPIRV module update to a spirv-tools version containing #6798) — maintainer-owned cadence (slang-maintainer SPIRV-update workflow). NEAR-TERMINAL: closes when the submodule bump lands. NO fixer dispatch — this is a maintainer routine op, not a fix task. Triager offered to prep the bump PR if jkwak wants it expedited; else the next module update closes it. Resumption trigger: jkwak requests an expedited bump PR, OR the SPIRV module update lands (then confirm #12104 closed).**
