...
- **⚠️ TRUNCATION COROLLARY (my own error, 07-16 #12109): a `head -N`-truncated grep is the SAME trap from the other direction.** A test that both `passed` (early) and `FAILED` (later, after an RPC-drop retry) appears in one log with the `passed` lines FIRST. Grepping with `head -20` caught only the 01:38Z `passed …syn(llvm)` lines and missed the `FAILED test: …syn(llvm)` at 01:49Z (~11 min later) → I wrongly told the babysitter to drop a VALID receipt and overrode its correct flag. **Rule: grep `FAILED test:` (and the RPC-drop context) across the WHOLE log, never head-truncated, before concluding "passed, not failed." A test appearing on a `passed` line does NOT mean it didn't also FAIL later in the same job.** This cuts both ways — I enforced it on the babysitter's name-grep, then committed the truncation variant myself.
