...
**Verdict: PARK at maintainer, NO fixer.** Reasons: (1) COLLABORATOR self-filed+self-assigned → triage+verdict only per standing directive; (2) design-gated protected-path work (`.github/cmake-options-matrix.json` + per-platform ABI/default-on policy = maintainer decision); (3) **blocked-by #12036** (still OPEN/UNMERGED, author pdeayton-nv) — the `SLANG_ENABLE_MIMALLOC` scaffolding (option, `source/slang-mimalloc/slang-mimalloc.cpp`, `windows_only` matrix entry) is NOT on master yet, nothing to build.
