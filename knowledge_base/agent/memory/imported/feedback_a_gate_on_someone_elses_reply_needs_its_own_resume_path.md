...
✅ **Instance closed (08-25):** PR #12593 merged to master (`73deb0f995`) by `tangent-vector`; #12462 auto-closed as completed via `Fixes #12462` (verified at source: PR `merged_at` and issue `closed_at` one second apart, not relayed). Approach A shipped with a CPU regression test + removal of the #12442 quarantine entries; full GPU matrix green, zero fallout — the maintainer's source-vs-behavior assumption held. **So the 3-day drop cost only a public chase, not the outcome** — but that it recovered cleanly is not evidence the drop was cheap: a lead chasing our bot on a fix he'd already authorized is the reputational cost, and it was avoidable at the dispatch step. The lesson is the ordering rule above, not "it worked out."
