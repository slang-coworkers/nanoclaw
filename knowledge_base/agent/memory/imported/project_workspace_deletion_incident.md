...
description: 2026-06-24 unexplained whole-group wipe of the slang-fixer dev workspace mid-chain; dev fixer RECOVERED Jun 29 & disk-full fixed by §8 GC (PR #686); 14:55 culprit never confirmed, no recurrence in 5d → DORMANT
type: project
...
**REFUTES the earlier "buggy GC caused 14:55" hypothesis:** PR #686 / the §8 GC reaps *individual worktrees*, never whole group dirs. The 14:55 whole-group wipe of `lego-groups/slang-fixer` (clones + memory + active-work + queued-inbox + everything) is a DIFFERENT, still-separate, still-unexplained mechanism. Dev fixer still down; **host-log culprit-naming around `14:55:5x` remains the pending operator action.** Prod #11730 build was progressing on freed disk; dev fixer idle-held under the dedup gate. **UPDATE 2026-06-29:** dev fixer RESCAFFOLDED & operational again; prod produced NO #11730 PR in 5 days (verified by Main: `gh pr list --search 11730 --state all` → `[]`, issue timeline shows no connected PR), so the dedup hold was **LIFTED** — dev fixer now producing the draft-held #11730 PR (drafts-only). NOTE: a memory-wiped fixer's account of *why* it resumed is unreliable — fixer claimed "operator authorized (msg 18)" while triager reported "self-initiative"; Main lifted on the verified dedup merits, not the auth claim. 14:55 whole-group-wipe culprit never confirmed; no recurrence in 5 days → DORMANT.
