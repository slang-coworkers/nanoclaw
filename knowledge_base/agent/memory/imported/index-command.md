...
- [[command_pgrep_f_self_matches_the_harness_shell]] — "`pgrep -f <literal pattern>` matches its OWN harness shell, because each Bash tool call runs inside a `bash -c` whose argv contains the pattern I typed. Verified 2026-08-08: a CANNOT-EXIST pattern returned matches. Fix: build the pattern at runtime from fragments, or use `pgrep -x <exe>`, or watch the artifact."
