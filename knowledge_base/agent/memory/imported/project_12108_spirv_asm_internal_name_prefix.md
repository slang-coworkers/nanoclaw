...
**07-24 04:33Z — READY TO MERGE (chain closed pending merge click):** jkwak-work **re-APPROVED @HEAD `4a5d797f7c`** (post doc-update), reviewDecision=APPROVED. CI fully green (Falcor flake rerun passed: Falcor + Falcor Perf + check-ci). Delivered: code fix (413 renames + parse-assert) + doc updates + regression test, full local suite green. **Only remaining step = human merge click (operator-gated).** No blocker.

**07-24 05:55Z — ✅ MERGED / TERMINAL:** PR #12190 **merged by jkwak-work**; issue #12108 auto-closed (Fixes #12108). Shipped: 413 spirv_asm register renames (hlsl+glsl meta.slang + generated GetDimensions in slang-core-module-textures.cpp) + core-module-scoped parse-time SLANG_ASSERT guardrail + regression test `tests/spirv/internal-spirv-asm-opname-prefix.slang` + coding-guidance docs (coding-conventions.md + stdlib-intrinsics.md). 2 maintainer approvals (csyonghe + jkwak-work), Falcor CI flake cleared, codex CODE/PLAN/OUTPUT all passed. Worktree wt-slang-12108 + sentinel reaped. Chain closed.
