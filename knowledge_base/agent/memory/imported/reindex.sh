...
if not CHECK:
    for fam in ('feedback','project'):
        ...
        # The source monolith is a BUILD INTERMEDIATE, fully superseded by the
        # shards it was just split into (row conservation asserted above), and
        # MEMORY.md navigates the shards, never index-<fam>.md. Leaving it on disk
        # is ~200 KB of byte-identical redundancy that reads as an OVERSIZE offender
        # to the OKF synth pass and regrows on every run. Delete it HERE — inside the
        # sharding branch, past the assert — so a small family that hit the `continue`
        # above (its monolith IS the sole index, e.g. technique/reference/command/user)
        # is never removed.
        os.remove(src)
