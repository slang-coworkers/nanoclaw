...
⛔ **2026-08-18 — THE DEDUP STEP IS LOAD-BEARING AND I SKIPPED IT, THEN CITED THE STALE ATTEMPT AS A LIVE RED.** I told the babysitter #12492 had "additional author-owned reds": `check-formatting` + `trigger-slangpy-tests`. The babysitter pushed back — both `pass` live. Re-pulled `filter=all` with **every attempt** on head `32946cc`: `check-formatting` had **3 rows same run_id `32035710932`** — failure 13:33, failure 13:36, **success 16:53** — and `trigger-slangpy-tests` had failure 13:33 → **success 13:36**. `filter=latest` (GitHub's own current view) confirmed **both success**. My query filtered `.conclusion=="failure"` on the `filter=all` firehose and printed the **stale attempt-1 failures** as current. **This is the `newest-per-group ALONE → FAILS CLOSED` half of the recipe above (#12436), re-committed as a fresh error** — except worse, because I didn't even reach the dedup step: I selected-for-failure across all attempts, which is dedup's exact inverse. ⭐⭐⭐**"Select the failing rows from `filter=all`" is NOT a CI-state read — it is a list of every red that ever appeared on this sha, most of them already reran green.** The `select(.conclusion=="failure")` prefilter I use to shorten output is the trap: it discards the success rows that would prove the failure superseded. ⇒ **dedup newest-per-(workflow_id,event,name) FIRST, THEN filter for failure — never the reverse.** Or just read `filter=latest` for the "is it green now" question and reserve `filter=all` for attempt-laddering a *specific* named leg. ⚠️**Same flattering-direction bias as the overstatement above:** the extra reds made my correction to the babysitter look more substantial (I was already correcting its "mergeable" framing, and "and here are two more reds you missed" amplified that) — the babysitter's cheapest probe (one live re-check) killed it, and I should have run that probe before citing, because I was *adding* claims in a correction, the highest-authority/lowest-scrutiny shape. See [[feedback_deference_drifts_to_whoever_corrected_you_last]] (THE SENDING SIDE) and [[feedback_cheap_to_verify_became_substitute_for_verified]].
