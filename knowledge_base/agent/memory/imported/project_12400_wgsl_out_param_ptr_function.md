...
## UPDATE 2026-08-12 → 09-02 — maintainer steer, reconciliation, and Tess's authoritative root (this block postdates the migration snapshot above)

**Re-opened 08-12** by jhelferty-nv (human) cmt 5270930132: param has an *implicit* semantic; issue is a *missing diagnostic*; "talk to Tess (@tangent-vector)." Routed to slang-triager (unpinned — a pin to stopped session `sess-1786035779632-1mffm6` bounced 4×; unpinned on canonical thread always delivered).

**Triager reconciled, posted #12400 cmt `5285462547` (08-13 19:32), all measured @ master `c0e5ca5c5`:** plain `out float4 extra` gets an implicit varying *location* (`OpDecorate %extra Location 0` spirv / `layout out` glsl) but no resolvable binding-semantic *name* (`slang-parameter-binding.cpp:2221`/`:2504`, `HasSemantic`) ⇒ invalid-input-to-diagnose, not a lowering bug — Tess's framing, "unsemanticed" was imprecise. Held a beat that caught a **two-defect split**: an *explicitly* user-semanticed `out float3 : TEXCOORD0` on a **VERTEX** entry still emits `ptr<function>` on **WGSL only** (metal/spirv/glsl/hlsl succeed) — a genuine WGSL lowering gap the diagnostic won't touch, posed as a question to Tess. Also corrected its own #8183 overreach (nested crash is a missing-semantic case, not semantics-independent).

⛔~~OPEN DISCREPANCY~~ **RETRACTED — this was MY stale read, no discrepancy existed.** I twice claimed 5285462547's "corrected above" pointer was false because 5207998744's `updated_at` read `2026-08-06T18:26:37Z`. **Wrong.** Live re-query (Sep-2): 5207998744 `updated=2026-08-13T22:38:35Z` with a dated "Corrected 2026-08-13" block in the body; 5208026808 (#8183) `updated=2026-08-14T04:46:17Z` with "Precision correction 2026-08-13". **Both edits landed Aug-13/14; the pointer resolves.** My earlier Sep-2 query returned the pre-edit `2026-08-06` value — a **stale/cached API read** (a past-dated edit cannot appear only on a *later* query unless the earlier one was stale). ⭐This is the **moment-vs-state axis** of [[feedback_a_negative_on_one_shape_is_not_a_property_of_the_target]] — I read a timestamp as current when it was an old snapshot, exactly the failure I'd recorded. The triager **verified live before complying and correctly declined to re-edit** (would have double-stamped correction blocks on a maintainer-watched issue) — it caught my stale snapshot. ⇒ ⭐⭐**a `gh api` `updated_at` can itself be stale; a "not edited" claim built on it is a moment-claim, not a state-claim — re-query at point of use, and a peer's live read that contradicts mine wins until I re-verify.**
...
- Params: **uniform** (`uniform` modifier + always-uniform types e.g. textures/buffers) vs **varying** (all else). **Return type = implicit `out` varying param.**
- Varying params may carry an explicit semantic; those without get an **auto-assigned** one later in compilation — **always a USER-DEFINED semantic**. Semantic = **system-value** (`SV_`, case-insensitive) or **user-defined**.
- Each stage has varying **boundaries** (`in`/`out`), and defines per boundary: (a) allowed SV semantics + types, and **(b) whether user-defined semantics are allowed on it**.
- ⭐**THE RULE:** *the `fragment` stage does not permit varying params with user-defined semantics along its `out` boundary.* Front-end (once stage known), on a param with a user-defined semantic **or no explicit semantic**, consults the stage/boundary policy and **diagnoses an error** if that boundary disallows user-defined semantics.

⇒ **Resolves the two-defect split AND the open question ("invalid regardless?" → NO, stage+boundary-dependent):** Defect 1 = fragment `out` + user-defined semantic (explicit or auto) → diagnose (covers plain repros + #8183 nested). Defect 2 = **vertex** `out : TEXCOORD0` → vertex `out` *permits* user-defined semantics (interpolant mechanism) ⇒ valid input, WGSL `ptr<function>` is a real lowering bug. Triager to GROUND in the stage/boundary policy + CONFIRM with Tess, not assert over her.

⚠️**Tess called the bot's comments "a whole lot of words without conveying much content" and demanded CONCISE.** Reply must be short: confirm model + measurement match, raise the single vertex-vs-fragment point, note the WGSL lowering as separate — a few sentences, no re-derivation of what she already wrote. The 5207998744 edit should now adopt her boundary/policy vocabulary.

**Memory note:** this OKF tree migrated from `~/.claude/projects/-workspace-agent/memory/` to `/workspace/agent/memory/` (leaves under `imported/`) between turns; the pre-migration snapshot lost this session's in-place edits, hence this consolidated append.
