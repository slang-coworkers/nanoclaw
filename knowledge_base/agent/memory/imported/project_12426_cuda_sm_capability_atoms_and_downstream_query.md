...
## 2026-08-24 (later still) — author ESCALATES + requests a split. Capabilities-reporting half is DEAD. `@nv-slang-bot` addressed directly.

`tdavidovicNV`, cmt 5466983417, cc @kaizhangNV @jhelferty-nv: *"by far the most important issue for us
on CUDA speed up. All we need is `getDownstreamCompilerPath`, the rest can go… @nv-slang-bot it might
be best to split this into the two actually desired PRs or issues:*
- *Implement `getDownstreamCompilerPath` (complement `setDownstreamCompilerPath`, mirror the prelude pair)*
- *add the missing `_cuda_sm_xxx` capabilities to the enum.*
*The request to report capabilities is no longer valid — the host app does it itself once it has
`getDownstreamCompilerPath`."*

**Decisive scope collapse:**
- ⛔ **Half-2 (the capabilities-reporting API, all of the original issue's API design + your Q5 + the
  NVRTC-symbol/optional-loading analysis) is WITHDRAWN by the author.** "no longer valid." The host
  will `dlopen` + `nvrtcGetSupportedArchs` itself once it can find the library path.
- The issue collapses to **exactly two deliverables**, author's priority order:
  1. **`getDownstreamCompilerPath`** — the string getter (URGENT, "the rest can go"). Grounded: still
     absent in `include/slang.h` (grep=0). One appended method mirroring `getDownstreamCompilerPrelude`.
     ABI-trivial. **Still an author/maintainer ABI task** — appended public vtable method, semantics
     choice (set-vs-resolved path, per my earlier flag). Not bot-implemented unsolicited unless asked.
  2. **`_cuda_sm_xxx` atoms** = our B. Grounded: still 9 CASE rows / 11 atoms. Unchanged, still on Q1.
- **A (the `sm_89→sm_80` CASE-table bug)** is NOT one of the author's two bullets — he doesn't know
  about it. It rides *inside* deliverable 2: adding atoms without CASE rows reproduces the exact defect
  at N new atoms, so whoever does 2 must fix A in the same change (bare-`-capability` `.target` test).
- **#12649** (arch clamp, OPEN): the author's "the rest can go" is about the *reporting API*, not
  necessarily this PR — but its motivation (avoid requesting an arch NVRTC rejects) partly overlaps
  what the host now plans to do itself. Someone should ask the maintainer if #12649 is still wanted.

**Explicit `@nv-slang-bot` request to split** = post-authorized action for the triager (real mention).
Routed to `slang-triager` (holds verdict+memo, has posted every prior comment, closest-to-state) to:
create the two split issues/PRs as the author asked, carry A into deliverable-2 scope, and confirm
#12649 disposition. **I did not create issues or post** — triager owns GitHub writes here.
**RESUME:** triager splits + reports, or maintainer (@kaizhangNV/@jhelferty-nv) acts.


## 2026-09-01 — ✅ TERMINAL. Maintainer CLOSED #12426; deliverables live in #12838/#12839.

`kaizhangNV` commented cmt 5498027675 *"close this issue as discussed offline"* and **closed #12426
himself** — verified via issues API: `closed_by: kaizhangNV`, `state_reason: completed`,
`closed_at 2026-09-01T17:46:54Z`. A **human close, not a bot action** (checked the actor before
treating it terminal — the never-auto-close guardrail was not touched).

Final state:
- **#12426** CLOSED/completed by maintainer. Reporting-API half withdrawn by author; tracking parent done.
- **#12838** OPEN — `getDownstreamCompilerPath` getter (maintainer ABI task).
- **#12839** OPEN — atoms **+ the `sm_89→sm_80` CASE-table bug in its title/scope** (the through-line:
  a routine triage surfaced a live silent-wrong-codegen bug that survived every scope change to land
  titled in a tracked issue). Fixer gated on maintainer answering Q1 (renumber vs append).
- **#12649** OPEN — arch clamp (jvepsalainen-nv), independent of #12426's closure.

No action owed (human close, children tracked). **RESUME (children only):** maintainer answers Q1 on
#12839 → route fixer (CASE rows + atoms + bare-`-capability` `.target` regression test); #12838
maintainer-implemented; #12649 disposition is a maintainer call.

⚠️ STORE NOTE (2026-09-01): written to `/workspace/agent/memory/imported/` — the memory store was
migrated during the id-28 restart; the old `/home/node/.claude/projects/-workspace-agent/memory/` is
now empty of leaves and the top-level `index.md` banner still points there (stale). Could not run
`reindex.sh` (moved). Flagged to operator.
