# Hermes port — dispatch plan v3 for the 64 gap-matrix rows (Bot-Mode re-baseline)

Written 2026-09-09. Supersedes `reports/hermes-dispatch-plan-2026-09-08.md`. Same structure, new content: every one of the 61 re-baselined rows carries a re-baseline **disposition** (`rows-final.json`), plus the fleet-assembly row **FLEET-F62** added 2026-09-16 and the P7 OpenShell rows **OSH-F63** / **OSH-F64** added 2026-09-17 (64 rows), so only `BUILD` and `CONFIGURE` rows are dispatched to the build chain. Copies: Mac `~/brev/nanoclaw/reports/hermes-dispatch-plan-v3.md`, box `~/haaggarwal/nemoclaw-coworkers/data/shared/hermes/dispatch-plan.md` (= `/workspace/shared/hermes/dispatch-plan.md` inside the Orchestrator's container).
...
| batch 0, 1a, 1b, 2, 3, 4, 5, 6 | this file | **dispatch batches** to the Orchestrator, one per phase in the `phase` field of `rows-final.json` (batch 6 = P7-openshell, added 2026-09-17) |

Batch order is now the row's own `phase` field: **P2 → P3-waveA → P4-sandbox → P5-rooms-veto → P6-fleet → P7-openshell → P8-upstream** (P7-openshell = batch 6, the two OSH rows added 2026-09-17; not in `rows-final.json`, like FLEET-F62). That order still satisfies the port plan's dependency line ("P4 blocks P6/P7 · P3a blocks P4/P5 · P2 blocks all plugin exits"), because P2 is where the compose plugin renders the config every later phase reads.
...
flowchart TD
  ...
  B4 --> B5
  B5 --> B6["Batch 6 · phase P7-openshell · 2 BUILD rows<br/>OSH-F63 OpenShell-native substrate: one sandbox per profile over ssh, openshell policy per bot<br/>OSH-F64 the fleet under OpenShell (demo, install into an existing NemoClaw sandbox)<br/>gate: batch 5 merged (batch5_merged); F64 starts on F63's tester PASS (osh_f63_first_pass), merges after F63 (hold osh-f63)"]
...
2. **Only `BUILD` and `CONFIGURE` rows are dispatched.** 33 of the 64. `ADOPT` (16) is a doc page plus one hermetic acceptance test, run on the adopt track, never through architect → builder → tester → reviewer. `MERGE→` (11) is never dispatched at all: the id becomes a named criterion `AC-<id>` in the target row's ADR and the target's PR cannot merge without it. `DEFER` (4) is recorded with its reason and not dispatched.
...
|---|---|---|
| `P0-LOOP` (not one of the 64) | `plugin/hello` travelled builder → fork draft PR → tester `[Test Report]` → reviewer verdict → Orchestrator merge gate, with all three criterion kinds exercised (`pytest:` / `ui:` / `live:`) | **DONE**, merge pending |
...
**Two BUILD rows, one substrate ruling.** Operator ruling 2026-09-17 (~15:30Z): R5 = **Option 1 — OpenShell-native sandboxes, one per profile**. Under OpenShell the fleet does NOT run podman inside the sandbox — measured 2026-09-17 13:02–13:07Z inside `brev-hermes` (openshell CLI 0.0.72, gateway `nemoclaw`): `pivot_root` EPERM (seccomp/mount), `/sys/fs/cgroup` read-only, no `/dev/net/tun`, `/proc/sys` read-only. Each profile gets its own OpenShell sandbox, reached through Hermes's built-in `terminal.backend: ssh` (`tools/environments/ssh.py`: `terminal.ssh_host` / `ssh_user` / `ssh_port` / `ssh_key`; the gateway user's `~/.ssh/config` applies); egress is policed by `openshell policy` per sandbox instead of iptables. Podman stays for plain-host deployments (FLEET-F62, batch 5). Neither row changes core and neither ships a plugin of its own: OSH-F63 extends the compose render (`substrate: openshell`) and nv-fleet-gates' sandbox predicate; OSH-F64 composes FLEET-F62 + OSH-F63.

Dispatch gate: every batch 5 row merged (or in `config.waive`) — `batch5_merged`, i.e. FLEET-F62 (gate added 2026-09-17; before that no later batch waited on batch 5). Inside the batch OSH-F64 is ordered after OSH-F63 and both halves of rule 3 are machine-enforced for it, the way `1a_first_pass` + `hold: 1a` enforce 1b vs 1a (2026-09-18): it may *start* when OSH-F63's PR has a tester PASS at its head — the queue gate `osh_f63_first_pass` (`hermes_queue.dispatch_order` gives OSH-F64 the tuple `("batch5_merged", "osh_f63_first_pass")`; true when OSH-F63 is merged or waived, has a tester PASS on its thread or in its ledger verdict, or a ledger state past testing) — and may not *merge* before OSH-F63 merges — the supervisor's `hold: osh-f63` at `gate` until `osh_f63_merged` (OSH-F63 merged or waived), evaluated after the `batch5` hold that keeps all of batch 6 behind FLEET-F62. The demo tracker orders OSH-F64 `after` OSH-F63 as well; no `paused_rows` entry is needed for the ordering, and a paused OSH-F64 behind the unmet gate never raises `idle-capacity`. Both rows' live criteria depend on an operator-provisioned, name-prefixed OpenShell access path for hermes-tester (the P7 prerequisite, as the podman socket was for P4); until it exists those criteria are carried under rule C.2 (`container/spines/hermes/context/delegated-decisions.md`) **once the operator extends C.2's to-row from `FLEET-F62` to the batch's landing row (`OSH-F64` for batch 6)** — until that ruling the carry is a DECISION NEEDED on the operator DM, not a delegated default (C.2 as written names only `FLEET-F62`, which is merged by then). OSH-F63's live criterion carries onto OSH-F64 as an `open` § Carried criteria row; OSH-F64's own live criteria 3–5 carry as `open` § Carried criteria rows onto an operator-opened follow-up row `OSH-F64.a` (hermes_queue accepts a ledger follow-up row as to-row, `carried_to_row`), with the P7 access path itself tracked by id (an `open` § Carried criteria row or a `UA-<n>`) so C.2's tracked-prerequisite clause is met and the coverage line shows them — never a ledger note, which `coverage.carried_line` cannot see. Two ssh routes are documented and the spec picks one, recording why: (a) gateway-brokered ssh via `openshell sandbox ssh-config` — the OpenShell gateway brokers ssh into a sandbox through its PID-1 `openshell-sandbox` on `/run/openshell/ssh.sock`. **Route (a) admissibility — MET, measured 2026-09-18 06:50–07:19Z:** the OpenShell docker driver mounts the gateway's client mTLS identity into EVERY sandbox (`/etc/openshell/tls/client/`, env `OPENSHELL_TLS_*`); from inside `brev-hermes` the static `openshell` CLI with a gateway entry built only from those env paths listed, created and `policy get` sibling sandboxes and **ssh + scp'd into a sibling via `ProxyCommand openshell ssh-proxy --gateway-name <g> --name <sandbox>`** (user `sandbox`; hostname/uid differed; file round-tripped). No operator credential enters any sandbox (`~/.config/openshell` is never copied; the CLI metadata is built from `OPENSHELL_TLS_*` + `OPENSHELL_ENDPOINT`); the worker image ships no openshell CLI; the finding that a plain sandbox holds gateway-admin credentials goes to the NemoClaw/OpenShell team as a question. Direct sandbox-to-sandbox TCP is blocked (000 both ways) and `openshell forward` stayed inconclusive, so route (b) is the documented fallback only.** The operator's `~/.config/openshell/{active_gateway,forwards,gateways}` gateway credentials are admin power over ALL sandboxes and never enter `<prefix>-gw`, any worker sandbox, or `brev-hermes`. Otherwise route (b): host-side `openshell forward` of an sshd port inside each worker sandbox to a bridge address the gateway sandbox reaches — needs sshd in the worker image + pubkey upload.

| Row | Name | Disp | Deliverable | AC |
|---|---|---|---|---|
| OSH-F63 | OpenShell-native sandbox substrate (P7): one OpenShell sandbox per coworker session (or per profile), reached over ssh | BUILD | The compose spec gains the key `substrate` (default `podman` = FLEET-F62's render, byte-unchanged; `openshell` new); for `openshell` the render emits per profile `terminal.backend: ssh` + `ssh_host` / `ssh_user` / `ssh_port` / `ssh_key` (a key PATH under the gateway's `$HERMES_HOME`, never a secret in config) pointing at THAT profile's OpenShell sandbox, plus one `openshell policy` YAML per profile rendered from the fleet spec's egress section (the OneCLI hop 172.17.0.1:10255, the inference route and the ssh control path — nothing else), plus a deterministic provisioning plan whose dry run prints the N `openshell sandbox create --name <fleet>-<profile> --from <pinned image> --policy <yaml> …` lines, the `openshell policy set` lines, the `openshell sandbox ssh-config` lines and the teardown, plus the runbook. Sandbox scope: Hermes's docker backend spawns one container per SESSION while the stock ssh backend is one static host per profile, so the spec key `sandbox_scope: session | profile` (default `session`, FLEET-F62's isolation) is part of this row — `profile` = the static per-profile sandbox, `session` = a `terminal-environment` provider `openshell` (Hermes seam `register_terminal_environment_provider`; a plugin seam, not a CORE-CHANGE) that creates a sandbox per session through the gateway API the gateway sandbox already reaches (measured ~2 s to Ready) and deletes it with the idle reaper. Both ssh routes documented; the spec picks one and records why — route (a) under the admissibility rule above, met 2026-09-18 by the platform-mounted sandbox identity (the operator's gateway credentials never inside a sandbox). No podman wrapper under this substrate; nv-fleet-gates' sandbox predicate (AC-ISO-F10) treats the profile's ssh environment as its own environment (stdio mcp / cross-profile still denied); ISO-F13 mount policing restated for OpenShell (the sandbox spec's mounts, no host paths); ISO-F14's egress render carried (proxy env + CA inside; lockdown = `openshell policy`); CRED-F28's per-profile OneCLI identity carried. No CORE-CHANGE. | Exactly six `AC-OSH-F63-1..6`: 1 pytest (render byte-equal to committed fixtures for `substrate: openshell` — five profiles → five terminal blocks, backend ssh, distinct host/key, no `docker_*` keys; FLEET-F62's committed fixtures render byte-identical with `substrate` absent or `podman`); 2 pytest (per-profile policy YAML byte-equal + a hermetic assertion — or `openshell policy prove` when the CLI is present — that only the hop, the inference route and the ssh control path are allowed; the OneCLI control plane 10256 never; the OpenShell gateway endpoint only if the policy grammar requires it for the sandbox's own control plane, never as a tool-egress allow); 3 pytest (nv-fleet-gates predicate with an ssh env: own env accepted, stdio mcp denied, cross-profile denied); 4 pytest (hermetic provisioning dry-run byte-equal: create / policy / ssh-config / teardown lines); 5 live (W0-style, on the box through the operator-provisioned name-prefixed OpenShell access path; if not ready, carried onto OSH-F64 as an `open` § Carried criteria row under rule C.2 once its to-row is extended — until then a DECISION NEEDED on the DM: one probe sandbox from the pinned image with the rendered policy, a `hermes -p <profile>` terminal call executes inside it — hostname / uid differ from the gateway host — and a denied egress attempt shows in `openshell logs` (`--source gateway` or `--source sandbox`, whichever records the policy denial — the ADR records which), teardown); 6 doc (`website/docs/user-guide/fleet-openshell.md`: architecture, why not nested podman with the probe facts, both ssh routes and the route-(a) admissibility rule, prerequisites). |
| OSH-F64 | Fleet under OpenShell (P7 demo): the five-profile gateway inside an OpenShell sandbox, one worker sandbox per profile, APF per bot, desktop attached | BUILD | The worker-sandbox image as a Dockerfile usable by `openshell sandbox create --from` (pinned Hermes runtime + tools + whatever the chosen ssh route needs); the gateway-side install path for an EXISTING NemoClaw Hermes sandbox as one idempotent script + runbook section (`hermes plugins install` of the port's plugins from the fork at the release tip, `hermes coworker compose --substrate openshell` from the demo spec = FLEET-F62's `tests/e2e-scenarios/FLEET-F62/spec/coworker-types.yaml` with `substrate: openshell` flipped, five coworker `hermes profile install` + a backed-up in-place edit of the EXISTING default profile's config.yaml — never `profile install default` over an existing home —, `gateway.multiplex_profiles` + allowlist, gateway restart the way the sandbox runs it — the script detects the supervisor, never assumes s6); the demo proof runs on a THROWAWAY gateway sandbox on the box — never on the user's `brev-hermes`; installing there is the operator's manual step (demo-path `P7-OPERATOR`, 8 h). Composes FLEET-F62 + OSH-F63 unchanged; no core change, no second gateway, no a2a. Ordered after OSH-F63: queue gate `osh_f63_first_pass` (start on its tester PASS), merge hold `osh-f63` (merge after it). | Exactly six `AC-OSH-F64-1..6`: 1 pytest (worker Dockerfile + install script lint / dry-run byte-equal fixtures; the dry run shows the default backup path and the diff applied to default, and a second run is a no-op on default); 2 pytest (hermetic `hermes coworker compose --substrate openshell` on the demo spec → five profiles + five policies + provisioning plan byte-equal; every key outside `terminal.*` / the policies / the provisioning plan byte-identical to FLEET-F62's committed render); 3 live (a throwaway gateway sandbox boots the multiplexed gateway with ≥ 2 profiles, each profile's terminal call executes in its own worker sandbox — nonce markers — and egress denied per policy is visible in `openshell logs`, `--source gateway` or `--source sandbox` as the ADR records); 4 live (two-bot delivery orchestrator → builder through the worker sandboxes with the fleet-admin veto live, bounded LIVE budget); 5 ui / desktop (the dashboard or hermes-desktop reaches the gateway sandbox through `openshell forward` / `service expose` and shows the profiles — Playwright or dashboard smoke); 6 doc (runbook § "install into an existing NemoClaw sandbox": operator step, rollback = restore the default backup + remove the five profiles + uninstall plugins + restart, what the NemoClaw dashboard / APF shows per sandbox). Live criteria 3–5, when the OpenShell access path is not provisioned, carried as `open` § Carried criteria rows onto the operator-opened follow-up row `OSH-F64.a` under rule C.2 once its to-row is extended (see above). |
...
33 dispatched (batch 1a **1** + 1b **18** + 2 **6** + 3 **4** + 4 **1** + 5 **1** + 6 **2**) + 16 adopt + 11 merge + 4 defer = **64**. Every one of the 64 ids appears exactly once across batches / adopt / defer / merge. Batch 0's `P0-LOOP` is not one of the 64.

By disposition: BUILD 10 · CONFIGURE 23 · ADOPT 16 · MERGE 11 · DEFER 4 = 64.
By phase (dispatched only): P2 19 · P3-waveA 6 · P4-sandbox 4 · P5-rooms-veto 1 · P6-fleet 1 · P7-openshell 2 · P8-upstream 0 = 33.

## What changed vs the 2026-09-08 plan

**Counts.** Dispatched rows drop **61 → 30** (31 since 2026-09-16, when FLEET-F62 made the batch-5 assembly a row; 33 since 2026-09-17, when OSH-F63 / OSH-F64 made the P7 OpenShell substrate and demo rows — batch 6). Plugins to be written drop **11 → 6** (`nv-coworker-compose`, `nv-fleet-gates`, `nv-approval-ledger`, `nv-artifact`, `nv-cost-cap`, `podman-onecli`). Upstream asks drop **17 esc=Y rows → 8 named asks + 1 parked + 2 operator core-change calls**.
...
- **Old batch 5 keeps its shape but loses its rows.** P6 had zero matrix rows after the re-baseline; on 2026-09-16 it regained exactly one, **FLEET-F62** — the fleet assembly itself, so the deployment shape is specified, built, tested and reviewed like every other row (and is the to-row for `AC-CRED-F28-2`). P7 (OpenShell/NemoClaw + APF) entered this file on 2026-09-17 as **batch 6** — OSH-F63 (the OpenShell-native substrate) and OSH-F64 (the fleet under OpenShell, demo + install into an existing NemoClaw sandbox) under the operator's Option 1 ruling: one OpenShell sandbox per profile over ssh, egress by `openshell policy`; not nested podman, which the sandbox spec blocks (measured the same day).
...
- `groups/orchestrator/reports/ledger.md` — the work list, one row per dispatched id (`merged/blocked` is the outcome). 33 rows now, not 64; the 31 non-dispatched ids (16 adopt + 11 merge + 4 defer) appear as `adopt` / `merged-into:<id>` / `deferred` lines so the coverage check stays checkable from the ledger alone.
