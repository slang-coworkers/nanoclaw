...
```
Create a coworker called "Slang Docs Writer" that reviews the user
guide for outdated examples and missing docs for new features.
```

Andy registers it, sets up its workspace and CLAUDE.md with the right instructions, and optionally schedules tasks. The coworker gets platform sections (shared learnings, subagents, report persistence) automatically via the template system.

**Agent Teams**: Coworkers can spawn parallel sub-workers for complex tasks. Workers run in separate tmux panes inside the container and survive across query turns.
