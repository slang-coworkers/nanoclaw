...
- `groups/templates/projects/slang/` — Slang overlays for global, main, and coworker prompts
...
This branch uses the v2 composition system instead:

- `groups/templates/projects/slang/global-overlay.yaml`
- `groups/templates/projects/slang/main-overlay.yaml`
- `groups/templates/projects/slang/coworker-base.yaml`
- `groups/coworker-types.json`

The base prompts stay clean. Slang-specific behavior is added through overlays and coworker role templates.

After merging the branch, refresh the checked-in composed prompts so `groups/main/CLAUDE.md` and `groups/global/CLAUDE.md` include the Slang overlays:
...
From the main chat, users can spawn specialist coworkers after choosing a type from `groups/coworker-types.json`:

```
@Andy spawn <type-from-groups/coworker-types.json> investigate-generics "Investigate generic type inference in the IR"
```
...
- `container/skills/slang-templates/templates/` — reusable template files and related building blocks

If new roles or templates are added later, they should be discovered from those paths rather than hardcoded in this skill.
