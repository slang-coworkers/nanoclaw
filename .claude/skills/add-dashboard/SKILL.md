...
- `groups/templates/projects/dashboard/formatting.yaml` — dashboard-specific web formatting guidance merged into the `Formatting` section for composed prompts
- Shared runtime wiring lives on `v2_main`:
  `src/channels/dashboard.ts`, `src/channels/index.ts`, `src/dashboard-ingress.ts`,
  `src/container-runner.ts`, `src/index.ts`, and `vitest.config.ts`
- `package.json` — `dashboard` script added

If the merge reports conflicts in shared runtime files, stop and fix branch ownership first.
This skill branch should only add dashboard payload, not `v2_main`-owned wiring.

### Prompt layering

The base `groups/main/CLAUDE.md` and `groups/global/CLAUDE.md` stay on `v2_main`.
No direct edits to `groups/main/CLAUDE.md` or `groups/global/CLAUDE.md` are needed.

After merging the dashboard branch, refresh the checked-in composed prompts so the dashboard formatting is reflected in both `groups/main/CLAUDE.md` and `groups/global/CLAUDE.md`:
