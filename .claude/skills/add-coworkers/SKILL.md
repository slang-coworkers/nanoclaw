---
name: add-coworkers
description: "Bootstrap the NanoClaw lego coworker system. Merges base infrastructure (composer, trait resolver, spine system), then offers project-specific add-ons (dashboard, slang, custom projects via /onboard-project)."
---

# Add Coworkers

Bootstrap the full lego coworker system into a NanoClaw installation.

## Pre-flight

If on a detached HEAD, create a local branch first:
```bash
git symbolic-ref HEAD 2>/dev/null || git checkout -b lego-coworkers
```
...
- Updated add-dashboard and add-slang skills (merge from lego-* branches)
...
Then invoke the skill. Each skill merges its own `origin/lego-*` branch.
