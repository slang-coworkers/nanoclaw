...
function render(map, reasons) {
  ...
  // HEADER already ends in a newline. Falling through with zero rows would append
  // a second one and write a trailing blank line — which the whitespace/EOF guard
  // rejects, so `--write` on a fully-paid baseline would emit an unmergeable file.
