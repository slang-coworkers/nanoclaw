...
- **Shared-source agent-runner.** Per-group `agent-runner-src/` overlays are gone; all groups mount the same agent-runner read-only. Per-group customization flows through composed `CLAUDE.md` (shared base + per-group fragments).
  > **History:** this did NOT ship with the release that claimed it — a 2026-08-10 correction recorded that `group-init.ts` still made a per-group copy and `container-runner.ts` still mounted it writable. It shipped later, on nv-main: the per-group copy and the `check:runner-staleness` tooling built to chase it are both gone. See [Agent-runner source](CLAUDE.md#agent-runner-source-one-shared-read-only-mount).
