...
_MAX_ACTIVE_SELECTIONS = 1000  # Cap to prevent unbounded memory growth
...
class FeedbackView(discord.ui.View):
    ...
    async def _toggle(self, label, interaction):
        ...
        if msg_id not in _active_selections and len(_active_selections) >= _MAX_ACTIVE_SELECTIONS:
            # Remove the oldest entry (first inserted key)
            oldest_key = next(iter(_active_selections))
            del _active_selections[oldest_key]
