...
function parseAllowedMcpTools(): string[] {
  if (!process.env.NANOCLAW_ALLOWED_MCP_TOOLS) return [];
  try {
    return (JSON.parse(process.env.NANOCLAW_ALLOWED_MCP_TOOLS) as string[]).filter((tool) => tool.startsWith('mcp__'));
  } catch {
