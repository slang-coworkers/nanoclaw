...
// INTENTIONAL FORK DIVERGENCE (upstream #3738 acceptance test, deliberately skipped).
//
// Upstream adopts an arriving fresh-session task into an idle pre-warmed query.
// nv-main must NOT: a task that wants a fresh session is spliced out of the push
// and left `pending` so the next poll routes it through the fresh-session path,
// preserving the per-series session isolation `ncl tasks` is built on. See the
// deferral block in poll-loop.ts and its own coverage at
// poll-loop.test.ts:2130 ('fresh-session task arriving mid-query — defer, never
// abort the active turn'), including the freshSessionArrivalAction defer→end
// table. That behaviour is prod-driven: on 2026-09-09 a scheduled fire landed
// while the same series' manual run was mid fan-out and killed its in-flight
// tool calls.
//
// The state this test constructs is also unreachable through nv-main's own poll
// loop, which never passes an empty initialPrompt (poll-loop.ts guards
// `messages.length === 0` before building the prompt) — it is reachable only by
// calling processQuery directly, as this test does.
//
// The rest of #3738 IS adopted: turn-route FIFO, publish-on-adopt, the full
// ReplyRoute stamp, and the sibling test below ('restores chat delivery when a
// task query receives a chat turn') all pass unmodified.
it.skip('adopts task delivery and writes a task log when an empty warm query receives a task', async () => {
  const pushed: string[] = [];
  const query: AgentQuery = {
    push: (p) => {
      pushed.push(p);
