        ...
        if (!isBookkeeping(filePath)) {
          ...
          if (hasPlan && !state.plan_written) {
            ...
            sendCodexResponse(server, req.id, {
              decision: 'reject',
              reason: `Write a plan to ${WORKSPACE_AGENT}/plans/ before editing source files.`,
            });
            return;
          }
          if (hasPlan && state.plan_stale) {
            ...
            sendCodexResponse(server, req.id, {
              decision: 'reject',
              reason: `Plan is stale (${state.edits_since_plan} edits since last plan). Write an updated plan to ${WORKSPACE_AGENT}/plans/ before continuing.`,
            });
            return;
          }
  ...
  if (hasPlan || hasCritique) {
    server.notificationHandlers.push((n: JsonRpcNotification) => {
      ...
      if (item.type === 'fileChange' || item.type === 'applyPatch') {
        ...
        if (filePath.includes(`${WORKSPACE_AGENT}/plans/`)) {
