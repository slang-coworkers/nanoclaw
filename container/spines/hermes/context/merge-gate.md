...
**P3 — tester PASS at that SHA, required rows green.** From the original `[Test Report]` message + the tester's canonical report file (evidence step 3): `**Verdict:** PASS`, `head <sha7>` equal to `APPROVED_SHA`, `round <k>` with `k <= 2`. In `## Results`, all of `DOCTOR`, `ACCEPTANCE`, `NEGATIVE-CONTROL` (acceptance *fails* on base), `RUFF`, `FOOTGUNS`, `SUITE T1` … `SUITE T12`, and `UI` (`PASS` or `SMOKE-ONLY`) are green; every non-advisory, non-skipped row is `PASS`, and each `SKIPPED` carries its reason. `DESKTOP` has exactly four green forms. (1) `PASS`. (2) `SKIPPED — install_packages: <pkgs>` — you own that request: action it (`install_packages`) so the next round can run the tier, and say so in the merge notice. (3) `SKIPPED — N/A: no UI paths in diff (<n> files checked)` — green **only if you verify it yourself**: the P4 file list you already hold (`gh pr diff --name-only`) contains no path under `apps/desktop/**`, `apps/shared/**`, `web/**`, `tui_gateway/**` or `hermes_cli/desktop*`, and the ADR carries no `desktop:` criterion. Never take the N/A from the tester's prose. A UI path in the diff under a SKIPPED row is red — blocked reply `P3 — DESKTOP skipped with UI paths in diff: <paths>`; the remedy is a same-head desktop smoke from the tester (no counted round), never a re-spec. (4) `FAIL` marked `ADVISORY` (or `FAIL(pre-existing: <spec>)`) with an upstream citation you can read; an uncited desktop failure is red. Any other wording — `non-blocking`, `deps present`, `not needed`, `reserved for nightly` — is red, whatever the rest of the report says.
...
- **At a cap, decide — do not park** (`delegated-decisions.md`). State the options on the row thread, one line each: a re-spec through hermes-architect, stopping the row (`blocked: STOP cap - <what>`), or one exceptional final round with the reason the next run will differ — that one is the operator's: `DECISION NEEDED` in the same turn (escalation.md), defaulting under C.1 (`round 3 authorized` in the ledger `verdict` cell, never `config.json`). Run `/codex-critique` on the pick, record it in the ledger, proceed. Spend past a ceiling, a scope change or a security call go out in the same `DECISION NEEDED` shape.
