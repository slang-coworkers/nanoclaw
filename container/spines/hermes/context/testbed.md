...
**Trees.**
- `/workspace/extra/hermes-release` — read-only mount of `NousResearch/hermes-agent` at tag `v2026.8.31` (v0.21.0). The ONLY citation source (`file:line` relative to it). Never edit it, never build or test inside it.
...
- `/workspace/agent/wt-<target-slug>` — one git worktree per target: `git worktree add /workspace/agent/wt-<target-slug> -b plugin/<name> v2026.8.31` (base on the fork's default branch instead if it has moved past the tag). All edits, env, tests, and commits happen there. Never read, write, or remove a sibling `wt-*`.
