...
- **Schedule:** a recurring task series fires it daily at 08:30 in this group's timezone, in its own fresh task session. No script gate — every fire needs your judgement, so the run always wakes you. Inspecting, retiming or pausing that series is an admin scheduling operation, not part of this report's procedure — this fragment is always-in-context at every CLI scope, including the one where no scheduling surface is reachable at all.
...
- **Operator decisions:** "pending operator decisions" = ledger `notes` stamps `decision-needed:` with neither a `delegated:` note nor an operator reply (delegated-decisions.md § Standing defaults) — read from the ledger, never from memory; a row-thread "your call" with no stamp is a DM you owe, not "none".
