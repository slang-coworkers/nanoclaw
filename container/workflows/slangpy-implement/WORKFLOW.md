...
**[MUST] Never gate an authorized human-facing reply on a build.** When a maintainer comment/review asks a question, requests a rename/label, or expects a response, and the inbound carried `<github-post-authorized />`, post that reply **on the same turn** — *before* starting any build- or CI-dependent work. A one-liner is fine ("on it — addressing your review; full follow-up once the build lands"), but do **not** batch the reply + edits + push into "one shot after the build": a build that hangs or never returns leaves the maintainer with silence while your session drains its inbound queue and is reaped on `absolute-ceiling` (the slang #11594 failure — an authorized reply + rename sat unposted for days behind a build that never reported back). Answer first, build second; authorization to post is a reason to post **now**, not to wait.
