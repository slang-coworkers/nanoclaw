...
overrides:
  ...
  verify: "Build: delegate to an `Agent` subagent using `/slangpy-build` commands (never run cmake/pip inline). Test: `pytest slangpy/tests -v`. Full suite: `pytest slangpy/tests -v && pytest samples/tests -vra && python tools/ci.py unit-test-cpp`. Format: `pre-commit run --all-files` (re-run if it modifies files). Verify type annotations with pyright if available. Autonomy additions: check prerequisites (libgl-dev etc.) before the first build subagent call — if missing, file one `install_packages` request with ALL missing packages before proceeding. On restart: check if `/workspace/agent/slangpy/` exists — if build artifacts exist, skip Clone and go to Verify; if tests were already passing, go to Ship. If pytest fails after 2 fix cycles, commit `wip:` branch and escalate."
---
