...
back, then hands that doc to `/slangpy-pr-approver`. There is one uniform model:
...
- Message carries a **PR list but no manifest** → generate the manifest first:
  ...
  slangpy-pr-approver skill). It resolves each PR's reviewed commits with
...
- The `MODE=pr-approve` line tells `{{vars.reviewer}}`'s workflow this is an
  ...
  `/slangpy-pr-review` step 5). Keep the `MODE=`/`REPO=`/`PR=`/`COMMIT=` lines
  byte-exact (`grep -oE`).
...
`dashboard:slangpy-pr-approver` (the per-decision `[Approval Decision]`
