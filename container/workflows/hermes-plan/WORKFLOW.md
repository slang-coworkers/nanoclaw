...
overrides:
  ...
  deliver: |
    **Deliver** — Write both artifacts, then critique them before anything leaves this session.
    ...
    3. `/codex-critique` with `STAGE: PLAN_REVIEW` — ARTIFACTS = both file paths (codex reads them itself; the `### Attested` hashes bind the review to these exact files). Must-fix → revise both files, re-run PLAN_REVIEW (max 2 rounds; then escalate to parent with the unresolved item rather than shipping a disputed spec).
    4. `/codex-critique` with `STAGE: OUTPUT_REVIEW` on the final pair plus the handoff text you are about to send. Any edit to either file after this round invalidates the attested hashes — re-run OUTPUT_REVIEW before Handoff.
