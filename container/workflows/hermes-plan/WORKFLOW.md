...
overrides:
  ...
  diagnose: |
    ...
    **ADR skeleton (mode = plan).** Draft with exactly these sections, in order:
    ...
    4. `## Acceptance criteria` — **required; an ADR without it is not handoff-ready.** A markdown table with this header, character for character:
       ...
       |---|---|---|
       | AC-<req-id>-1 | <one observable behaviour, testable as written> | pytest: tests/plugins/test_<plugin>_acceptance.py::test_ac_<req_id>_1 — <what it asserts> |
       | AC-<req-id>-2 | <what the operator sees in the dashboard> | ui: tests/e2e-scenarios/<req-id>/AC-<req-id>-2.md — <n> steps against the stub model, outline in §Scenario outlines |
       ...
       | AC-<req-id>-4 | <bot A delegates to bot B and the reply is visible> | live: tests/e2e-scenarios/<req-id>/AC-<req-id>-4.md — model: live via OneCLI, outline in §Scenario outlines |
       ```

       One row per **testable** criterion, in a fixed order, ids `AC-<req-id>-<n>` with `<n>` 1-based and contiguous from 1 (`AC-FR-3-1`, `AC-LOOP-F35-2`). Split the `## Requirement` sentence into the behaviours a test can assert separately: one observable behaviour per row, never two joined by "and", never `works correctly` / `no regressions` / `is documented` (unfalsifiable rows are the failure this table exists to prevent — a doc requirement becomes a criterion only when a test can read the rendered page). Derive a criterion the requirement row only implies, and mark it `(derived)` in the criterion cell.

       **Every `how it will be verified` cell opens with a kind prefix — `pytest:` | `ui:` | `desktop:` | `live:` — followed by the artifact that proves it** (the pytest node id `test_ac_<req_id>_<n>`, the id lowercased with `-`→`_`, for `pytest:`; the scenario file for `ui:` and `live:`; the Playwright spec for `desktop:`), plus one clause naming what it proves. An unprefixed cell reads as `pytest:`, the historical default. Kinds are per criterion and one requirement may carry all four; three rules decide the mix:

       - The behaviour is visible to a user in the dashboard or the desktop app → at least one `ui:` or `desktop:` criterion. A behaviour nobody can be shown is not the one the requirement asked for.
       - Two agents talk (a2a, delegation, peer wiring, topology) → at least one `live:` criterion. A stub cannot prove that a real model answered a real peer. Mint at most **2** `live:` criteria per requirement; a third agent-to-agent behaviour becomes another step inside an existing live scenario, not a new criterion.
       - Every requirement keeps at least one `pytest:` criterion whatever else it carries: the hermetic floor, re-runnable on any head with no browser and no budget.

       A criterion this ADR does not intend to test yet still gets a row, with `how it will be verified` = `no test — criterion unimplemented`, and the builder must close it before the tester can report PASS.
    ...
    5. `## Scenario outlines` — **required whenever any criterion is not `pytest:`; write the section either way.** One `###` block per `ui:`/`desktop:`/`live:` criterion, in criterion order, which the builder turns into the scenario file or the Playwright spec as written (it invents no steps, and the tester never edits them):
       ...
       - **Fixtures:** fixtures/<req-id-lowercase>-<role>/ (a profile dir with `distribution.yaml` at its root, the shape the testbed installs; the tester installs the `fixtures:` list itself, so `## Setup` never re-installs one)
       - **Setup:** what must be true after the fixtures are installed and before step 1 (roster / `ui_meta` writes, peer start-up, seeding) — not a `hermes profile install` of a listed fixture
       - **Steps:**
       ...
       - **Evidence:** `step-<n>.png` per step, plus a `python3 -c` query over `$HERMES_HOME/profiles/<bot>/state.db` where the fact is a row rather than a pixel (the coworker image has no `sqlite3` CLI).
       ```

       `timeout_s` is the ceiling for ONE step's bounded wait, not for the whole scenario: default `300` for `ui`, `600` for `live`. A `desktop:` block keeps the same fields with `Ships as:` = the spec path and `Fixtures:` = the upstream `apps/desktop/e2e` fixtures. A `live:` block reads `(live, model: live, timeout_s: <s>)`, names both profiles that must exist before step 1, and names the `base_url` and the model id the live provider block carries — the tester pays real model calls for it and may not invent an endpoint. No `ui:`/`desktop:`/`live:` row → still write the section, with the single line `none (every criterion is pytest)`, so an absent section always means an unfinished ADR rather than a hermetic one.
    6. `## Plugin surface` — chosen from the plugin docs table (`{{vars.release_tree}}/website/docs/developer-guide/plugins/index.md`) and pinned to code: the hook name(s) from `VALID_HOOKS` and/or the `PluginContext.register_*` method(s), the manifest `kind` if any, cited as `{{vars.release_tree}}/hermes_cli/plugins.py:<line>`. State the plugin shape: directory plugin (`plugin.yaml` with `manifest_version: 1`, `name`, `version`, `description`, `provides_tools`, `provides_hooks` + `__init__.py` exposing `register(ctx)`) or portable Agent Plugins v1 (`plugin.json` + `skills/<name>/SKILL.md` [+ `mcp.json`]). Give the plugin key (`<name>` flat under `plugins/`, or `<category>/<name>`) and note that it must be listed in `plugins.enabled` to load.
    7. `## Design` — files under `plugins/<name>/` (or `plugins/<category>/<name>/`), config keys under `plugins.entries.<id>.settings` (never a new `HERMES_*` env var for non-secret config), durable state via `plugins.plugin_storage.plugin_data_dir(<name>)`, paths via `get_hermes_home()`, cross-platform notes.
    8. `## Alternatives` — at least two, each with the footprint-ladder rung (extend existing → CLI + skill → `check_fn` tool → plugin → MCP → core tool) and the constraint that ruled it out. If only one is viable, name the constraint — that is load-bearing.
    9. `## CORE-CHANGE` — default text: `none — all edits under plugins/**, website/docs/**, tests/**, apps/desktop/e2e/**`. If ANY edit outside those four trees is required, this section must cite the exact `{{vars.release_tree}}/<file>:<line>` that blocks a plugin and describe the minimal GENERIC widening (a new hook, a new `ctx` method, a new config key) — never a special case for this plugin in core. Without that citation the builder must refuse the core edit and the reviewer must-changes the PR.
    10. `## DeepWiki disagreements` — each DeepWiki claim that the tree contradicted, with the tree's `file:line`; `none` if none.
    11. `## Acceptance test` — target path `tests/plugins/test_<plugin>_acceptance.py`, **one test function per `pytest:` `## Acceptance criteria` id** named `test_ac_<req_id>_<n>` (the id lowercased, `-`→`_`) with the criterion text as the docstring's first line, what each proves, and why the file must FAIL on the stock `{{vars.release_tag}}` tree. That naming is the whole mechanism by which a criterion id resolves to a pytest node id without anyone reading prose — keep it mechanical. A `ui:`, `desktop:` or `live:` id gets no test function here: its artifact is the scenario file or the spec named in `## Scenario outlines`.
    12. `## References` — release-tree `file:line` list; `gh` URLs for prior art.

    **Acceptance-test spec (mode = plan).** Mirror `{{vars.release_tree}}/tests/hermes_cli/test_plugin_api_compat.py` exactly in shape: `tmp_path`-rooted `hermes_home`; copy the plugin directory into `<hermes_home>/plugins/<key>`; write `<hermes_home>/config.yaml` with `plugins: {enabled: [<key>]}`; create an EMPTY bundled dir and `monkeypatch.setenv` `HOME`, `HERMES_HOME`, `HERMES_BUNDLED_PLUGINS`; `manager = PluginManager(); manager.discover_and_load()`; `loaded = manager._plugins[<key>]`; assert `loaded.enabled is True`, `loaded.error is None`, `loaded.module is not None`, and the declared hooks/tools are in `loaded.hooks_registered` / registered tools; then DRIVE the behaviour (`manager.invoke_hook(<hook>, ..., future_additive_field=...)`, or the tool handler / `ctx.dispatch_tool`) and assert the OUTCOME from the acceptance criterion. **One `def test_ac_<req_id>_<n>(...)` per `pytest:` `## Acceptance criteria` row, same order, docstring line 1 = the criterion text**: each asserts its own criterion's observable behaviour and nothing else — a test that only imports the plugin, asserts `True`, or re-proves that discovery loaded the key is a row the reviewer must-changes, because it is the shape that lets an unmet criterion read as PASS. Shared setup goes in a fixture, not in a criterion test. Rules: behavior contract, not a snapshot — no model lists, version literals, enumeration counts; never read source files in the test; no network; nothing written under `~/.hermes`; hook callbacks under test accept `**kwargs`; tool handlers return a JSON string. The test must fail on the stock tree (plugin absent → `KeyError`/not enabled) and pass once the plugin exists — that asymmetry is what makes it an acceptance test.
  ...
  deliver: |
    **Deliver** — Write both artifacts, then critique them before anything leaves this session.
    ...
    3. **Handoff-readiness check, before the critique:** `grep -n '^## Acceptance criteria' /workspace/agent/reports/{{target_slug}}.md` and read the table back — it must exist, carry contiguous `AC-<req-id>-<n>` ids from 1, and every cell must open with a kind prefix. Then check both halves mechanically:
       ```bash
       ADR=/workspace/agent/reports/{{target_slug}}.md
       grep -c '^| AC-<req-id>-' "$ADR"                                            # = number of criteria
       grep -cE '^\| AC-<req-id>-[0-9]+ \|.*\| *pytest:' "$ADR"                    # the hermetic floor: 1 or more
       grep -c 'def test_ac_' /workspace/agent/reports/{{target_slug}}-acceptance_test.py   # = the number of pytest: rows
       ...
       ```
       `grep -c 'def test_ac_'` equals the number of rows whose cell opens `pytest:` — **not** the total row count, since a `ui:`/`desktop:`/`live:` id carries no test function — and every `ui:`/`desktop:`/`live:` id has a `### <id>` block under `## Scenario outlines`. A missing or short table, or one `NOT HANDOFF-READY` line, is not a nit to fix later: it is the single input `hermes-tester` and `hermes-reviewer` fail closed on, so fix it here rather than shipping the ADR.
...
- **The ADR mints the acceptance-criterion ids and is the only place they are minted.** `## Acceptance criteria` (header `| id | criterion | how it will be verified |`) carries one row per testable behaviour with a contiguous `AC-<req-id>-<n>` id and a kind prefix (`pytest:` | `ui:` | `desktop:` | `live:`) on every `how it will be verified` cell; the acceptance test carries one `def test_ac_<req_id>_<n>` per `pytest:` row, and `## Scenario outlines` carries one `###` block per row of the other three kinds. `hermes-tester` emits one `## Results` row per id, `hermes-reviewer` cross-walks one row per id, and the Orchestrator's merge gate joins the two on those ids — so an ADR without the table cannot be verified, cannot be reviewed and cannot be merged, and no downstream role is allowed to invent the missing ids. Never renumber or reuse an id across rounds.
- Every capability is a Hermes plugin. Any edit outside `plugins/**`, `website/docs/**`, `tests/**`, `apps/desktop/e2e/**` needs the ADR's `## CORE-CHANGE` section citing the blocking `{{vars.release_tree}}/<file>:<line>`; the reviewer must-changes a PR without it.
