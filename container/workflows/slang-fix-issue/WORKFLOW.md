...
7. **Push + draft PR** {#draft-pr} — Once verify is green:
   ...
   **[MUST] Never gate an authorized human-facing reply on a build.** When a maintainer comment/review asks a question, requests a rename/label, or otherwise expects a response, and the inbound carried `<github-post-authorized />`, post that reply **on the same turn** — *before* you start any build- or CI-dependent work. A one-liner is fine ("on it — re-stacking per your review; will follow up with the full reply once the build lands"), but do **not** batch the reply + rename + code push into "one shot after the build." A build that hangs or never returns then leaves the maintainer with silence while your session drains its inbound queue and is reaped on `absolute-ceiling` — exactly the #11594 failure (an authorized consolidated reply + `[1/4]` rename sat unposted for days behind a build that never reported back, while the maintainer asked "why no progress?" daily). Answer first, build second; authorization to post is a reason to post **now**, not to wait.
