...
# /discord-answer — Answer a Discord Support Question

Use this workflow when processing summon requests from `summon_requests.jsonl` or when asked to answer a Discord question.

## Step 0: ENSURE LOCAL REPO

Check if the project source is available for local code exploration:
...
Read the Discord thread to understand the user's question:
...
- Whether another user already answered (if yes, skip to Step 6)

## Step 2: RESEARCH via DeepWiki (MANDATORY)

Query DeepWiki for relevant Slang documentation. This step is NOT optional — even if you think you know the answer, verify it.
...
## Step 3: RESEARCH via GitHub (MANDATORY)

Search for related issues, PRs, or discussions that provide additional context:
...
Compose a clear, helpful answer that:
...
Send the draft to the orchestrator for review. This step is NOT optional.

```
send_message(text="[Draft] Thread: <thread_name> (ID: <thread_id>)\n\nQ: <one-line question summary>\n\nA:\n<your drafted answer>\n\nSources:\n- <deepwiki finding>\n- <github issue link>\n- <source file path>")
```

## Step 6: SAVE and mark handled

Save the draft to disk and mark the summon as handled:
...
3. For each unhandled request, run Steps 1-6 **SEQUENTIALLY — one thread at a time**

**IMPORTANT: Max 2 parallel MCP calls at any time.** Do NOT use subagents (Agent tool) to parallelize thread processing. Do NOT fire multiple DeepWiki + GitHub calls simultaneously. Process one thread fully (Steps 0-6) before starting the next. This prevents rate limiting and response stream hangs.
