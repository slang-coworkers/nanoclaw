...
When both subprocesses complete (or just A if B was skipped), call the skill's summarizer for each run-directory. The skill returns severity counts, per-subagent cost, and a drift signal (GitHub-write attempts must be 0 — non-zero means the read-only allowlist leaked).

## Step 4: REPORT {#report}

Output goes to the caller — never to GitHub.
