...
overrides:
  read: |
    **Read the issue** — There is no GitHub issue: the input is one **requirement row** from the shared Hermes requirements matrix, dispatched by the orchestrator. Do not run `gh issue view`.
    ...
    - **id** — the requirement row's own id, verbatim: a gap-matrix id (`LOOP-F35`, `GOV-F22`, `P0-LOOP`) or a classic `FR-x` (functional) | `SR-x` (security/safety) | `PR-x` (port/parity) | `NF-x` (non-functional). Never re-mint or normalise it — it is the thread (`hermes-<req-id>`), the scenario dir (`tests/e2e-scenarios/<req-id>/`), the spec name (`<req-id-lowercase>-ac<n>.spec.ts`) and the `AC-` id stem all at once, so a rename here silently breaks four joins downstream. Wherever an inherited step says `<number>` or `{{vars.repo}}#<number>`, write this id (`FR-3`, not `{{vars.repo}}#FR-3`). The memo path is `/workspace/agent/memory/triage-<req-id>.md`.
    - **title** and **requirement text verbatim**; the **acceptance criteria** — the list of separately testable observable behaviours the row demands (derive the ones it only implies and label each derived). One sentence is the summary that goes in the ADR's `## Requirement`; the list is what becomes the ADR's `## Acceptance criteria` table with ids `AC-<req-id>-<n>`. Split on behaviour, not on grammar: two things a test asserts separately are two criteria, and `works correctly` / `no regressions` is not a criterion at all. For each behaviour also record **how a person would see it happen**: in the web dashboard, in the desktop app, only in the CLI or on disk, or as two agents talking to each other. That reading is what fixes the criterion's verification kind (`pytest:` / `ui:` / `desktop:` / `live:`) in the classify step, so take it now while you have the row in front of you.
  ...
  research: |
    **Research** — Fan out via subagents; cost is your context, not wall clock. Three pillars, in authority order:
    ...
    0. **Recall — wiki first, before any design.** Spawn one `Agent`: "Check if `/workspace/shared/wiki/index.md` exists. IF YES read it, open at most 2 concept pages about <the gap-matrix row's name and the Hermes subsystems it touches: hooks, profiles, gateway, plugins, testbed> with `limit=60`. IF NO `wiki/`: Grep `/workspace/shared/learnings/` for the same keywords, at most 3 hits. Return at most 5 bullets (title, one line, path) or `no prior hits`." Put the hits in the ADR's `## Context` (a prior finding that shaped a criterion is cited by path) and never re-derive a known testbed gap, environment cause or false-green pattern. Never read `learnings/INDEX.md` inline.
  ...
  classify: |
    ...
    **Classification (exactly one):**
    ...
    |---|---|---|
    | **PORT-as-plugin** (default) | Met entirely by a Hermes plugin — directory plugin (`plugin.yaml` `manifest_version: 1` + `__init__.py register(ctx)`) or portable Agent Plugins v1 (`plugin.json` package) — touching only `plugins/**`, `website/docs/**`, `tests/**`, `apps/desktop/e2e/**` (the last only for a `desktop:` acceptance spec). | The plugin surface (hook(s) from `VALID_HOOKS` / `register_*` method / `kind`) with its `{{vars.release_tree}}/hermes_cli/plugins.py:<line>`; the plugin key (`<name>` or `<category>/<name>`); the bundled plugin used as template. |
    ...
    **[MUST] Handoff-readiness: the ADR carries `## Acceptance criteria`, plus `## Scenario outlines` for every criterion that is not `pytest:`.** Before this requirement leaves your session both sections exist in the ADR. `## Acceptance criteria` uses the header `| id | criterion | how it will be verified |` verbatim and holds one row per testable criterion with a contiguous `AC-<req-id>-<n>` id from 1 — the shape hermes-plan's Synthesize step specifies. What is new is that every `how it will be verified` cell opens with a **kind prefix**, and the prefix is what tells the builder which artifact to commit and the tester which tier to run the criterion in:

    | kind | verified by | artifact the builder commits in the draft PR |
    |---|---|---|
    ...
    - The requirement's behaviour is visible to a user in the dashboard or the desktop app → at least one `ui:` or `desktop:` criterion. A behaviour nobody can be shown is not the one the requirement asked for.
    - The requirement has two agents talking (a2a, delegation, peer wiring, topology) → at least one `live:` criterion. A stub cannot prove that a real model answered a real peer. Per the fleet baseline (`topology.md`) the default live shape is Bot Chat inside ONE gateway — bot A `message_agent`s bot B and B's reply is visible in the room; two gateways over a2a only when the row is itself about cross-gateway or cross-tenant peering. **Mint at most 2 `live:` criteria per requirement**: a third agent-to-agent behaviour is folded into an existing live scenario as another step, not a new criterion — the tester pays real model calls per scenario and the round carries its own ceiling.
    - Every requirement keeps at least one `pytest:` criterion whatever else it carries: that row is the hermetic floor, the one the tester can re-run on any head with no browser and no budget.

    Rows, all four kinds (header character for character, one clause after the artifact naming what it proves):

    ```markdown
    | id | criterion | how it will be verified |
    |---|---|---|
    | AC-<req-id>-1 | <one observable behaviour, testable as written> | pytest: tests/plugins/test_<plugin>_acceptance.py::test_ac_<req_id>_1; asserts <…> |
    | AC-<req-id>-2 | <what the operator sees in the dashboard> | ui: tests/e2e-scenarios/<req-id>/AC-<req-id>-2.md; <n> steps against the stub model, outline below |
    | AC-<req-id>-3 | <what the desktop app renders> | desktop: apps/desktop/e2e/<req-id-lowercase>-ac3.spec.ts; upstream e2e fixtures + mock inference server |
    | AC-<req-id>-4 | <bot A delegates to bot B and the reply is visible> | live: tests/e2e-scenarios/<req-id>/AC-<req-id>-4.md; model: live via OneCLI, outline below |
    ```

    **`## Scenario outlines` is the ADR section that makes those kinds buildable.** In the hermes-plan skeleton it sits immediately after `## Acceptance criteria` and before `## Plugin surface` (skeleton section 5; `## Plugin surface` and everything below shift down by one). It carries one `###` block per `ui:`/`desktop:`/`live:` criterion, and the builder turns each block into the scenario file or the Playwright spec as written: it does not invent steps, and the tester never edits them.

    ```markdown
    ## Scenario outlines

    ### AC-<req-id>-2 (ui, model: stub, timeout_s: 300)
    - **Ships as:** tests/e2e-scenarios/<req-id>/AC-<req-id>-2.md
    - **Fixtures:** fixtures/<req-id-lowercase>-<role>/ (a profile dir with `distribution.yaml` at its root, the shape the testbed installs; the dir basename is also the `--name`, so `p0-loop-bot-a` never collides with a T-suite bot in the shared testbed home)
    - **Setup:** what must hold after those fixtures are installed and before step 1 (the `ui_meta.hermes-bots` write, peer start-up, seeding). The tester installs the `fixtures:` list itself, so a `## Setup` line that re-installs one is a scenario defect, not a duplicate the tester silently drops.
    - **Steps:**
      1. <UI action naming the visible label to click or the field to fill> → expect: <one observable fact>
      2. <…one step per screenshot the tester will take…>
    - **Pass:** the criterion holds when <one sentence>.
    - **Evidence:** `step-<n>.png` per step, plus a `python3 -c` query over `$HERMES_HOME/profiles/<bot-b>/state.db` where the fact is a row rather than a pixel (the coworker image ships no `sqlite3` CLI, so name the stdlib form).
    ```

    `timeout_s` is the ceiling for ONE step's bounded wait, not the whole scenario (the tester's testbed skill caps the scenario separately): default `300` for `ui`, `600` for `live`. A `desktop:` block keeps the same fields with `Ships as:` = the spec path and `Fixtures:` = the upstream `apps/desktop/e2e` fixtures instead of profile dirs. A `live:` block reads `(live, model: live, timeout_s: <s>)`, names both profiles that must exist before step 1, and names the `base_url` and the model id the live provider block carries — the tester pays real model calls for it and is forbidden from inventing an endpoint or reading one out of an unrelated env var. No `ui:`/`desktop:`/`live:` row → still write the section, with the single line `none (every criterion is pytest)`, so an absent section always means an unfinished ADR rather than a hermetic one. Check all of it, don't assume it:

    ```bash
    ADR=/workspace/agent/reports/<slug>.md
    grep -n '^## Acceptance criteria' "$ADR"
    grep -c '^| AC-<req-id>-' "$ADR"                                            # = number of criteria
    grep -cE '^\| AC-<req-id>-[0-9]+ \|.*\| *pytest:' "$ADR"                    # the hermetic floor: 1 or more
    grep -c 'def test_ac_' /workspace/agent/reports/<slug>-acceptance_test.py   # = the number of pytest: rows
    grep -n '^## Scenario outlines' "$ADR"                                      # required unless every row is pytest:
    for id in $(grep -oE '^\| AC-<req-id>-[0-9]+ \|[^|]*\| *(ui|desktop|live):' "$ADR" | grep -oE 'AC-<req-id>-[0-9]+'); do
      grep -q "^### $id" "$ADR" || echo "NOT HANDOFF-READY: $id has no outline"
    done
    ```

    Missing table, short table, a requirement that breaks one of the three rules, or one `NOT HANDOFF-READY` line → go back to hermes-plan (mode = plan) and finish the sections before the report step; never hand off an ADR without them and never let the builder or the tester "fill it in". A `ui:`/`desktop:`/`live:` id whose artifact is absent at the PR head is a `FAIL` row for the tester exactly like a missing pytest function, so an outline you skip surfaces as the builder's failure two hops downstream. You are the ONLY role in this chain permitted to mint an `AC-` id: `hermes-tester` emits one `## Results` row per id and fails its whole report when the ADR has none, `hermes-reviewer` cross-walks one row per id, and the Orchestrator's merge gate joins the two on those ids — so an ADR without the table cannot be verified, reviewed or merged, and the requirement stalls with nobody able to unblock it. Ids are stable for the life of the requirement: never renumbered, never reused, a withdrawn criterion keeps its id. Then write the memo via heredoc (the file is new; `Write` requires Read-first):
    ...
    - Acceptance criteria (ADR ## Acceptance criteria — ids are stable, never renumbered; each row carries a kind prefix):
      - AC-<req-id>-1 — <criterion verbatim> → pytest: tests/plugins/test_<plugin>_acceptance.py::test_ac_<req_id>_1
      - AC-<req-id>-2 — <criterion verbatim> → ui: tests/e2e-scenarios/<req-id>/AC-<req-id>-2.md  (kinds: pytest | ui | desktop | live; live is mandatory for agent-to-agent)
    - Scenario outlines (ADR ## Scenario outlines): one per ui/desktop/live id | none (every criterion is pytest)
  ...
  report: |
    **Report up to parent** — This is the gated **`[Spec handoff]`**: the architect's terminal report UP to the orchestrator and the ONLY send on a requirement that carries the `Spec handoff` marker. It goes out BEFORE the delegation to {{vars.fixer}} — the builder receives nothing until this send has been accepted, so the spec passes DIAGNOSIS_REVIEW / PLAN_REVIEW / OUTPUT_REVIEW before anyone builds on it.
    ...
    2. **Send `[Spec handoff]` as a reply on the orchestrator edge** — `in_reply_to=<orchestrator-inbound-id>` (the requirement dispatch; the always-on chain-routing gate refuses a marker-prefixed send without it) — and attach the artifacts with the same `in_reply_to`:
       ```
       send_message(to="parent", in_reply_to=<orchestrator-inbound-id>, text="[Spec handoff] <req-id>: <title>\n- **Classification:** <PORT-as-plugin | CORE-CHANGE | not-needed> / <severity-priority> / <component> / duplicate: <id|none>\n- **Plugin surface:** <hook/register_*/kind> — {{vars.release_tree}}/hermes_cli/plugins.py:<line>; key <name|category/name>; template plugins/<…>/\n- **CORE-CHANGE:** none | {{vars.release_tree}}/<file>:<line> — <blocking reason> → <generic widening>\n- **Acceptance criteria:** <n> ids — AC-<req-id>-1 … AC-<req-id>-<n> (ADR §Acceptance criteria; kinds pytest <a> / ui <b> / desktop <c> / live <d>, one test_ac_<req_id>_<n> per pytest id) — the join key the tester rows, the reviewer cross-walks and your merge gate reads\n- **Scenario outlines:** <n> in ADR §Scenario outlines, one per ui/desktop/live id | none (every criterion is pytest)\n- **ADR / acceptance test:** attached — /workspace/agent/reports/<slug>.md, /workspace/agent/reports/<slug>-acceptance_test.py (not-needed: proof <file:line> + smoke check instead)\n- **Routing:** forwarding to {{vars.fixer}} on thread hermes-<req-id> | not-needed — nothing to build")
  ...
  forward: |
    **Forward to {{vars.fixer}} — always** — Runs ONLY after the gated `[Spec handoff]` above was accepted (the send returned without a gate denial). This is the UNMARKED fresh delegation DOWN: {{vars.fixer}} has never written to you on this requirement, so there is no inbound to reply to, and the always-on chain-routing gate denies a marker-prefixed send without `in_reply_to`. The first line must not begin with `[`; explicit `thread_id` = the canonical requirement thread you were dispatched on, propagated unchanged:
    ...
    ```
    send_message(to="{{vars.fixer}}", thread_id="hermes-<req-id>", text="Spec handoff <req-id>: <title>\nPriority: <pri> | Component: <comp>\n- **Requirement:** <one-line acceptance criterion>\n- **Plugin surface:** <hook/register_* method; plugin key>\n- **CORE-CHANGE:** none | {{vars.release_tree}}/<file>:<line>\n- **Acceptance criteria:** AC-<req-id>-1 … AC-<req-id>-<n> — ADR §Acceptance criteria; every row prefixed pytest: | ui: | desktop: | live:, one test_ac_<req_id>_<n> per pytest id. hermes-tester emits one Results row per id and FAILs the report if one is missing; do not renumber or add ids — ask me\n- **Scenario outlines:** ADR §Scenario outlines, one per ui/desktop/live id; commit each as tests/e2e-scenarios/<req-id>/AC-<req-id>-<n>.md (desktop: apps/desktop/e2e/<req-id-lowercase>-ac<n>.spec.ts) with its fixtures in the SAME draft PR. A scenario or spec missing at the PR head is a FAIL for that id\n- **Acceptance test:** attached — copy to tests/plugins/test_<plugin>_acceptance.py; must fail before, pass after\n- **ADR + memo:** attached — alternatives, repro, risks in the memo")
...
- **Acceptance-criterion ids are minted here and nowhere else.** The ADR's `## Acceptance criteria` table (header `| id | criterion | how it will be verified |`, one row per testable behaviour, contiguous `AC-<req-id>-<n>` from 1, every `how it will be verified` cell prefixed `pytest:` | `ui:` | `desktop:` | `live:`, one `test_ac_<req_id>_<n>` node id per `pytest:` row) is a **handoff-readiness condition**: no table → no `[Spec handoff]`, no forward to `{{vars.fixer}}`. `hermes-tester` fails its whole report on an ADR without it, `hermes-reviewer` must-changes the PR, and the Orchestrator's merge gate joins verdict to report on those ids — none of them may invent one. Ids never move: not renumbered between rounds, not reused, a withdrawn criterion keeps its id.
- **Kinds and outlines ride with the ids.** A requirement whose behaviour a user sees carries at least one `ui:` or `desktop:` criterion, one where two agents talk (a2a, delegation, peer wiring, topology) carries at least one `live:` criterion, and every requirement keeps at least one `pytest:` criterion as the hermetic floor. Each `ui:`/`desktop:`/`live:` criterion also gets a `###` block in the ADR's `## Scenario outlines` (setup, numbered steps, pass sentence, fixtures needed), which the builder turns into `tests/e2e-scenarios/<req-id>/AC-<req-id>-<n>.md` or an `apps/desktop/e2e/` Playwright spec and commits in the same draft PR. That section sits directly after `## Acceptance criteria` in the hermes-plan skeleton, and an ADR carrying such a criterion with no outline is not handoff-ready, exactly like a missing table.
