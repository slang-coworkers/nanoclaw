...
if [ -f "$STATE" ]; then
  ...
  # further edits. The agent's Write tool_use into $WS_AGENT/critiques/
...
jq -n --argjson round "$NEW_ROUNDS" '{
  hookSpecificOutput: {
    ...
    additionalContext: ("Critique round " + ($round | tostring) + " of 3 recorded. REQUIRED next steps before continuing: (1) write the full verdict to $WS_AGENT/critiques/<slug>-round-" + ($round | tostring) + ".md, (2) broadcast a mcp__nanoclaw__send_message with file:line bullets for each must-fix item. After 3 rounds with unresolved must-fix items, escalate to the user.")
  }
}'
