...
**[MUST] Always specify the destination — `send_message`/`send_file` without `to=` FAILS for the supervisor.** The orchestrator/supervisor has **multiple destinations**, so a bare `send_message`/`send_file` (no `to=`) errors with *"You have multiple destinations — specify 'to'"* and the board is **silently never delivered** (observed: a full reconcile completed, wrote the on-disk tracker, then the final `send_file` failed on the missing `to=` and the turn ended with nothing on the dashboard). The board is the **primary deliverable** (the on-disk tracker is only a backup), so this MUST not fail:
- Resolve your operator/report channel destination once at the start (`ncl destinations list` — for the orchestrator it is the `orchestrator` channel destination; for a delegated supervisor it is `parent`). Call it `<report-dest>`.
- **Post the inline table with `send_message(to="<report-dest>", text=…)`** — `to=` is REQUIRED, never omit it.
- If you also attach the full tracker, use `send_file(to="<report-dest>", path="reports/issue-chain-tracker.md", text=…)` — again `to=` REQUIRED.
- The inline `send_message` table is the deliverable that must land; `send_file` is optional/secondary. If `send_file` errors, the inline table must already have been posted — never let an attachment failure swallow the board.
...
- **Don't call `send_message`/`send_file` without `to=`.** The supervisor has multiple destinations; a bare call errors with *"specify 'to'"* and the board is silently never delivered. Always pass `to="<report-dest>"`, post the inline table before any attachment, and never let a `send_file` failure swallow the board. (§6)
