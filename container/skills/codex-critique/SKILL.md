...
Everything else (the plan, the diff, source code, invariants, reports, project conventions) — give codex paths and let it `Read` them itself. The whole `/workspace/agent` tree is mounted read-only into codex's process, including the project source, `plans/`, `reports/`, `critiques/`, and any project-specific invariant docs (`<project>/AGENTS.md`, `<project>/CONTRIBUTING.md`, etc.).
...
2. **Call `mcp__codex__codex` directly.** Pass:
   ...
   - `cwd`: `"/workspace/agent"` — the workspace root. Codex can navigate to any cloned project directories, `plans/`, `reports/`, etc. from here.
...
- <project>/AGENTS.md or <project>/CONTRIBUTING.md  (the project's non-negotiable invariants)
- <project>/PLAN.md or <project>/ROADMAP.md  (current project plan, if present)
- Any *.md under <project>/invariants/ if present
