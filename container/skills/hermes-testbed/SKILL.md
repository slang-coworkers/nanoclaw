...
export HERMES_TESTBED_LIVE_HOST=inference-api.nvidia.com   # the ONE live host — the coworkers' own gateway (hermes-common.vars.live_inference_host); the scenario's provider block must use `api: https://inference-api.nvidia.com` LITERALLY (never `${ANTHROPIC_BASE_URL}` — inside a traced container that is the loopback claude-trace proxy), api_mode anthropic_messages, model aws/anthropic/bedrock-claude-opus-4-8 (no `[1m]` — that suffix is a Claude Code convention; sent verbatim it is an unknown model and the gateway answers 403 key_model_access_denied, P0-LOOP round 2), key_env ANTHROPIC_API_KEY with a dummy value. Any other host has no OneCLI credential and 401s (P0-LOOP round 1).
...
- **Verdict:** PASS | FAIL | FAIL (env) — <cause, outside plugin code> | FAIL ×2 — ESCALATE
...
- **Round cap:** 2 in-plugin FAILs per review cycle (a `[Review Verdict] REQUEST_CHANGES` starts a new cycle and the counter restarts), then ESCALATE. A `FAIL (env)` — every failing row caused outside `plugins/**` (harness, core, provider environment, network) — is reported as FAIL for the builder to read but never counts, like `ESCALATE`. Nightly: once per day against the fork's default branch, one report, no rounds.
