...
| `refresh-viewers.sh` | Publishes coworker artifacts under the 8091 root: `/explanations/` (every group's `reports/pr-explanations/` plus a newest-first index), `/status/latest.html` (the Orchestrator's daily report), `/test-reports/` (the tester's `test-report-<sha7>.md` and scenario screenshots); then runs `autopilot/collect-acks.sh` (`>> logs/collect-acks.log`, never fatal). | Cron `*/15 * * * *` as `~/.config/nanoclaw/refresh-nemo-explanations.sh`. Idempotent. |
| `autopilot/collect-acks.sh` | Host-side, hostname-guarded (exit 9 elsewhere): the newest `processing_ack` row of every active `hermes-<ROW>` session — role + thread from the central DB (`scripts/q.ts`, `./bin/ncl` fallback), one `bun:sqlite` readonly pass over their `outbound.db` files — written atomically to `data/shared/hermes/autopilot/acks.json`. Feeds the supervise tick's bounce and idle-turn detection (`hermes_supervise.py --acks`, autopilot.md §2.5); a copy older than 2 h is ignored there. Writes nothing when no session source answers. | Every 15 min from `refresh-viewers.sh`. |
| `slack-rows.py` | Mirrors every Hermes row into one Slack thread in `#hermes-port` (`C0C14PWDUMC`): a root message per row (`<ROW> · <plan name> · batch <b> · dispatched <date>`), every role card PNG as a threaded upload (`files.getUploadURLExternal` → bytes → `files.completeUploadExternal`, caption `<ROW> · <role> · <OUTCOME> — <headline>`), then one `✅ merged <sha>` / `⛔ blocked` line from the ledger (`blocked: STOP …` and a red gate `blocked: P<n> — …` both post ⛔). Reads the same cards (`rows-board.scan_cards`), plan and ledger as the rows board; remembers what it posted in `data/shared/hermes/slack-threads.json`; `--max-posts` (12) per run; `--dry-run`. Bot token from `$SLACK_BOT_TOKEN` or the checkout's `.env`, host-side only, never printed; mrkdwn-escapes everything agent-written. One run at a time (flock on the state file); cards younger than 60 s or without a PNG `IEND` tail wait for the next run. Exit 2 = bot not invited to the channel; 1 = no token / token-or-channel error / corrupt state. Tests: `python3 -m unittest ops/nemoclaw-coworkers/test_slack_rows.py`. | Called by `refresh-viewers.sh` after the rows board (`>> logs/slack-rows.log`, never fatal). Idempotent. |
| `demo_path.py` + `demo-path.json` | The demo-path tracker: five rungs (R1 governed fleet … R5 OpenShell/APF), the matrix rows each needs with their live state from `data/shared/hermes/autopilot/state.json` + `config.json`, a status per rung (done / in progress / blocked by gate / not started / unknown) and an ETA from a deterministic model (planning hours BUILD 48 h · CONFIGURE 15 h · ADOPT 10 h × stage factor, list-scheduled in `hermes_queue.dispatch_order` on `config.wip` slots, one BUILD at a time, gated rows after their gate's rows, paused/blocked rows excluded and flagged, manual steps after their prerequisites). Rendered at the top of `/rows/index.html` (`rows-board.py`) and as ONE `*Demo path*` message in `#hermes-port` that `slack-rows.py` edits in place with `chat.update` (state key `demo_path` in `slack-threads.json`). Edit `demo-path.json` to change rungs, rows, hours or factors. `python3 demo_path.py --root <ROOT> --slack\|--html\|--json` to debug; tests: `python3 -m unittest ops/nemoclaw-coworkers/test_demo_path.py`. See `docs/hermes-port/autopilot.md` §10.2. | Read by both scripts on every `refresh-viewers.sh` run (15 min). Never fatal to either. |
...
| `autopilot/hermes_supervise.py` | the supervisor: row threads + fork PRs + nudge ledger (+ `acks.json`) → stage per row, SLO check, bounded `nudge` / `alert` / `gate` / `hold` actions (§2, §3, §5), plus the marker-less stalls of §2.5: `infra-hold` / `operator-ruling` alerts and Orchestrator re-arms for a role's hold line, re-arms + `bounce-repeat` for a bounced turn, an early nudge for a turn that ended without its hand-off |
...
| `autopilot/collect-acks.sh` | on the host (the container cannot read other sessions' `outbound.db`): the newest `processing_ack` per active `hermes-<ROW>` session → `data/shared/hermes/autopilot/acks.json`, every 15 min via `refresh-viewers.sh`; `pull-state.sh` passes it as `--acks`, stale (> 2 h) or missing = bounce/idle detection off for that tick |
