...
| `slack-rows.py` | Mirrors every Hermes row into one Slack thread in `#hermes-port` (`C0C14PWDUMC`): a root message per row (`<ROW> · <plan name> · batch <b> · dispatched <date>`), every role card PNG as a threaded upload (`files.getUploadURLExternal` → bytes → `files.completeUploadExternal`, caption `<ROW> · <role> · <OUTCOME> — <headline>`), then one `✅ merged <sha>` / `⛔ blocked` line from the ledger. Reads the same cards (`rows-board.scan_cards`), plan and ledger as the rows board; remembers what it posted in `data/shared/hermes/slack-threads.json`; `--max-posts` (12) per run; `--dry-run`. Bot token from `$SLACK_BOT_TOKEN` or the checkout's `.env`, host-side only, never printed. Exit 2 = bot not invited to the channel. Tests: `python3 -m unittest ops/nemoclaw-coworkers/test_slack_rows.py`. | Called by `refresh-viewers.sh` after the rows board (`>> logs/slack-rows.log`, never fatal). Idempotent. |
