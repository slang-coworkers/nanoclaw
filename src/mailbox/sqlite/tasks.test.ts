...
function insertBasicTask(
  ...
  processAfter: string = new Date().toISOString(),
) {
  insertTaskRow(db, {
    id,
    seriesId: id,
...
// updateTask only reaches future-pending or paused occurrences (a due row is
// already an execution candidate), so the toggle tests must seed a future one.
...
describe('updateTask — new_session toggle (default-on semantics)', () => {
  ...
  it('setting newSession=true writes new_session:true into content', () => {
    const db = freshDb();
    insertBasicTask(db, 'task-1', '*/5 * * * *', FUTURE);
  ...
  it('setting newSession=false PERSISTS new_session:false (opt-out)', () => {
    const db = freshDb();
    insertBasicTask(db, 'task-2', '*/5 * * * *', FUTURE);
