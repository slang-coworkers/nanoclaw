...
export async function requestCreateAgentHold(content: Record<string, unknown>, session: Session): Promise<void> {
  ...
  // Preserve the dashboard sidebar group across the approval round-trip: an
  // approved replay re-enters createAgent with this payload as its `content`.
  ...
  await requestApproval({
    ...
    action: 'create_agent',
    payload: { name, instructions, group: rawGroup },
...
export async function createAgent(
  ...
  // any other value is a user id the coworker is scoped under. On an approved
  // replay `content` is the stored hold payload, which carries `group`.
  const rawGroup = typeof content.group === 'string' ? content.group : null;
