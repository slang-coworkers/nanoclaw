 ...
 *
 * Restart honesty: at startup, surviving rows re-arm and already-expired ones
 * get an honest timeout edit. Resolution on this fork is MAP-keyed, not
 * row-keyed — the in-memory entry is the duplicate-click claim (see the
 * fork-override note in `resolveOneCLIApproval`), so upstream's late-decision
 * path for a card whose process died is deliberately not wired in. A periodic
...
export async function resolveOneCLIApproval(approvalId: string, selectedOption: string): Promise<boolean> {
  ...
  // `state` is non-null: the early return above is the fork's duplicate-click
  // claim, so a card whose entry is gone never reaches here. Upstream's
  // late-decision path is unreachable under that claim and is therefore not
  // wired in — see the fork-override note above.
...
async function handleRequest(request: GatewayApprovalRequest): Promise<Decision> {
  ...
  await createPendingApproval({
    ...
    // The DM'd approver is the one identity allowed to resolve this card. Null
    // when no approver had a reachable DM: upstream early-returns 'deny' in that
    // case so it can dereference target here, but this fork keeps the row for
    // dashboard resolution, so the column stays open rather than naming an
    // approver who was never messaged.
