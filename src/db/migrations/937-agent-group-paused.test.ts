...
/**
 * migration937 is declared as the `Migration` union, whose `up` takes either the
 * portable driver or a raw SQLite handle. Narrow on the discriminant so the test
 * hands it the driver its own implementation actually accepts, while the
 * assertions below keep querying the raw handle for PRAGMA introspection.
 */
async function runMigration937(raw: Database.Database): Promise<void> {
  if (migration937.sqliteOnly) throw new Error('migration937 is portable — expected the DbDriver signature');
  await migration937.up(new SqliteDriver(raw));
}

describe('migration937 — agent_groups.paused', () => {
  it('adds the paused column as NOT NULL DEFAULT 0', async () => {
    const db = baseAgentGroups();
    db.prepare('INSERT INTO agent_groups (id, folder) VALUES (?, ?)').run('ag-1', 'orchestrator');

    await runMigration937(db);
  ...
  it('backfills existing rows to 0 (not paused) — no behaviour change for current groups', async () => {
    const db = baseAgentGroups();
    db.prepare('INSERT INTO agent_groups (id, folder) VALUES (?, ?)').run('ag-1', 'orchestrator');
  ...
  it('is idempotent — re-running does not throw or duplicate the column', async () => {
    const db = baseAgentGroups();
    await runMigration937(db);
    await expect(runMigration937(db)).resolves.toBeUndefined();
    const n = (
  ...
  it('a paused row round-trips 0/1', async () => {
    const db = baseAgentGroups();
    await runMigration937(db);
