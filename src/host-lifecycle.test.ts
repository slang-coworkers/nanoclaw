...
describe('host lifecycle orchestration', () => {
  it('starts after delivery is ready and before delivery polling', () => {
    ...
    // Matches the call, not its argument: nv-main builds the adapter inline and
    // passes a local, so upstream's literal `createChannelDeliveryAdapter()`
    // needle never appears here. The invariant under test is the ORDERING.
    const deliveryReady = source.indexOf('setDeliveryAdapter(');
