...
export async function buildLocalAgentEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    // Session identity forwarded to hook child processes — the curl hook in
    // settings.json reads these to stamp X-NanoClaw-Session-Id /
    // X-NanoClaw-Session-Thread-Id so /api/hook-event can route events to
    // the right session. Docker sets these via `-e`; local mode needs them
    // on the bun env the Claude SDK inherits when it spawns hook commands.
    NANOCLAW_SESSION_ID: session.id,
    NANOCLAW_SESSION_THREAD_ID: session.thread_id ?? '',
    ...
    // Fresh coworker session — do NOT inherit the host-side Claude Code's
    // CLAUDE_CODE_FORK_SUBAGENT=1 flag. If nanoclaw itself was launched from
    // inside another claude-code process (dev setup), that flag propagates
    // via `...process.env` above and makes in-container hooks (e.g.
    // workflow-state-reset.sh) believe every coworker is a subagent and
    // skip state initialization — breaking the plan/critique gates.
    // Container mode is immune because docker doesn't forward this env.
  ...
  for (const key of [
    ...
    // CLAUDE_CODE_FORK_SUBAGENT intentionally NOT forwarded — we override to '0'
    // above so workflow-state-reset.sh and subagent-scoped logic in hooks don't
    // treat coworker bun children as subagents of the nanoclaw parent.
    'CODEX_PROFILE',
    'CODEX_HOME',
    'CODEX_BASE_URL',
    'CODEX_MODEL',
