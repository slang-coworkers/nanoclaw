 ...
 * contract, and the singleton duties tolerate running alongside sessions
 * (spawn re-heals the egress network itself; the scans are central queries,
 * and the two cost reconcilers reach sessions only through `wakeContainer`,
 * whose `wakePromises` map dedups concurrent wakes per session). 8 is a modest
 * fan-out for a remote store; local SQLite is synchronous IO and neither gains
 * nor loses.
 */
...
export function startHostSweep(): void {
  ...
  queue = createReconcileQueue({
    ...
    singletons: {
      ...
      // Cost-approval reconciler: 24h expiry-dismiss, card resend, and override
      // re-drive for any decision whose enqueue threw. No-op under the S1 flag.
      // Central scan, once/tick — but note the re-drive path reaches a session
      // through routeCostOverrideToSession (writeSessionMessage + requestWake),
      // so it is not purely central. That is safe alongside the session's own
      // reconcile: the wake dedups per session, and the message write is a
      // deterministic upsert that also feeds the queue.
      ...
      // count. Central scan, once/tick. Its own wake failures are caught and
      // persisted as next_attempt_at, so they never reach the queue's per-key
      // backoff; only an escaping error (the central query, or the
      // attempt-recording write) does.
  ...
  // Registering this is also what makes session-manager's existing
  // enqueueSessionReconcile calls do anything: before the port nothing was
  // registered, so every mail-write enqueue was dropped.
